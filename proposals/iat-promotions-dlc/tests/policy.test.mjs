...
const mutations = [
  ...
  ["attestation domain drift", (p) => (p.identityAttestation.domain = "different-domain")],
  ["long-lived attestation", (p) => (p.identityAttestation.maximumLifetimeSeconds = 3_600)],
  ["raw X user ID disclosure", (p) => (p.identityAttestation.rawXUserIdInEnvelope = true)],
  ["disabled transparency log", (p) => (p.publicEvidence.appendOnlyTransparencyLog = false)],
  ["uncheckpointed transparency history", (p) => (p.publicEvidence.checkpointedHashChain = false)],
