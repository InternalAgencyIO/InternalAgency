...
- three required-key collision controls spanning literal keys and escaped ASCII
  spellings of `candidate` and `transportMarker`;
- twelve pre-candidate collision rejections covering six decoded-key aliases
  and six normalization lookalikes that remain distinct unexpected keys in
  Node and zero-dependency Python;
- key-collision control-set commitment
...
- four canonical marker-value controls spanning the literal marker, escaped
  ASCII, escaped solidus, and fully escaped canonical ASCII sequence;
- sixteen pre-candidate marker-value rejections covering three raw controls,
  four escaped controls, three case variants, four NFKC-equivalent variants,
  and two cross-script confusables in Node and zero-dependency Python;
- marker-value control-set commitment
...
- four valid UTF-8 byte-ingress controls spanning one-, two-, three-, and
  four-byte Unicode scalar encodings;
- sixteen fatal pre-JSON byte rejections covering four truncated sequences,
  four overlong encodings, four surrogate-code-point encodings, and four
  invalid continuation patterns in Node and zero-dependency Python;
- fatal UTF-8 control-set commitment
  `68621bc5d34259865bb893e895f62c5eb1e130b11f5d68a99f201d42c1e867d0`,
  rejection-set commitment
...
- three ordinary bounded-view controls proving a pre-decode copy preserves
  bytes and candidate meaning after candidate, marker, and excluded-prefix
  alias mutations;
- full, bounded, and empty `SharedArrayBuffer`-backed views rejected as
  `SHARED_BYTE_VIEW_UNSAFE` before snapshot, UTF-8, JSON, or candidate work;
- immutable-input snapshot control-set commitment
  `911a9435376122ebd6344df9db9532528c50ad8ed5c3ee2bca8e135076ac9fc0`,
  shared-rejection commitment
  `0648ff010573042b6a8c1a61c5839654b514af85adc408bf9654988a6a767d3f`,
  and combined replay commitment
  `18c32f8123725d75976f27737e193409f5848a305766646f3d6b37300ce88b69`.

Current proposal-only result: **510 tests passed**. This consists of 28 protected
...
transport suite.
Twelve required-key collision deterministic regeneration, decoded-equality
rule, escaped-alias order, dual-escape, normalization-distinct, error-boundary,
compact-publication, independent Python parity, tamper-exit, source-safety, and
manifest-coverage tests complete the collision-boundary suite.
Twelve transport-marker value deterministic regeneration, exact-value rule,
escaped-canonical control, raw-control syntax, escaped-control semantics,
case-variant, NFKC-equivalent, cross-script confusable, error-boundary,
independent Python parity, tamper-exit, source-safety, and manifest-coverage
tests complete the exact marker-value suite.
Twelve fatal UTF-8 deterministic regeneration, fatal-decoder rule,
one/two/three/four-byte scalar controls, truncated, overlong,
...
Twelve immutable-input snapshot deterministic regeneration, fixed copy rule,
independent-buffer, candidate and marker mutation isolation, excluded-prefix
control, full/bounded/empty shared-view rejection, pre-decode error-boundary,
compact-publication, independent Python parity, tamper-exit, source-safety, and
manifest-coverage tests complete the immutable parser-input suite.
...
Add a compact detached-`ArrayBuffer` view corpus proving detached full, bounded,
and empty `Uint8Array` inputs reject before decode without collapsing into valid
empty input. Keep bytes and candidates runtime-only; do not contact a local
