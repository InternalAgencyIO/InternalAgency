/**
 * Generates compact canonical-safe-integer JSON token evidence.
...
import {
  evaluateNumericTokenCorpus,
  NUMERIC_TOKEN_RULES,
} from "./settlement-contention-composition-transport-limits.mjs";

const OUTPUT_PATH = fileURLToPath(new URL("./settlement-contention-composition-numeric-token-audit.v1.json", import.meta.url));
...
export function generateNumericTokenAudit() {
  const base = parse(BASE_PATH);
  const { controls, rejections } = evaluateNumericTokenCorpus(base);
  return {
    vectorVersion: 1,
    vectorId: "iat-promotions-dlc-contention-composition-numeric-tokens-v1",
    ...
    sources: {
      ...
      generator: { path: "generate-settlement-contention-composition-numeric-token-audit.mjs", normalizedTextSha256: normalizedTextSha256(GENERATOR_PATH) },
    },
    contract: {
      mode: "CANONICAL_SAFE_INTEGER_JSON_TRANSPORT",
      numericTokenRules: NUMERIC_TOKEN_RULES,
      ...
      equivalentSpellingsRejectedBeforeCandidate: true,
      negativeZeroRejectedBeforeCandidate: true,
      unsafeIntegersRejectedBeforeCandidate: true,
      nonfiniteEquivalentsRejectedBeforeCandidate: true,
    ...
    summary: {
      ...
      allNoncanonicalOrUnsafeTokensRejectedBeforeCandidate: true,
...
export function renderNumericTokenAudit() {
  return `${JSON.stringify(generateNumericTokenAudit(), null, 2)}\n`;
}

if (process.argv[1] === fileURLToPath(import.meta.url)) {
  if (process.argv.includes("--write")) {
    writeFileSync(OUTPUT_PATH, renderNumericTokenAudit(), "utf8");
    console.log("Wrote four canonical controls and sixteen pre-candidate numeric-token rejections; serialized inputs and candidates remain runtime-only.");
  } else {
    process.stdout.write(renderNumericTokenAudit());
  }
}
