...
import { ecuadorPalette, ecuadorProhibitions, ecuadorSceneSpecs } from "./ecuador-scene-specs.mjs";

const batch = 355;
const country = "Ecuador";
const countrySlug = "ecuador";
const firstScene = 1440;
...
const commonProhibitions = ecuadorProhibitions;
...
const sceneSpecs = ecuadorSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Ecuador location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Netherlands",
  nextQueueBatch: 356,
  nextQueueScenes: [1444, 1445, 1446, 1447],
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Ecuador flag, coat of arms, official seal, sacred symbol, copied ceremonial or traditional pattern, political messaging, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular Andean ridges, volcanic cones, equatorial arcs, river braids, orchid forms and Pacific-current spirals instead.",
    palette,
    ...
    cultureScene: "The scenes foreground Quito's historic center, Cotopaxi, Cuenca's Tomebamba riverfront, and Bartolome Island in the Galapagos.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Ecuador images plus one accepted Guatemala image",
    captionIfEligible: `Ecuador ${heartGlyph} Guatemala ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-355-ecuador-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-355-ecuador-preflight.json"),
