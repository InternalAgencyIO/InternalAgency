...
import { belgiumPalette, belgiumProhibitions, belgiumSceneSpecs } from "./belgium-scene-specs.mjs";

const batch = 366;
const country = "Belgium";
const countrySlug = "belgium";
const firstScene = 1484;
const root = path.resolve("tmp/world-195x4/batch-366");
...
const commonProhibitions = belgiumProhibitions;
...
const sceneSpecs = belgiumSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Belgium location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Haiti",
  nextQueueBatch: 367,
  nextQueueScenes: [1488, 1489, 1490, 1491],
  nextCinematicTheme: { active: "deep-sea submersible couture", batchOrdinalWithinTheme: 1 },
  researchSources: [
    { url: "https://www.britannica.com/place/Brussels", usedFor: "Brussels Grand Place and city setting" },
    { url: "https://www.britannica.com/place/Belgium", usedFor: "Belgium cities, rivers and regional geography" },
    { url: "https://whc.unesco.org/en/list/996/", usedFor: "Bruges historic center and canal architecture" },
    { url: "https://www.britannica.com/place/Antwerp-Belgium", usedFor: "Antwerp Scheldt waterfront and port setting" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Belgium flag, coat of arms, official seal, sacred symbol, copied ceremonial or traditional pattern, political messaging, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular guildhall rooflines, canal currents, belfry arcs, port geometry, river bands and limestone cliff profiles instead.",
    palette,
    minimumCoverage: "Scenes 1484 and 1485 each carry hard large Belgium motifs on three women and solar-observation construction language on at least two. Scenes 1486 and 1487 use four different theme-led solar-observation outfits without country map prints while Belgium landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Brussels Grand Place, Bruges canals, Antwerp waterfront, and Dinant on the Meuse.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Belgium images plus one accepted South Sudan image",
    captionIfEligible: `Belgium ${heartGlyph} South Sudan ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-366-belgium-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-366-belgium-preflight.json"),
