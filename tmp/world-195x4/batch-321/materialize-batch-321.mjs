...
const palette = "lagoon turquoise, reef cobalt, coral pink, palm green, sand white, manta charcoal, sunset gold, mangrove jade, monsoon silver, and deep-ocean navy";
...
const commonProhibitions = "Use respectful secular Maldivian atoll geometry, coral reefs, lagoons, sandbanks, mangroves, wetlands, causeways, research overlooks, navigation buoys, boats, and public paths only. No literal flag, crescent, coat of arms, official seal, sacred symbol, religious building, copied ceremonial pattern, copied uniform, badge, readable text, brand, alcohol, or political insignia.";
...
if (maleScene !== 1304) throw new Error(`Male scene drifted to ${maleScene}`);

const weatherSafeText = (result, scene) => {
  if (scene === 1305 && result === "snow flurries") return "the stored snow-flurries weather roll appears as windblown white sea-spray flakes in tropical air, never snow accumulation";
  return result;
};
...
const garment = (character, plan, spec, index) => {
  const colors = ["lagoon turquoise", "coral pink", "mangrove jade", "deep-ocean navy"];
...
for (const spec of sceneSpecs) {
  ...
  const propAction = poseAction(poseTargetRoll.roll, handler, spec.target);
  ...
  if (hasMale) outfits.Male = "the established Scene 1136 adult male face with closely trimmed beard and athletic build, an opaque fitted short-sleeve sand-white top carrying a large complete Baa Atoll lagoon and manta-current composition, fitted black jeans, and practical black boots";
  ...
  const renderPrompt = [
    ...
    `Place large complete secular Maldives motifs across at least two outfits as dominant full bodice, skirt, hip, and panel fields: ${spec.motifs}. ${commonProhibitions}`,
    `Four visibly distinct coherent emotions: ${emotions}${hasMale ? `; Male: roll ${maleEmotion.roll}=${maleEmotion.result}, performed as ${spec.emotionNuance.Male}` : ""}.`,
    ...
    spec.composition,
    `Exact hand inventory, no others: ${spec.hands.join("; ")}.`,
    ...
    hosieryLine,
    paws.active ? spec.pawsPlan : "PAWS is inactive. No kitten.",
    ...
    rainbowOnly.active ? `Rainbow-only is active across the outfits while preserving complete Maldives motifs.` : "Rainbow-only is inactive; do not convert the wardrobe to rainbow-only styling.",
  ...
  scenePlans[String(spec.scene)] = {
    ...
    pawsPlan: paws.active ? spec.pawsPlan : null,
...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch321-maldives keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  nextQueueCountry: "Cabo Verde",
  nextQueueBatch: 322,
  nextQueueScenes: [1308, 1309, 1310, 1311],
  researchSources: [
    { url: "https://www.unesco.org/en/mab/baa-atoll", usedFor: "Baa Atoll lagoons, faros, micro-atolls, patch reefs, coral islands, seagrass, mangroves, and tropical monsoon setting" },
    { url: "https://www.unesco.org/mab/50anniversary/en/baa-atol", usedFor: "Hanifaru Bay, manta aggregation, heart-shaped atoll reef, bright shallows, reef habitats, and marine conservation" },
    { url: "https://www.unesco.org/en/mab/fuvahmulah", usedFor: "Fuvahmulah single-island atoll, shallow-bowl form, kilhi wetlands, mangroves, fringing reef, and Thoondu pebble beach" },
    { url: "https://www.unesco.org/en/articles/panna-india-fuvahmulah-and-addu-atoll-maldives-join-unescos-world-network-biosphere-reserves", usedFor: "Addu horseshoe atoll ecosystem, lagoons, reef passes, seagrass, sandbanks, coral islands, mangroves, wetlands, and linked settlements" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Maldives flag, crescent, coat of arms, official emblem, or sacred symbol is copied. Large researched secular atoll, lagoon, reef, sandbank, mangrove, wetland, causeway, manta-current, pebble-beach, cloud, and route compositions replace them.",
    palette,
    minimumCoverage: "Every image carries large complete secular Maldives compositions across at least two outfits as dominant full bodice, skirt, hip, or panel fields.",
    cultureScene: "The scenes foreground Baa Atoll and Hanifaru Bay, Fuvahmulah and Thoondu, Addu Atoll's linked island landscape, and Baa micro-atolls.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Maldives images plus one accepted Malta image",
    captionIfEligible: `Maldives ${heartGlyph} Malta ${hashtags.join(" ")}`,
  ...
  anatomyGate: {
    fourPersonScenes: "Scenes 1305, 1306, and 1307 require exactly eight arms and eight hands, two per woman, with every limb continuously traceable to one owner.",
    fivePersonScene: "Scene 1304 requires exactly ten arms and ten hands, two per adult, with the male added without replacing any woman.",
...
fs.writeFileSync(path.join(root, "batch-321-maldives-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-321-maldives-preflight.json"),
