...
import { micronesiaPalette, micronesiaProhibitions, micronesiaSceneSpecs } from "./micronesia-scene-specs-v2.mjs";

const batch = 335;
const country = "Micronesia";
const countrySlug = "micronesia";
const firstScene = 1360;
const root = path.resolve("tmp/world-195x4/batch-335");
...
const palette = micronesiaPalette;
...
const commonProhibitions = micronesiaProhibitions;
...
const sceneSpecs = micronesiaSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Micronesia location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch335-micronesia keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct. Batch 335 also stores one hard-love roll, mascot-state roll and holder selector, and odd-prop activation, holder, and family selectors per scene.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  nextQueueCountry: "Tonga",
  nextQueueBatch: 336,
  nextQueueScenes: [1364, 1365, 1366, 1367],
  nextCinematicTheme: { active: "civilian helicopter flight couture", batchOrdinalWithinTheme: 2 },
  researchSources: [
    { url: "https://visit-micronesia.fm/pohnpei/", usedFor: "Pohnpei, Nan Madol basalt islets and canals, mangroves, volcanic ridges, and lagoon setting" },
    { url: "https://visit-micronesia.fm/kosrae/", usedFor: "Kosrae, Lelu archaeological walls, mangroves, rainforest ridges, channels, and reef water" },
    { url: "https://visit-micronesia.fm/chuuk/", usedFor: "Chuuk Lagoon reef ring, high volcanic islands, passes, coral shelves, and Pacific horizon" },
    { url: "https://visit-micronesia.fm/yap/", usedFor: "Yap rai-stone heritage paths, tropical greenery, taro terraces, lagoon water, and low ridges" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Micronesia flag, coat of arms, official emblem, sacred symbol, religious building, ceremonial pattern, or ceremonial attire is copied. Country-led scenes use large dimensional secular basalt-log, tidal-canal, mangrove-root, breadfruit-leaf, island-chain, Kosrae-ridge, reef, and current compositions instead.",
    palette,
    minimumCoverage: "Scenes 1360 and 1361 each carry hard large Micronesia motifs on three women and civilian-helicopter construction language on at least two. Scenes 1362 and 1363 use four different theme-led civilian-helicopter outfits without country map prints while the Micronesia landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Nan Madol on Pohnpei, Lelu on Kosrae, Chuuk Lagoon, and a Yap stone-money heritage path.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Micronesia images plus one accepted Saint Lucia image",
    captionIfEligible: `Micronesia ${heartGlyph} Saint Lucia ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-335-micronesia-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-335-micronesia-preflight.json"),
