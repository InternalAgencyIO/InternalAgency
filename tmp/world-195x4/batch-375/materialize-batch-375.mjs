...
import { namibiaPalette, namibiaProhibitions, namibiaSceneSpecs } from "./namibia-scene-specs.mjs";

const batch = 375;
const country = "Namibia";
const countrySlug = "namibia";
const firstScene = 1520;
const root = path.resolve("tmp/world-195x4/batch-375");
...
const commonProhibitions = namibiaProhibitions;
...
const sceneSpecs = namibiaSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Namibia location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Lithuania",
  nextQueueBatch: 376,
  nextQueueScenes: [1524, 1525, 1526, 1527],
  nextCinematicTheme: { active: "civilian helicopter flight couture", batchOrdinalWithinTheme: 2 },
  researchSources: [
    { url: "https://www.britannica.com/place/Namibia", usedFor: "Namibia geography, cities and regions" },
    { url: "https://www.britannica.com/place/Windhoek", usedFor: "Windhoek highland city setting" },
    { url: "https://whc.unesco.org/en/list/1430/", usedFor: "Namib Sand Sea and Sossusvlei setting" },
    { url: "https://www.britannica.com/place/Etosha-National-Park", usedFor: "Etosha salt-pan landscape" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Namibia flag, coat of arms, official seal, sacred symbol, copied traditional pattern, copied ceremonial pattern, political messaging, copied military identity, police identity, emergency-service identity, or official service identity is used. Country-led scenes use large dimensional secular mountain basins, civic rooflines, dune arcs, clay-pan bands, surf lines, salt-pan curves and acacia silhouettes instead.",
    palette,
    minimumCoverage: "Scenes 1520 and 1521 each carry hard large Namibia motifs on three women and civilian-helicopter construction language on at least two. Scenes 1522 and 1523 use four different theme-led civilian-helicopter outfits without country map prints while Namibia landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Windhoek, Sossusvlei, the Skeleton Coast, and Etosha.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Namibia images plus one accepted Armenia image",
    captionIfEligible: `Namibia ${heartGlyph} Armenia ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-375-namibia-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-375-namibia-preflight.json"),
