...
const countrySlug = "seychelles";
const firstScene = 1352;
const root = path.resolve("tmp/world-195x4/batch-333");
...
const palette = "granite silver, Indian Ocean turquoise, coco-de-mer green, coral rose, cinnamon brown, cloud white, deep ocean navy, sunset gold, black-parrot charcoal, and hibiscus magenta";
...
const commonProhibitions = "Use respectful secular Seychelles island-chain silhouettes, granite-boulder structures, coco-de-mer leaf fans, cinnamon-bark spirals, reef crescents, takamaka-leaf rhythms, mountain ridges, beach arcs, cloud bands, current lines, and public conservation overlooks only. No literal flag, coat of arms, official seal, sacred symbol, religious building, copied ceremonial pattern, ceremonial attire, copied uniform, airline branding, badge, readable text, alcohol, military imagery, or political insignia.";
...
const sceneSpecs = [
  {
    ...
    motifs: "Seychelles island-chain silhouette, Victoria clock-tower geometry, Morne Seychellois ridge, granite-boulder facets, coco-de-mer leaf fans, cinnamon spirals, reef crescents, and Indian Ocean current arcs",
  ...
  },
  {
    ...
    motifs: "Seychelles island-chain silhouette, Vallee de Mai palm-crown fans, coco-de-mer leaf ribs, cinnamon-bark spirals, granite outcrop facets, black-parrot wing arcs, mountain openings, and forest route lines",
  ...
  },
  {
    ...
    culture: "Anse Source d'Argent remains an equally dominant Seychelles foreground through its pale granite boulders, shallow turquoise water, reef shelves, sand arcs, takamaka trees, and distant islands. All four outfits are private-jet aviation originals with no forced country maps or matching surface prints.",
    motifPolicy: "Theme-led scene. All four outfits use different private-jet construction ideas and no garment uses an all-over Seychelles map print. The landmark itself carries the country read at equal visual weight.",
  ...
  },
  {
    ...
    culture: "Aldabra remains an equally dominant Seychelles foreground through its vast lagoon, raised coral rim, mangrove channels, reef flats, open ocean, distant wildlife, and conservation route. All four outfits are private-jet aviation originals with no forced country maps or matching surface prints.",
...
for (const spec of sceneSpecs) {
  ...
  if (hasMale) outfits.Male = "the established Scene 1136 adult male face with closely trimmed beard and athletic build, an opaque fitted short-sleeve cloud-white top with restrained Seychelles leaf-rib embroidery and subtle private-jet wing-sweep seaming, fitted black jeans, and practical black boots";
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Seychelles location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch333-seychelles keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct. Batch 333 also stores one hard-love roll, mascot-state roll and holder selector, and odd-prop activation, holder, and family selectors per scene.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  nextQueueCountry: "Grenada",
  nextQueueBatch: 334,
  nextQueueScenes: [1356, 1357, 1358, 1359],
  nextCinematicTheme: { active: "private-jet aviation couture", batchOrdinalWithinTheme: 2 },
  researchSources: [
    { url: "https://www.seychelles.com/en/explore/islands/mahe", usedFor: "Victoria, Mahe, Morne Seychellois, granite slopes, harbor, and reef setting" },
    { url: "https://whc.unesco.org/en/list/261/", usedFor: "Vallee de Mai, coco-de-mer palm forest, granite, and conservation context" },
    { url: "https://www.seychelles.com/en/explore/islands/la-digue", usedFor: "Anse Source d'Argent, granite boulders, shallow water, reef, sand, and takamaka setting" },
    { url: "https://whc.unesco.org/en/list/185/", usedFor: "Aldabra Atoll lagoon, raised coral rim, mangroves, reefs, wildlife, and conservation context" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Seychelles flag, coat of arms, official emblem, sacred symbol, religious building, ceremonial pattern, or ceremonial attire is copied. Country-led scenes use large dimensional secular island-chain, granite, coco-de-mer leaf, cinnamon, ridge, reef, and current compositions instead.",
    palette,
    minimumCoverage: "Scenes 1352 and 1353 each carry hard large Seychelles motifs on three women and private-jet construction language on at least two. Scenes 1354 and 1355 use four different theme-led private-jet outfits without country map prints while the Seychelles landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Victoria and Morne Seychellois, Vallee de Mai, Anse Source d'Argent, and Aldabra Atoll.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Seychelles images plus one accepted Saint Lucia image",
    captionIfEligible: `Seychelles ${heartGlyph} Saint Lucia ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-333-seychelles-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-333-seychelles-preflight.json"),
