...
import { senegalPalette, senegalProhibitions, senegalSceneSpecs } from "./senegal-scene-specs.mjs";

const batch = 353;
const country = "Senegal";
const countrySlug = "senegal";
const firstScene = 1432;
...
const commonProhibitions = senegalProhibitions;
...
const sceneSpecs = senegalSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Senegal location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Guatemala",
  nextQueueBatch: 354,
  nextQueueScenes: [1436, 1437, 1438, 1439],
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Senegal flag, official seal, sacred symbol, copied ceremonial or traditional pattern, political messaging, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular river currents, island-quay geometry, wetland channels, mangrove fans, shell-islet arcs and Atlantic wave bands instead.",
    palette,
    ...
    cultureScene: "The scenes foreground Dakar's Atlantic coast, Saint-Louis island and river mouth, Djoudj wetlands, and the Saloum Delta.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Senegal images plus one accepted Somalia image",
    captionIfEligible: `Senegal ${heartGlyph} Somalia ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-353-senegal-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-353-senegal-preflight.json"),
