...
import { grenadaPalette, grenadaProhibitions, grenadaSceneSpecs } from "./grenada-scene-specs.mjs";

const batch = 334;
const country = "Grenada";
const countrySlug = "grenada";
const firstScene = 1356;
const root = path.resolve("tmp/world-195x4/batch-334");
...
const commonProhibitions = grenadaProhibitions;
...
const legacyGrenadaDraftSceneSpecs = [
  {
    scene: 1356,
    ...
    motifs: "Grenada island-chain silhouette, Victoria clock-tower geometry, Morne Seychellois ridge, granite-boulder facets, coco-de-mer leaf fans, cinnamon spirals, reef crescents, and Indian Ocean current arcs",
    culture: "Victoria and Mahe are presented through secular civic and natural geography: the clock-tower silhouette, Morne Seychellois ridge, granite slopes, harbor, reef water, tropical vegetation, and public overlook. Private-jet aviation couture appears through original wing-sweep tailoring, turbine pleats, oval-window geometry, cabin-light piping, polished composite panels, and runway-line footwear without airline branding or copied uniforms.",
    motifPolicy: "Hard country motifs dominate Radiance, Ellie, and Alia through three different dimensional techniques. Radiance and ECE carry the private-jet construction language, so at least one look is a true country-theme hybrid and ECE remains theme-led. No repeated all-over maps or palette-swapped copies.",
  ...
  },
  {
    ...
    motifs: "Grenada island-chain silhouette, Vallee de Mai palm-crown fans, coco-de-mer leaf ribs, cinnamon-bark spirals, granite outcrop facets, black-parrot wing arcs, mountain openings, and forest route lines",
    culture: "Vallee de Mai is presented through secular forest conservation, towering coco-de-mer palms, huge fan leaves, cinnamon trunks, granite outcrops, filtered light, and an empty protected path. Private-jet aviation couture appears through original wing structures, turbine pleats, oval-window forms, cabin-light piping, polished composite tailoring, and runway geometry without airline branding or copied uniforms.",
    motifPolicy: "Hard country motifs dominate Radiance, Ellie, and Alia through leaf architecture, cinnamon weaving, granite relief, and bird-wing abstraction. Alia and ECE carry unmistakable private-jet construction language. No repeated map-print surfaces.",
  ...
  },
  {
    ...
    culture: "Anse Source d'Argent remains an equally dominant Grenada foreground through its pale granite boulders, shallow turquoise water, reef shelves, sand arcs, takamaka trees, and distant islands. All four outfits are private-jet aviation originals with no forced country maps or matching surface prints.",
    motifPolicy: "Theme-led scene. All four outfits use different private-jet construction ideas and no garment uses an all-over Grenada map print. The landmark itself carries the country read at equal visual weight.",
  ...
  },
  {
    ...
    culture: "Aldabra remains an equally dominant Grenada foreground through its vast lagoon, raised coral rim, mangrove channels, reef flats, open ocean, distant wildlife, and conservation route. All four outfits are private-jet aviation originals with no forced country maps or matching surface prints.",
    motifPolicy: "Theme-led scene. All four outfits use different private-jet silhouette and material systems. Country identity comes from the unmistakable Aldabra landmark, not repeated map graphics.",
...
const sceneSpecs = grenadaSceneSpecs;
...
const weatherSafeText = (result, scene) => {
  ...
  if (result === "heavy snow or cinematic blizzard") return "the stored cinematic-blizzard roll appears as a dense curtain of harmless white crater mist, windblown pale blossom petals, and cloud droplets beyond the shelter, with no snow accumulation and dry nonslip footing";
  return result;
};
...
for (const spec of sceneSpecs) {
  ...
  if (hasMale) outfits.Male = "the established Scene 1136 adult male face with closely trimmed beard and athletic build, an opaque fitted short-sleeve cloud-white top with restrained Grenada nutmeg-stem embroidery and subtle private-jet cabin-seam tailoring, fitted black jeans, and practical black boots";
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Grenada location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch334-grenada keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct. Batch 334 also stores one hard-love roll, mascot-state roll and holder selector, and odd-prop activation, holder, and family selectors per scene.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  nextQueueCountry: "Micronesia",
  nextQueueBatch: 335,
  nextQueueScenes: [1360, 1361, 1362, 1363],
  nextCinematicTheme: { active: "civilian helicopter flight couture", batchOrdinalWithinTheme: 1 },
  researchSources: [
    { url: "https://www.puregrenada.com/explore/st-georges/", usedFor: "St George's Carenage, colorful hillside facades, red-tile roofs, harbor, and volcanic ridge setting" },
    { url: "https://www.puregrenada.com/explore/grand-etang-national-park/", usedFor: "Grand Etang volcanic crater lake, cloud forest, giant ferns, spice trees, and ridgelines" },
    { url: "https://www.puregrenada.com/explore/annandale-waterfalls/", usedFor: "Annandale Falls, volcanic rock amphitheater, rainforest canopy, pool, and spice vegetation" },
    { url: "https://www.puregrenada.com/explore/levera-national-park/", usedFor: "Levera Beach, Sugar Loaf Island, Grenadine islets, reef shelf, and volcanic headland" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Grenada flag, coat of arms, official emblem, sacred symbol, religious building, ceremonial pattern, or ceremonial attire is copied. Country-led scenes use large dimensional secular nutmeg, mace, cocoa-pod, Carenage, crater-ring, fern, volcanic-ridge, waterfall, reef, and current compositions instead.",
    palette,
    minimumCoverage: "Scenes 1356 and 1357 each carry hard large Grenada motifs on three women and private-jet construction language on at least two. Scenes 1358 and 1359 use four different theme-led private-jet outfits without country map prints while the Grenada landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground St George's Carenage, Grand Etang Lake, Annandale Falls, and Levera Beach with Sugar Loaf Island.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Grenada images plus one accepted Saint Lucia image",
    captionIfEligible: `Grenada ${heartGlyph} Saint Lucia ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-334-grenada-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-334-grenada-preflight.json"),
