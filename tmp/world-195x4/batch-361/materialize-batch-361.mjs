...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Rwanda location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Burundi",
  nextQueueBatch: 362,
  nextQueueScenes: [1468, 1469, 1470, 1471],
  nextCinematicTheme: { active: "Mars-surface expedition couture", batchOrdinalWithinTheme: 2 },
  researchSources: [
    { url: "https://www.britannica.com/place/Kigali", usedFor: "Kigali rolling-hill city setting" },
    { url: "https://www.britannica.com/place/Rwanda", usedFor: "Rwanda rolling hills, lakes and volcanic geography" },
    { url: "https://whc.unesco.org/en/list/1697/", usedFor: "Nyungwe forest landscape and biodiversity context" },
    { url: "https://www.britannica.com/place/Lake-Kivu", usedFor: "Lake Kivu shoreline and island landscape" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Rwanda flag, coat of arms, official seal, sacred symbol, copied ceremonial or traditional pattern, political messaging, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular rolling-hill terraces, lake currents, volcanic profiles, canopy layers, tea-field ribbons and city-light arcs instead.",
    palette,
    minimumCoverage: "Scenes 1464 and 1465 each carry hard large Rwanda motifs on three women and Mars-expedition construction language on at least two. Scenes 1466 and 1467 use four different theme-led Mars expedition outfits without country map prints while Rwanda landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Kigali, Lake Kivu, the Virunga volcano chain, and Nyungwe rainforest.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Rwanda images plus one accepted Guinea image",
    captionIfEligible: `Rwanda ${heartGlyph} Guinea ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-361-rwanda-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-361-rwanda-preflight.json"),
