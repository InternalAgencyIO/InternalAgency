...
import { gabonPalette, gabonProhibitions, gabonSceneSpecs } from "./gabon-scene-specs.mjs";

const batch = 380;
const country = "Gabon";
const countrySlug = "gabon";
const firstScene = 1540;
const root = path.resolve("tmp/world-195x4/batch-380");
...
const commonProhibitions = gabonProhibitions;
...
const sceneSpecs = gabonSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Gabon location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Botswana",
  nextQueueBatch: 381,
  nextQueueScenes: [1544, 1545, 1546, 1547],
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Gabon flag, coat of arms, official seal, sacred symbol, copied traditional pattern, political messaging, copied military identity, police identity, emergency-service identity, or official service identity is used. Country-led scenes use large dimensional secular waterfront curves, Libreville skyline terraces, forest-savanna bands, river arcs and hill layers instead.",
    palette,
    minimumCoverage: "Scenes 1540 and 1541 each carry hard large Gabon motifs on three women and orbital-spaceship construction language on at least two. Scenes 1542 and 1543 use four different theme-led orbital-spaceship outfits without country map prints while Gabonese landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Libreville, Lopé, Loango, and Kongou Falls.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Gabon images plus one accepted Qatar image",
    captionIfEligible: `Gabon ${heartGlyph} Qatar ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-380-gabon-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-380-gabon-preflight.json"),
