...
const templatePromptPath = path.join(root, "scene-1551-clean-fresh-round-30-prompt.txt");
const scene = 1551;
const round = 31;
...
if (sha256File(contractPath) !== expectedContractSha) throw new Error("Authoritative contract changed before clean round 31");
if (sha256File(ledgerPath) !== expectedLedgerSha) throw new Error("X ledger changed before clean round 31");
if (!contract.romance?.radianceRealtimeAgreementPartyRule) throw new Error("Radiance agreement rule missing");
if (checkpoint.status !== "active-four-scene-gate-incomplete-after-clean-fresh-round-30") throw new Error(`Unexpected checkpoint status: ${checkpoint.status}`);
if (checkpoint.renderStrategyReset?.nextCleanRound !== round) throw new Error("Checkpoint does not authorize round 31");
if (checkpoint.renderAttempts.freshRound31) throw new Error("Round 31 was already materialized");
...
const invitationKey = "batch382-georgia-scene1551-radianceLiveInvitation-round31";
const responseKey = "batch382-georgia-scene1551-radianceLiveResponse-round31";
const participantsKey = "batch382-georgia-scene1551-radianceLiveParticipants-round31";
...
const radianceResponse = "Radiance explicitly redirects ECE's optional victory-dance invitation back to route work: while securely held in Ellie's shallow dip, she keeps warm direct eye contact with ECE, gives one clear small head shake, and extends her existing left arm in a fully visible open-palm sweep toward ECE's hands-free blue route map. ECE visibly accepts with one calm nod. No celebration begins.";
const radiancePartyState = {
  round,
  rollMethod: "FNV-1a over recorded round-31 live-narrative keys, reduced modulo 100",
  ...
  visibleResponseEvidence: [
    "Radiance's clear small head shake while looking warmly at ECE",
    "Radiance's fully visible open left palm sweeping toward ECE's hands-free route map",
    "Radiance and ECE's sustained warm mutual eye line",
    "ECE's calm accepting nod while both hands remain on the compass",
  ],
  continuityState: "Radiance redirects the optional celebration to route planning. partyActivation remains false for everyone. Ellie completes only the already-started public-safe dip, ECE accepts the redirect, and Alia remains in her separate mission lane.",
  consentScope: "This redirect applies only to this recorded round-31 invitation in scene 1551 and does not transfer to another act, participant, prop interaction, scene, country, or future image.",
};
if (invitationFullHash !== 3926323492 || invitationRoll !== 92 || invitationRoll % invitations.length !== 0) throw new Error("Round 31 invitation roll changed");
if (responseFullHash !== 2616210480 || responseRoll !== 80 || responseCategory !== "explicit redirect") throw new Error("Round 31 response roll changed");
if (participantsFullHash !== 952250325 || participantsRoll !== 25 || participantSet.join("|") !== "Radiance|Ellie|AI ECE") throw new Error("Round 31 participant roll changed");
if (partyActivation || willingParticipants.length !== 0) throw new Error("Round 31 party state mismatch");

const templatePrompt = fs.readFileSync(templatePromptPath, "utf8");
let prompt = templatePrompt.replaceAll("round 30", "round 31");
...
Use one clean 28 mm eye-level full-body exposure with generous plain negative space. At left-center, make the controlled dip unmistakable in silhouette: Ellie stands upright at 24 percent of frame width with feet shoulder-width and knees stable; Radiance is immediately in front of her at 39 percent, torso leaning back twenty-five degrees with her head clearly lower than Ellie's shoulder and both complete feet planted apart on the tiles. Put their one shoulder-height clasp far left in clean air. Ellie's separate white-sleeved support arm must be fully visible outside both torso silhouettes and end high on Radiance's uninterrupted open back. Radiance's separate left arm extends rightward through clean air toward the blue route map, ending in one large open redirect palm that touches nobody. Their securely covered hips meet at one stable support point. Put ECE front-on at 61 percent behind one narrow knee-high compass pedestal, directly inside Radiance's warm eye line. Put Alia large at 84 percent in rear-three-quarter strict right-facing profile inside a separate closed training lane. Her open-back strapless shell, clear narrow waist reveal, exact two arms, exact two hands, and full-size metallic inert training replica remain unobstructed. Keep every face, torso, leg, foot, shoulder, elbow, forearm, wrist, palm, and finger cluster visible. Reserve lower-left for the dry mascot lounge and the upper third for Batumi landmarks. No stacked body, cropped foot, layered foreground, or decorative clutter.`;
...
Romance roll 86 = Alia spins Radiance under linked hands while ECE steadies Radiance and Ellie reaches toward the beacon. Materialize the completed spin settling into Ellie's unmistakable shallow dip while Radiance redirects ECE's optional victory dance toward the route map. Alia stays separated in jealous profile. Do not add literal hands beyond the exact graph below.
Compound-love roll 28 = ECE stays close at Radiance's side while warmly greeting Ellie and Alia answers beside them. Materialize its emotional triangle through Radiance and ECE's affectionate redirect eye line, Ellie's willing support, and Alia's excluded tight-jaw profile.
Hard-love roll 40 = a controlled dance dip with stable planted feet and supported back, a second partner catching the dipped adult's free hand, and the fourth answering with visible jealousy or invitation. Ellie is the visible supporting partner: her left palm supports Radiance's open upper back and her right hand catches Radiance's right hand in a shoulder-height clasp. Radiance's securely covered cobalt hip rests against Ellie's covered white hip as a stable third contact. Alia is the fourth and shows excluded jealousy. These three and only three relationship contacts are large and unmistakable. Radiance's separate open redirect palm touches nobody.`;
...
Participant-selector key ${participantsKey}; full hash ${participantsFullHash}; roll ${participantsRoll}; hypothetical affirmative participants = ${participantSet.join(", ")}; willing participants = none because Radiance redirected.
Radiance response: ${radianceResponse}
partyActivation = FALSE. No one participates in an InternalAgency party beat. Show Radiance's clear small head shake, her fully visible open left palm sweeping toward the hands-free route map, and her sustained warm mutual eye line with ECE. ECE gives one calm accepting nod while keeping both hands on the compass. Ellie completes only the already-begun stable public-safe dip. Alia remains separate. A redirect is a complete valid story outcome. Add no crowd, drink, confetti, balloon, sign, text, stage, ornament, or party object. The redirect is limited to this exact invitation and image.`;
...
Ellie stands upright at left. Her white-sleeved left arm stays entirely outside both torso silhouettes and ends in one open left palm spread high on Radiance's uninterrupted open upper back. Her separate white-sleeved right arm extends leftward through clean air and ends at shoulder height in one clear palm-to-palm clasp with Radiance's right hand. Show both sleeves, elbows, forearms, wrists, palms, and finger clusters continuously.
Radiance performs a conventional shallow rear-three-quarter dip with her head lower than Ellie's shoulder and both complete feet planted. Her right arm extends leftward through clean air to the shoulder-height clasp with Ellie's right hand. Her separate left arm extends rightward toward ECE's blue map, ending in one large open redirect palm with all five fingers separated. It touches nobody. Show both shoulders, elbows, forearms, wrists, palms, and finger clusters continuously.
...
const promptAudit = {
  ...
  plannedPasses: {
    ...
    recoverySourceIfNeeded: "only the clean round 31 fresh raw",
  ...
  redesignedComposition: {
    dipOwnership: "Ellie supports Radiance in a diagonal shallow dip while Radiance redirects ECE toward the map",
    handGraph: {
      Ellie: ["left palm supports Radiance upper back", "right hand shoulder-height-clasps Radiance right hand"],
      Radiance: ["right hand shoulder-height-clasps Ellie right hand", "left open redirect palm sweeps toward ECE map without contact"],
      ECE: ["left compass handle", "right compass handle"],
      Alia: ["primary mission grip", "separate support grip"],
    },
    relationshipContacts: 3,
    partyGraph: "party inactive; Radiance explicitly redirects ECE through head shake and open palm toward route map",
    missionGeometry: "isolated right lane with open-back strapless shell, narrow waist reveal, full-size inert training replica, and complete backstop",
  },
...
plan.freshRound31 = { ...promptAudit, prompt };
plan.radianceRealtimeAgreementParty = radiancePartyState;
const preparedAt = new Date().toISOString();
checkpoint.status = "active-four-scene-gate-clean-fresh-round-31-materialized";
...
checkpoint.xPost.note = "Georgia has accepted scenes 1548, 1549, and 1550. Clean round 31 is materialized for missing scene 1551 from original identity anchors only. Publication remains mandatory after scene 1551 is accepted and the four-scene completion checkpoint is pushed.";
checkpoint.nextQueueStatus = "locked-on-Georgia-until-four-accepted-git-pushed-and-X-live-verified";
checkpoint.nextWakeAction = {
  country: "Georgia",
  batch: 382,
  action: "launch-clean-fresh-round-31-from-original-identity-anchors-scene-1551-only",
