...
import { georgiaPalette, georgiaProhibitions, georgiaSceneSpecs } from "./georgia-scene-specs.mjs";

const batch = 382;
const country = "Georgia";
const countrySlug = "georgia";
const firstScene = 1548;
const root = path.resolve("tmp/world-195x4/batch-382");
...
const commonProhibitions = georgiaProhibitions;
...
const sceneSpecs = georgiaSceneSpecs;
...
const resolveHardLove = (scene, handler, beat, oddHolder) => {
  const defaultOrders = {
    1548: ["Ellie", "Radiance", "Alia"],
    1549: ["Alia", "Ellie", "Radiance"],
    1550: ["Radiance", "Alia", "Ellie"],
    1551: ["Ellie", "Alia", "Radiance"],
  };
  const hosieryOrders = {
    1548: ["Radiance", "AI ECE", "Ellie"],
    1549: ["AI ECE", "Radiance", "Ellie"],
    1550: ["Radiance", "AI ECE", "Ellie"],
    1551: ["AI ECE", "Radiance", "Ellie"],
  };
  ...
  } else if (oddHolder && beat.includes("controlled dance dip")) {
...
for (const spec of sceneSpecs) {
  ...
  const resolvedLove = resolveHardLove(spec.scene, handler, hardLoveBeat.result, interestingProp.active ? interestingPropHolder.result : null);
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Georgia location must both be immediate foreground reads. ${spec.culture}`,
    stored,
    ...
    `${handler} alone actively handles one photorealistic full-size polished rainbow-gradient Desert Eagle-style inert cinema training prop. ${effectivePropAction}`,
  ...
  scenePlans[String(spec.scene)] = {
    ...
    poseTargetRoll,
    materializedPropAction: effectivePropAction,
...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch382-georgia keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct. Batch 382 also stores one hard-love roll, mascot-state roll and holder selector, and odd-prop activation, holder, and family selectors per scene.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  nextQueueCountry: "Fiji",
  nextQueueBatch: 383,
  nextQueueScenes: [1552, 1553, 1554, 1555],
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Georgia flag, coat of arms, official seal, sacred symbol, copied traditional pattern, political messaging, copied military identity, police identity, emergency-service identity, or official service identity is used. Country-led scenes use large dimensional secular river curves, suspension arcs, sulfur-dome scales, Caucasus ridges, gorge strata and balcony planes instead.",
    palette,
    minimumCoverage: "Scenes 1548 and 1549 each carry hard large Georgia motifs on three women and Mars-expedition construction language on at least two. Scenes 1550 and 1551 use four different theme-led Mars-surface outfits without country map prints while Georgian landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Tbilisi on the Mtkvari, Stepantsminda below Mount Kazbek, Sighnaghi over the Alazani Valley, and Batumi on the Black Sea.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 4,
    attachmentShape: "two Georgia images plus one accepted Honduras image",
    captionIfEligible: `Georgia ${heartGlyph} Honduras ${hashtags.join(" ")}`,
  ...
  renderAttempts: {
    ...
    recovery: { status: "not-started", maximumPerBlockedScenePerRound: 1, laterWakeFreshRoundsAllowed: true },
  },
  acceptedAssets: [],
  rejectedAssets: [],
  xPost: { status: "blocked-until-four-accepted-and-git-pushed", minimumCurrentCountryAcceptedAssets: 4 },
};
...
fs.writeFileSync(path.join(root, "batch-382-georgia-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-382-georgia-preflight.json"),
