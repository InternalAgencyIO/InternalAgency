...
const choreography = {
  1548: `The rolled hard-love beat is the first read: a moving three-person slow-dance arch while ECE steps through the open side as the excluded rival. Place Radiance back-left, Ellie slightly forward-center, and Alia back-right, with open sky visible between all three torsos. Radiance and Alia lift one hand each and link those two hands high above Ellie's head, fully silhouetted against sky. Four other hands stay on near-side surfaces: Radiance's other palm on Ellie's left shoulder; Ellie's left palm on Radiance's near-side waist; Ellie's right palm on Alia's near-side shoulder; Alia's other palm on Ellie's near-side waist. All six dance wrists and hands are fully visible and no arms cross in front of a face. ECE remains separate at far right and owns only the two prop hands. This yields exactly eight hands. Bent knees, turning hems, aligned romantic eye lines, and at least three contacts make it a real dance, not a lineup.`,
  1550: `The rolled hard-love beat is the first read: Alia sits securely sideways across Radiance's lap on a broad low bench, with both of Alia's feet planted and the bench carrying both adults. Radiance holds the three-balloon odd prop outside the embrace at far left with both hands. Alia uses one palm on Radiance's near-side shoulder and extends her other hand rightward to link with Ellie's near hand against open sky. Ellie stands immediately right of the bench, uses one hand in that single clear link, and places her other palm on Alia's outer shoulder. ECE remains separate at far right and owns only the two prop hands. Offset the adult male behind and camera-left of ECE so one male palm is fully visible on ECE's upper arm and the other fully visible on ECE's near-side waist. His strongest sustained eye line stays on ECE. Exactly ten hands are visible. No self-clasp, side-by-side substitute, hidden hand, or extra contact.`,
  1551: `The rolled hard-love beat is the first read: Ellie supports Radiance in a stable shallow backward dip at left. Rotate Radiance to a face-visible three-quarter rear angle so her open-back couture is clear. Ellie's first open palm lies visibly on Radiance's near-side waist and Ellie's second open palm lies visibly across the center of Radiance's blue upper back, each surrounded by contrasting fabric. Radiance places one visible hand on Ellie's near-side shoulder and reaches her other visible hand to ECE's near-side shoulder. ECE stands immediately beside the dip and owns only the two compass-table hands. Alia remains separate at far right and owns only the two prop hands. Exactly eight hands are visible, every wrist has an air gap, and no hand is behind a torso.`,
};

const mission = {
  1548: `Preserve the authoritative water target exactly. ECE stands at far right in strict side profile on the secured dry terrace and angles the orange-plugged inert training replica about 25 degrees down and right. Directly below-right is one fluorescent orange empty floating route disk centered in a narrow cordoned empty Mtkvari water lane. Camera geometry must show the uninterrupted diagonal muzzle-to-disk line ending at the disk itself. Open empty water fills the frame beyond the disk. All people, MAX, bridge, buildings, banks, boats, paths, and camera remain behind or left of the muzzle plane. Do not add a basin, paper wall target, laser beam, boat, person, animal, or occupied object downrange. Both of ECE's hands are separated and visible on the grip; the trigger finger is straight on the frame outside the guard.`,
  1550: `Preserve the authoritative basin target exactly. ECE stands at far right in strict side profile and angles the orange-plugged inert training replica about 20 degrees down and right. A transparent shallow blue cinema-safety basin sits alone on a secure pedestal immediately below-right. One fluorescent orange empty floating route disk is centered exactly on the diagonal muzzle axis. Camera geometry must show the uninterrupted muzzle-to-disk line ending at the disk itself. The basin is enclosed by transparent safety panels and separated from walls, paths, vineyards, and valley drops. Every other adult stays behind and left of the muzzle plane. No holographic ring, open-air target, person, animal, occupied object, camera-facing muzzle, ammunition, or magazine. Both of ECE's prop hands and indexed trigger finger are clear.`,
  1551: `Preserve the authoritative paper target exactly. Alia stands at far right in strict side profile inside the closed transparent-panel lane. The orange-plugged inert training replica is perfectly horizontal. One plain white paper square with one black non-humanoid route diamond is fixed to the thick sand backstop at the exact same height as the muzzle. The diamond center, muzzle center, and Alia's eye line form one horizontal line. The paper target must be directly in front of the muzzle, never below or above it. Every person and mascot remains behind and left of Alia's muzzle plane. Both of Alia's hands are separated and visible on the grip; the trigger finger is straight on the frame outside the guard. No text, ammunition, magazine, firing, threat, or camera-facing muzzle.`,
};
...
for (const scene of scenes) {
  ...
  const outputPath = path.join(root, `scene-${scene}-fresh-round-4-prompt.txt`);
...
checkpoint.status = "active-four-scene-gate-fresh-round-4-materialized";
...
checkpoint.nextWakeAction = {
  country: "Georgia",
  batch: 382,
  action: "launch-fresh-round-4-missing-scenes-only",
