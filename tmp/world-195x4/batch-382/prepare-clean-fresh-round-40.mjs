...
const templatePromptPath = path.join(root, "scene-1551-clean-fresh-round-39-prompt.txt");
const scene = 1551;
const round = 40;
...
if (sha256File(contractPath) !== "69EB6C84A2467A4234D901C3086ECECE9E583B55C41BB9382CD5ED523C482EF5") throw new Error("Contract changed before round 40");
if (sha256File(ledgerPath) !== "B94AA3FB2B1AB1BCAB100CE689F0173C77407B82439D73379D8BC0FA3EEC2455") throw new Error("Ledger changed before round 40");
if (!contract.romance?.radianceRealtimeAgreementPartyRule) throw new Error("Radiance agreement rule missing");
if (checkpoint.status !== "active-four-scene-gate-incomplete-after-clean-fresh-round-39") throw new Error(`Unexpected status: ${checkpoint.status}`);
if (checkpoint.renderStrategyReset?.nextCleanRound !== round) throw new Error("Checkpoint does not authorize round 40");
if (checkpoint.renderAttempts.freshRound40) throw new Error("Round 40 already materialized");
...
const invitationKey = "batch382-georgia-scene1551-radianceLiveInvitation-round40";
const responseKey = "batch382-georgia-scene1551-radianceLiveResponse-round40";
const participantsKey = "batch382-georgia-scene1551-radianceLiveParticipants-round40";
...
const invitations = [
  "AI ECE asks Radiance whether she wants to mark the safe route lesson with one restrained shoulder-turn shared with Ellie and ECE while Alia remains outside the party beat.",
  "Ellie asks Radiance whether she wants to keep the supported lean for one celebratory count with Ellie and ECE while Alia remains task-focused.",
  "AI ECE offers Radiance one optional fully clothed rain-step with Ellie and ECE before the quartet returns to planning.",
  "Alia offers Radiance the choice to answer the safe demonstration with one public-safe celebration count beside Ellie and ECE or return quietly to the route map.",
];
...
const radianceResponse = partyActivation
  ? "Radiance explicitly accepts the offered shoulder-turn with one clear up-down nod and broad willing smile directly toward ECE, then gives Ellie's existing low clasp one visible reciprocal squeeze while keeping her high palm on Ellie's covered shoulder."
...
const radiancePartyState = {
  round,
  rollMethod: "FNV-1a over recorded round-40 live-narrative keys, reduced modulo 100",
  ...
  visibleAgreementEvidence: partyActivation ? [
    ...
    "Ellie's willing smile while maintaining the stable support contacts",
    "ECE's willing smile and sustained mutual eye line toward Radiance while both compass hands stay fixed",
    "Alia's magnetic-confidence profile remains neutral and task-focused outside the party beat",
  ] : [],
  visibleResponseEvidence: partyActivation ? [] : [radianceResponse],
  continuityState: partyActivation
    ? "Radiance accepts exactly one fully clothed public-safe shoulder-turn with Ellie and ECE. Alia remains visibly outside this invitation-specific party beat and keeps both mission hands fixed downrange."
    : "No party activates. Every adult remains in the safe route-lesson pose.",
  consentScope: `This ${responseCategory} applies only to the recorded round-40 invitation in scene 1551 and only to ${willingParticipants.join(", ") || "no party participants"}. It does not transfer to another act, participant, prop interaction, scene, country, or future image.`,
};

let prompt = fs.readFileSync(templatePromptPath, "utf8");
prompt = prompt.replaceAll("round 39", "round 40").replaceAll("round-39", "round-40");

const compositionBlock = /SIMPLE LEFT-TO-RIGHT COMPOSITION[\s\S]*?(?=\n\nDETERMINISTIC CHARACTER ROLLS)/;
const newCompositionBlock = `FOUR CLEAR LANES, WIDE CAMERA
Use one clean 32 mm eye-level full-body exposure from far enough back that every body, foot, prop, panel, paper target, and all four edges of the sand backstop sit comfortably inside frame. Keep large plain-air gaps between all four lanes.
LANE 1, far left around 10 percent: ECE stands front-on behind one narrow waist-high compass pedestal. Both complete arms descend to two tall opposite handles. Her face turns right toward Radiance. Leave clear pavement between ECE, compass, and the relationship pair so no other hand can reach the compass.
LANE 2, center-left around 30 percent: Ellie stands front-three-quarter right-facing, both feet planted and both shoulders visible. Radiance stands beside her around 43 percent, rear-three-quarter to camera with her entire bare upper and middle back visible and both feet planted in a shallow fifteen-degree supported lean toward Ellie. Their torsos stay separated by a clear open-air gap. Their exact four relationship hands make three vertically separated contacts: HIGH, Radiance's bare left palm on Ellie's white-covered right shoulder; MIDDLE, Ellie's white-sleeved left palm high on Radiance's near left shoulder blade; LOW, Ellie's white-sleeved right arm and Radiance's bare right arm descend through the empty gap into one low right-hand clasp below both hips. Show every shoulder, upper arm, elbow, forearm, wrist, palm, and finger cluster continuously against plain sky, sea, or pavement. Radiance's head turns clearly left toward ECE, chin visibly mid-nod and both eyes aimed directly at ECE.
LANE 3, right around 61 percent: Alia stands in strict right-facing side profile, complete body and footwear in frame, with her large continuous bare upper and middle back visible. Her secure self-supporting front-only strapless copper shell ends before both rear ribs with no halter, neck loop, shoulder strap, back strap, rear band, closure, mesh, or fabric. Her exact two arms and hands hold one full-size thirty-centimeter dark polished-metal inert training replica in large clean side profile. Her straight right trigger index lies flat on the metal side plate above and entirely outside the complete empty black oval guard.
LANE 4, safety lane: put one complete transparent rectangular safety panel near 76 percent. Put one complete narrow freestanding sand backstop between 82 and 90 percent, leaving at least ten percent clear rainy margin to the right frame edge and visible margin above, below, and left. Fix one white paper route diamond at Alia's exact eye, barrel-center, muzzle-center, and shoulder-height horizontal row. Keep every person, mascot, public path, and occupied object behind and left of the muzzle plane.
Reserve lower center-left behind ECE's lane for the dry mascot lounge, and preserve Batumi's recognizable Alphabet Tower, Ferris wheel, Black Sea, palms, and skyline across the upper third. No cropped foot, cropped panel, cropped backstop, foreground overlap, layered arm, decorative clutter, curved architecture, hidden contact, or hand near the compass except ECE's two hands.`;
...
Pole-theme roll 67 = inactive; show no pole. Rainbow-only roll 15 = inactive; do not convert the group wardrobe to rainbow styling. Rainbow-hosiery roll 14 = ACTIVE; wearer selector roll 38 = Radiance; palette selector roll 54 = original independent rainbow gradient. Radiance and ECE remain the affectionate narrative center through a sustained warm mutual eye line. partyActivation is TRUE for exactly Radiance, Ellie, and AI ECE. Express their one-step celebration only through Radiance's visible nod and smile toward ECE, her reciprocal squeeze in Ellie's low clasp, Ellie's willing smile while both support contacts remain fixed, and ECE's reciprocal willing smile while both compass hands remain fixed. Alia stays visibly outside the party beat with magnetic-confidence focus downrange and no party smile, nod, dance motion, or participant cue. Add no heel lift and no party object. Alia alone handles the inert mission prop.`;
...
Compound-love roll 28 = ECE stays close against Radiance's side while warmly greeting Ellie and Alia answers beside them. Materialize its square through Radiance and ECE's sustained affectionate eye line, Ellie's willing support, and Alia's magnetic-confidence focus outside the party beat while every task hand remains fixed.
Hard-love roll 40 = a controlled dance dip with stable planted feet and supported back, a second partner catching the dipped adult's free hand, and the fourth answering with visible jealousy or invitation. Ellie supports Radiance through the middle support palm and low caught-hand clasp; Radiance answers with her high palm on Ellie's covered shoulder. These three and only three relationship contacts are large, ordinary, vertically separated, and unmistakable. Alia answers as the non-party fourth through a confident guarded profile while remaining safely downrange. ECE remains the affectionate center through mutual eye line while owning the compass.`;
...
partyActivation = TRUE. Show exactly one adult, consensual, fully clothed, public-safe InternalAgency shoulder-turn involving exactly Radiance, Ellie, and AI ECE. Radiance visibly nods and smiles directly toward ECE while squeezing Ellie's existing low clasp. Ellie gives one willing smile while maintaining the high and middle support contacts. ECE gives one reciprocal willing smile toward Radiance while both hands remain on opposite compass handles. Alia remains outside the party beat with a neutral confident mission expression, fixed feet, fixed mission hands, and rightward sight line. Add no party object, crowd, drink, confetti, balloon, sign, text, stage, ornament, or unsafe heel lift. Agreement is limited to this exact invitation, participant set, and one shoulder-turn.`;
...
ECE stands alone at far left. Her left arm descends visibly to the tall left compass handle and her right arm descends visibly to the tall right handle. Both arms and hands are separated by the compass face. ECE touches no person and owns the compass alone; no other human hand enters her lane.
...
prompt = prompt.replace(
  ...
  "The clean photograph must pass exact identity, eight-arm, eight-hand, continuous owner-path, three vertically separated contact romance, explicit Radiance affirmative with partyActivation true for exactly Radiance Ellie and ECE, Alia visibly outside the party beat, Radiance-ECE affectionate center, ECE-exclusive compass ownership, collarless mascots, wardrobe-roll, Alia strapless fully open back, full-size metallic inert training replica, exact horizontal target axis, complete safety panel, complete four-edge backstop with ten-percent right margin, indexed-trigger, location-theme fusion, and surface-quality gates.",
);
...
if (/round39|round-39|willing participants = Radiance, AI ECE, Alia/.test(prompt)) throw new Error("Stale round-39 party state remains");
...
const promptAudit = {
  ...
  referenceAudit,
  plannedPasses: { cleanFreshPasses: 1, maximumTargetedRecoveryPasses: 1, recoverySourceIfNeeded: "only the clean round 40 fresh raw", laterFreshSourcePolicy: "original identity anchors only" },
  radianceRealtimeAgreementParty: radiancePartyState,
  redesignedComposition: {
    lanes: ["ECE and compass far left", "Ellie and Radiance center-left", "Alia right", "panel and complete backstop far right"],
    ...
    partyGraph: "party active for Radiance Ellie and ECE; Alia remains a visibly non-party mission handler",
    missionGeometry: "Alia at 61 percent, complete panel near 76 percent, complete backstop between 82 and 90 percent with at least ten percent right margin",
  },
...
plan.freshRound40 = { ...promptAudit, prompt };
plan.radianceRealtimeAgreementParty = radiancePartyState;
const preparedAt = new Date().toISOString();
checkpoint.status = "active-four-scene-gate-clean-fresh-round-40-materialized";
...
checkpoint.xPost.note = "Georgia has accepted scenes 1548, 1549, and 1550. Clean round 40 is materialized for missing scene 1551 from original identity anchors only with an explicit Radiance affirmative limited to Radiance, Ellie, and ECE; Alia remains the non-party mission handler. The live X webview also requires retry. Publication remains mandatory after scene 1551 is accepted and the four-scene completion checkpoint is pushed.";
checkpoint.nextQueueStatus = "locked-on-Georgia-until-four-accepted-git-pushed-and-X-live-verified";
checkpoint.nextWakeAction = { country: "Georgia", batch: 382, action: "launch-clean-fresh-round-40-from-original-identity-anchors-scene-1551-only", preserveAcceptedSceneNumbers: [1548, 1549, 1550], sceneNumbers: [scene], laterCountryStartAllowed: false };
