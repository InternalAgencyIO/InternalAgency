...
import { vaticanCityPalette, vaticanCityProhibitions, vaticanCitySceneSpecs } from "./vatican-city-scene-specs.mjs";

const batch = 349;
const country = "Vatican City";
const countrySlug = "vatican-city";
const firstScene = 1416;
const root = path.resolve("tmp/world-195x4/batch-349");
...
const palette = vaticanCityPalette;
...
const commonProhibitions = vaticanCityProhibitions;
...
const sceneSpecs = vaticanCitySceneSpecs;
...
const weatherSafeText = (result, scene) => {
  ...
  if (result === "heavy snow or cinematic blizzard" && scene === 1416) return "the stored cinematic-blizzard roll appears as a dense curtain of harmless white sea mist, windblown pale frangipani petals, and cloud droplets beyond the waterfront shelter, with no snow accumulation and dry nonslip footing";
  if (result === "heavy snow or cinematic blizzard" && scene === 1418) return "the stored cinematic-blizzard roll appears as a dense line of natural white blowhole spray plumes beyond the covered overlook, with no snow accumulation and dry nonslip footing";
...
for (const spec of sceneSpecs) {
  ...
  if (hasMale) outfits.Male = "the established Scene 1136 adult male face with closely trimmed beard and athletic build, an opaque fitted short-sleeve travertine-white top with restrained Vatican museum-courtyard embroidery and subtle polar-airship seam tailoring, fitted black jeans, and practical black boots";
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Vatican City location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch349-vatican-city keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct. Batch 347 also stores one hard-love roll, mascot-state roll and holder selector, and odd-prop activation, holder, and family selectors per scene.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  nextQueueCountry: "Djibouti",
  nextQueueBatch: 350,
  nextQueueScenes: [1420, 1421, 1422, 1423],
  nextCinematicTheme: { active: "polar airship couture", batchOrdinalWithinTheme: 2 },
  researchSources: [
    { url: "https://www.museivaticani.va/content/museivaticani/en/patrimonio-naturale-archeologico/giardini/presentazione-giardini/giardini.html", usedFor: "Vatican Gardens and Italian Garden landscape" },
    { url: "https://www.museivaticani.va/content/museivaticani/en/collezioni/musei/museo-gregoriano-egizio/terrazza-del-nicchione/terrazza-del-nicchione.html", usedFor: "Courtyard of the Pinecone and Belvedere exedra architecture" },
    { url: "https://www.museivaticani.va/content/dam/museivaticani/pdf/visita_musei/servizi_visitatori/mappa_musei_vaticani.pdf", usedFor: "Vatican Museums visitor route and courtyard context" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Vatican City flag, official seal, sacred symbol, religious building, copied ceremonial or traditional pattern, political messaging, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular pinecone reliefs, exedra arcs, museum marble grids, botanical fans and helical architecture instead.",
    palette,
    minimumCoverage: "Scenes 1416 and 1417 each carry hard large Vatican City motifs on three women and polar-airship construction language on at least two. Scenes 1418 and 1419 use four different theme-led polar-airship outfits without country map prints while Vatican City landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground the Courtyard of the Pinecone, Vatican Gardens, Momo helical staircase, and Belvedere Courtyard.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Vatican City images plus one accepted Tuvalu image",
    captionIfEligible: `Vatican City ${heartGlyph} Tuvalu ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-349-vatican-city-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-349-vatican-city-preflight.json"),
