...
import { haitiPalette, haitiProhibitions, haitiSceneSpecs } from "./haiti-scene-specs.mjs";

const batch = 367;
const country = "Haiti";
const countrySlug = "haiti";
const firstScene = 1488;
const root = path.resolve("tmp/world-195x4/batch-367");
...
const commonProhibitions = haitiProhibitions;
...
const sceneSpecs = haitiSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Haiti location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Jordan",
  nextQueueBatch: 368,
  nextQueueScenes: [1492, 1493, 1494, 1495],
  nextCinematicTheme: { active: "deep-sea submersible couture", batchOrdinalWithinTheme: 2 },
  researchSources: [
    { url: "https://www.britannica.com/place/Port-au-Prince", usedFor: "Port-au-Prince bay and hillside city setting" },
    { url: "https://www.britannica.com/place/Haiti", usedFor: "Haiti mountains, bays, islands and cities" },
    { url: "https://whc.unesco.org/en/list/180/", usedFor: "Citadelle Laferrière architecture and mountain setting" },
    { url: "https://www.britannica.com/place/Jacmel", usedFor: "Jacmel coastal city and regional landscape" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Haiti flag, coat of arms, official seal, sacred symbol, copied ceremonial or traditional pattern, political messaging, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular bay currents, mountain terraces, fortress geometry, Jacmel rooflines, waterfall steps and island reef bands instead.",
    palette,
    minimumCoverage: "Scenes 1488 and 1489 each carry hard large Haiti motifs on three women and submersible construction language on at least two. Scenes 1490 and 1491 use four different theme-led deep-sea outfits without country map prints while Haiti landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Port-au-Prince Bay, Citadelle Laferrière, Jacmel with Bassin Bleu, and Île-à-Vache.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Haiti images plus one accepted Belgium image",
    captionIfEligible: `Haiti ${heartGlyph} Belgium ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-367-haiti-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-367-haiti-preflight.json"),
