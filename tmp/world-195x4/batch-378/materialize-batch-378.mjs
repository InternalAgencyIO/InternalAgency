...
import { jamaicaPalette, jamaicaProhibitions, jamaicaSceneSpecs } from "./jamaica-scene-specs.mjs";

const batch = 378;
const country = "Jamaica";
const countrySlug = "jamaica";
const firstScene = 1532;
const root = path.resolve("tmp/world-195x4/batch-378");
...
const commonProhibitions = jamaicaProhibitions;
...
const sceneSpecs = jamaicaSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Jamaica location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Gambia",
  nextQueueBatch: 379,
  nextQueueScenes: [1536, 1537, 1538, 1539],
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Jamaica flag, coat of arms, official seal, sacred symbol, copied traditional pattern, political messaging, copied military identity, police identity, emergency-service identity, or official service identity is used. Country-led scenes use large dimensional secular harbor curves, skyline terraces, mountain layers, shoreline bands, fort-wall geometry and harbor-mouth arcs instead.",
    palette,
    minimumCoverage: "Scenes 1532 and 1533 each carry hard large Jamaica motifs on three women and fictional rescue-vessel construction language on at least two. Scenes 1534 and 1535 use four different theme-led rescue-vessel outfits without country map prints while Jamaica landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Kingston Harbour, Port Royal, Dunn's River Falls, and Negril.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Jamaica images plus one accepted Qatar image",
    captionIfEligible: `Jamaica ${heartGlyph} Qatar ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-378-jamaica-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-378-jamaica-preflight.json"),
