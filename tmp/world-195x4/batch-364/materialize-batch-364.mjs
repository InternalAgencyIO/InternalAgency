...
import { tunisiaPalette, tunisiaProhibitions, tunisiaSceneSpecs } from "./tunisia-scene-specs.mjs";

const batch = 364;
const country = "Tunisia";
const countrySlug = "tunisia";
const firstScene = 1476;
const root = path.resolve("tmp/world-195x4/batch-364");
...
const commonProhibitions = tunisiaProhibitions;
...
const sceneSpecs = tunisiaSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Tunisia location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueBatch: 365,
  nextQueueScenes: [1480, 1481, 1482, 1483],
  nextCinematicTheme: { active: "near-Sun solar-observation couture", batchOrdinalWithinTheme: 1 },
  researchSources: [
    { url: "https://www.britannica.com/place/Tunis", usedFor: "Tunis city, lake and Mediterranean setting" },
    { url: "https://www.britannica.com/place/Tunisia", usedFor: "Tunisia Mediterranean, desert and salt-lake geography" },
    { url: "https://whc.unesco.org/en/list/38/", usedFor: "El Jem amphitheatre architecture" },
    { url: "https://www.britannica.com/place/Chott-el-Djerid", usedFor: "Chott el Jerid salt-lake landscape" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Tunisia flag, coat of arms, official seal, sacred symbol, copied ceremonial or traditional pattern, political messaging, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular Mediterranean currents, white-city terraces, blue door arches, amphitheatre arcades, salt-lake bands and oasis fans instead.",
    palette,
    minimumCoverage: "Scenes 1476 and 1477 each carry hard large Tunisia motifs on three women and Moon-expedition construction language on at least two. Scenes 1478 and 1479 use four different theme-led lunar expedition outfits without country map prints while Tunisia landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Tunis, Sidi Bou Said, El Jem, and Chott el Jerid with Tozeur.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Tunisia images plus one accepted Bolivia image",
    captionIfEligible: `Tunisia ${heartGlyph} Bolivia ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-364-tunisia-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-364-tunisia-preflight.json"),
