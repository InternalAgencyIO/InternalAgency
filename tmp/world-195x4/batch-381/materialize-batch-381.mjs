...
import { botswanaPalette, botswanaProhibitions, botswanaSceneSpecs } from "./botswana-scene-specs.mjs";

const batch = 381;
const country = "Botswana";
const countrySlug = "botswana";
const firstScene = 1544;
const root = path.resolve("tmp/world-195x4/batch-381");
...
const commonProhibitions = botswanaProhibitions;
...
const sceneSpecs = botswanaSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Botswana location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Georgia",
  nextQueueBatch: 382,
  nextQueueScenes: [1548, 1549, 1550, 1551],
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Botswana flag, coat of arms, official seal, sacred symbol, copied traditional pattern, political messaging, copied military identity, police identity, emergency-service identity, or official service identity is used. Country-led scenes use large dimensional secular delta channels, papyrus ridges, salt horizons, baobab crowns, granite ribs and hill layers instead.",
    palette,
    minimumCoverage: "Scenes 1544 and 1545 each carry hard large Botswana motifs on three women and Mars-expedition construction language on at least two. Scenes 1546 and 1547 use four different theme-led Mars-surface outfits without country map prints while Botswana landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Okavango Delta, Makgadikgadi Pans, Tsodilo Hills, and Gaborone with Kgale Hill and Gaborone Dam.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Botswana images plus one accepted Qatar image",
    captionIfEligible: `Botswana ${heartGlyph} Qatar ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-381-botswana-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-381-botswana-preflight.json"),
