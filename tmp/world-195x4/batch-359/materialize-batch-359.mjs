...
import { guineaPalette, guineaProhibitions, guineaSceneSpecs } from "./guinea-scene-specs.mjs";

const batch = 359;
const country = "Guinea";
const countrySlug = "guinea";
const firstScene = 1456;
const root = path.resolve("tmp/world-195x4/batch-359");
...
const commonProhibitions = guineaProhibitions;
...
const sceneSpecs = guineaSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Guinea location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Benin",
  nextQueueBatch: 360,
  nextQueueScenes: [1460, 1461, 1462, 1463],
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Guinea flag, coat of arms, official seal, sacred symbol, copied ceremonial or traditional pattern, political messaging, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular Atlantic currents, island contours, Fouta Djallon escarpments, waterfall ribbons, Nimba ridges and bauxite facets instead.",
    palette,
    minimumCoverage: "Scenes 1456 and 1457 each carry hard large Guinea motifs on three women and orbital-spaceship construction language on at least two. Scenes 1458 and 1459 use four different theme-led spaceship outfits without country map prints while Guinea landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Conakry and the Loos Islands, Fouta Djallon, Mount Nimba, and the Niger headwaters.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Guinea images plus one accepted Zimbabwe image",
    captionIfEligible: `Guinea ${heartGlyph} Zimbabwe ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-359-guinea-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-359-guinea-preflight.json"),
