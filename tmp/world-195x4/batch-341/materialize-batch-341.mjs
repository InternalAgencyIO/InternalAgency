...
import { stkittsNevisPalette, stkittsNevisProhibitions, stkittsNevisSceneSpecs } from "./saint-kitts-and-nevis-scene-specs.mjs";

const batch = 341;
const country = "Saint Kitts and Nevis";
const countrySlug = "saint-kitts-and-nevis";
const firstScene = 1384;
const root = path.resolve("tmp/world-195x4/batch-341");
...
const palette = stkittsNevisPalette;
...
const commonProhibitions = stkittsNevisProhibitions;
...
const legacySceneSpecs7 = [
  {
    scene: 1336,
    theme: "doctor-clinical-command couture",
    landmark: "a broad dry covered civic-waterfront overlook above SÃ£o TomÃ© city's Ana Chaves Bay, with the complete bay crescent, Pico de SÃ£o TomÃ© ridgeline, palm-lined shore, coral-stone quay geometry, empty marina reach, green hills, and layered Atlantic clouds",
    motifs: "complete SÃ£o TomÃ© island outline, Ana Chaves Bay crescent, Pico ridgeline, palm-shore rhythm, coral-stone quay grid, cocoa-pod contour, Atlantic current arc, and route-grid compositions",
    culture: "SÃ£o TomÃ© is presented through secular civic waterfront geography, Ana Chaves Bay, volcanic ridges, palm-lined shores, coral-stone quay geometry, cocoa ecology, and an empty public overlook. Doctor-clinical-command couture is public-safe fictional command fashion only, with no patient, clinic, examination, treatment, diagnosis, procedure, medical instrument, copied uniform, badge, red cross, caduceus, injury, emergency, or sexualized care.",
  ...
  },
  {
    scene: 1337,
    theme: "doctor-clinical-command couture",
    landmark: "a broad dry covered geological overlook facing Pico CÃ£o Grande in ObÃ´ Natural Park, with the complete volcanic needle, rainforest canopy, mist layers, fern bands, basalt ridges, and an empty protected observation path",
    motifs: "complete SÃ£o TomÃ© island outline, Pico CÃ£o Grande needle, rainforest canopy, fern spiral, basalt ridge, mist veil, cocoa-pod contour, and route-grid compositions",
    culture: "Pico CÃ£o Grande and ObÃ´ are presented through secular volcanic geology, rainforest conservation, fern bands, basalt ridges, cocoa ecology, mist layers, and an empty protected overlook. Doctor-clinical-command couture is public-safe fictional command fashion only, with no patient, clinic, examination, treatment, diagnosis, procedure, medical instrument, copied uniform, badge, red cross, caduceus, injury, emergency, or sexualized care.",
  ...
  },
  {
    scene: 1338,
    theme: "adult nightlife dance-performance couture",
    landmark: "a broad dry covered cocoa-estate overlook at RoÃ§a SÃ£o JoÃ£o dos Angolares, with a complete secular cocoa-drying courtyard, terraced tropical hills, cacao groves, an Atlantic glimpse, stone arcade geometry, empty service paths, and warm evening route lights",
    motifs: "complete SÃ£o TomÃ© island outline, cocoa-pod clusters, drying-courtyard grid, cacao-leaf rhythm, hill terraces, Atlantic band, stone-arcade geometry, and evening route-light compositions",
  ...
  },
  {
    scene: 1341,
    theme: "adult nightlife dance-performance couture",
    landmark: "a broad dry covered coastal overlook above Praia Banana on PrÃ­ncipe, with the complete golden beach crescent, twin rocky headlands, turquoise bay, rainforest canopy, reef shelves, empty shore, and a distant tropical rain veil",
    motifs: "complete PrÃ­ncipe island outline, Praia Banana crescent, headland silhouettes, turquoise-bay band, reef shelf, rainforest canopy, rain veil, and route-arc compositions",
    culture: "PrÃ­ncipe is presented through secular coastal conservation, Praia Banana's beach crescent, rocky headlands, turquoise bay, reef shelves, rainforest canopy, tropical weather, and an empty public overlook. Adult nightlife dance-performance couture is public-safe fictional evening fashion expressed through still editorial movement only, with no stripping, explicit dance, nightclub, alcohol, copied uniform, badge, threat, or combat.",
...
const sceneSpecs = stkittsNevisSceneSpecs;
...
const weatherSafeText = (result, scene) => {
  ...
  if (result === "heavy snow or cinematic blizzard" && scene === 1384) return "the stored cinematic-blizzard roll appears as a dense curtain of harmless white sea mist, windblown pale frangipani petals, and cloud droplets beyond the waterfront shelter, with no snow accumulation and dry nonslip footing";
  if (result === "heavy snow or cinematic blizzard" && scene === 1386) return "the stored cinematic-blizzard roll appears as a dense line of natural white blowhole spray plumes beyond the covered overlook, with no snow accumulation and dry nonslip footing";
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Saint Kitts and Nevis location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const hashtags = ["#SaintKittsAndNevis"];
...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch341-saint-kitts-and-nevis keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct. Batch 341 also stores one hard-love roll, mascot-state roll and holder selector, and odd-prop activation, holder, and family selectors per scene.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  nextQueueCountry: "Liechtenstein",
  nextQueueBatch: 342,
  nextQueueScenes: [1388, 1389, 1390, 1391],
  nextCinematicTheme: { active: "Mars-surface expedition couture", batchOrdinalWithinTheme: 2 },
  researchSources: [
    { url: "https://whc.unesco.org/en/list/910", usedFor: "Brimstone Hill Fortress basalt construction, twin volcanic peaks and Caribbean coastline" },
    { url: "https://whc.unesco.org/en/statesparties/kn/", usedFor: "Basseterre and Charlestown historic-zone context" },
    { url: "https://www.visitstkitts.com/", usedFor: "Basseterre waterfront and Timothy Hill island panorama" },
    { url: "https://www.visitnevisisland.com/", usedFor: "Pinney's Beach and Nevis Peak setting" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Saint Kitts and Nevis flag, coat of arms, official emblem, sacred symbol, religious building, ceremonial pattern, ceremonial attire, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular twin-island contours, Basseterre waterfront arcs, Brimstone basalt geometry, Timothy ridge lines, dual-ocean currents, Pinney beach ribbons and Nevis Peak fans instead.",
    palette,
    minimumCoverage: "Scenes 1384 and 1385 each carry hard large Saint Kitts and Nevis motifs on three women and Mars-expedition construction language on at least two. Scenes 1386 and 1387 use four different theme-led Mars-expedition outfits without country map prints while Saint Kitts and Nevis landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Basseterre waterfront, Brimstone Hill Fortress, Timothy Hill's Atlantic-Caribbean panorama, and Pinney's Beach beneath Nevis Peak.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Saint Kitts and Nevis images plus one accepted Andorra image",
    captionIfEligible: `Saint Kitts and Nevis ${heartGlyph} Andorra ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-341-saint-kitts-and-nevis-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-341-saint-kitts-and-nevis-preflight.json"),
