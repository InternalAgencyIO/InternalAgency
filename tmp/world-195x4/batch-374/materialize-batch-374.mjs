...
import { armeniaPalette, armeniaProhibitions, armeniaSceneSpecs } from "./armenia-scene-specs.mjs";

const batch = 374;
const country = "Armenia";
const countrySlug = "armenia";
const firstScene = 1516;
const root = path.resolve("tmp/world-195x4/batch-374");
...
const commonProhibitions = armeniaProhibitions;
...
const sceneSpecs = armeniaSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Armenia location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Namibia",
  nextQueueBatch: 375,
  nextQueueScenes: [1520, 1521, 1522, 1523],
  nextCinematicTheme: { active: "civilian helicopter flight couture", batchOrdinalWithinTheme: 1 },
  researchSources: [
    { url: "https://www.britannica.com/place/Armenia", usedFor: "Armenia geography, cities and regions" },
    { url: "https://www.britannica.com/place/Yerevan", usedFor: "Yerevan civic architecture and mountain setting" },
    { url: "https://www.britannica.com/place/Lake-Sevan", usedFor: "Lake Sevan basin and mountain setting" },
    { url: "https://www.britannica.com/place/Dilijan", usedFor: "Dilijan forest and town setting" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Armenia flag, coat of arms, official seal, sacred symbol, copied carpet pattern, copied ceremonial or traditional pattern, political messaging, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular Cascade steps, rose-tuff terraces, civic arcs, mountain silhouettes, lake curves, basalt columns and forest rooflines instead.",
    palette,
    minimumCoverage: "Scenes 1516 and 1517 each carry hard large Armenia motifs on three women and private-jet construction language on at least two. Scenes 1518 and 1519 use four different theme-led private-jet outfits without country map prints while Armenia landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Yerevan, Lake Sevan, Garni Gorge, and Dilijan.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Armenia images plus one accepted Honduras image",
    captionIfEligible: `Armenia ${heartGlyph} Honduras ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-374-armenia-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-374-armenia-preflight.json"),
