...
import { hondurasPalette, hondurasProhibitions, hondurasSceneSpecs } from "./honduras-scene-specs.mjs";

const batch = 373;
const country = "Honduras";
const countrySlug = "honduras";
const firstScene = 1512;
const root = path.resolve("tmp/world-195x4/batch-373");
...
const commonProhibitions = hondurasProhibitions;
...
const sceneSpecs = hondurasSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Honduras location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Armenia",
  nextQueueBatch: 374,
  nextQueueScenes: [1516, 1517, 1518, 1519],
  nextCinematicTheme: { active: "private-jet aviation couture", batchOrdinalWithinTheme: 2 },
  researchSources: [
    { url: "https://whc.unesco.org/en/list/129/", usedFor: "Copan archaeological terraces and valley setting" },
    { url: "https://www.britannica.com/place/Honduras", usedFor: "Honduras geography, cities and regions" },
    { url: "https://www.britannica.com/place/Tegucigalpa", usedFor: "Tegucigalpa mountain-basin city setting" },
    { url: "https://www.britannica.com/place/Roatan", usedFor: "Roatan island and Caribbean reef setting" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Honduras flag, coat of arms, official seal, sacred symbol, copied Indigenous glyph, copied ceremonial or traditional pattern, political messaging, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular mountain basins, river bands, civic rooflines, stone terraces, Caribbean reef arcs, lake curves and cloud-forest silhouettes instead.",
    palette,
    minimumCoverage: "Scenes 1512 and 1513 each carry hard large Honduras motifs on three women and private-jet construction language on at least two. Scenes 1514 and 1515 use four different theme-led private-jet outfits without country map prints while Honduras landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Tegucigalpa, Copan Ruinas, Roatan, and Lake Yojoa.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Honduras images plus one accepted Czechia image",
    captionIfEligible: `Honduras ${heartGlyph} Czechia ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-373-honduras-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-373-honduras-preflight.json"),
