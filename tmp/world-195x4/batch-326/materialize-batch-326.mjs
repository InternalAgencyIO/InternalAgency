...
const countrySlug = "iceland";
const firstScene = 1324;
const root = path.resolve("tmp/world-195x4/batch-326");
...
const palette = "glacier blue, basalt black, geothermal turquoise, moss green, lava red, aurora violet, snow white, silver mist, midnight navy, and warm geothermal gold";
...
const commonProhibitions = "Use respectful secular Icelandic glacier tongues, volcanic ridges, rift lines, basalt columns, lava fields, geothermal steam, waterfall arcs, black-sand bands, iceberg facets, aurora ribbons, civic-glass geometry, and public overlooks only. No literal flag, coat of arms, official seal, sacred symbol, religious building, copied ceremonial pattern, copied uniform, badge, readable text, brand, alcohol, or political insignia.";
...
const garment = (character, plan, spec, index) => {
  const colors = ["glacier blue", "aurora violet", "moss emerald", "basalt navy"];
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Place large complete secular Iceland motifs across at least two outfits as dominant full bodice, skirt, hip, and panel fields: ${spec.motifs}. ${commonProhibitions}`,
    ...
    rainbowOnly.active ? `Rainbow-only is active across the outfits while preserving complete Iceland motifs.` : "Rainbow-only is inactive; do not convert the wardrobe to rainbow-only styling.",
...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch326-iceland keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  nextQueueCountry: "Vanuatu",
  nextQueueBatch: 327,
  nextQueueScenes: [1328, 1329, 1330, 1331],
  researchSources: [
    { url: "https://visitreykjavik.is/service/harpa-concert-hall-and-conference-centre", usedFor: "Harpa's secular colored-glass waterfront geometry and Reykjavik harbor setting" },
    { url: "https://www.thingvellir.is/en/", usedFor: "Thingvellir rift valley, Almannagja fault walls, lake, lava fields, and public paths" },
    { url: "https://www.vatnajokulsthjodgardur.is/en/areas/jokulsarlon", usedFor: "Jokulsarlon glacier lagoon, Breidamerkurjokull tongue, icebergs, black sand, and open water" },
    { url: "https://www.south.is/en/place/reynisfjara-black-sand-beach", usedFor: "Reynisfjara black-sand coast, basalt columns, Reynisdrangar sea stacks, and Atlantic surf" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Iceland flag, coat of arms, official emblem, sacred symbol, or religious building is copied. Large researched secular glacier, rift, basalt, lava, iceberg, civic-glass, black-sand, aurora, waterfall, and route compositions replace them.",
    palette,
    minimumCoverage: "Every image carries large complete secular Iceland compositions across at least two outfits as dominant full bodice, skirt, hip, or panel fields.",
    cultureScene: "The scenes foreground Harpa and Reykjavik harbor, Thingvellir rift geology, Jokulsarlon glacier lagoon, and Reynisfjara's basalt and black-sand coast.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Iceland images plus one accepted Bahamas image",
    captionIfEligible: `Iceland ${heartGlyph} Bahamas ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-326-iceland-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-326-iceland-preflight.json"),
