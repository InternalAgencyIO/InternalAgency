...
import { liechtensteinPalette, liechtensteinProhibitions, liechtensteinSceneSpecs } from "./liechtenstein-scene-specs.mjs";

const batch = 342;
const country = "Liechtenstein";
const countrySlug = "liechtenstein";
const firstScene = 1388;
const root = path.resolve("tmp/world-195x4/batch-342");
...
const palette = liechtensteinPalette;
...
const commonProhibitions = liechtensteinProhibitions;
...
const sceneSpecs = liechtensteinSceneSpecs;
...
const weatherSafeText = (result, scene) => {
  ...
  if (result === "heavy snow or cinematic blizzard" && scene === 1388) return "the stored cinematic-blizzard roll appears as a dense curtain of harmless white sea mist, windblown pale frangipani petals, and cloud droplets beyond the waterfront shelter, with no snow accumulation and dry nonslip footing";
  if (result === "heavy snow or cinematic blizzard" && scene === 1390) return "the stored cinematic-blizzard roll appears as a dense line of natural white blowhole spray plumes beyond the covered overlook, with no snow accumulation and dry nonslip footing";
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Liechtenstein location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch342-liechtenstein keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct. Batch 342 also stores one hard-love roll, mascot-state roll and holder selector, and odd-prop activation, holder, and family selectors per scene.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  nextQueueCountry: "Monaco",
  nextQueueBatch: 343,
  nextQueueScenes: [1392, 1393, 1394, 1395],
  nextCinematicTheme: { active: "Moon-surface expedition couture", batchOrdinalWithinTheme: 1 },
  researchSources: [
    { url: "https://tourismus.li/map/poi/schloss-vaduz-8fb7e6cf-99b5-4ffb-b600-9e4c66398e1f.html", usedFor: "Vaduz Castle above the capital and Rhine Valley" },
    { url: "https://en.tourismus.li/map/poi/burg-gutenberg-2721f037-4b6b-415f-9f32-725ed4b03321.html", usedFor: "Gutenberg Castle on its hill at Balzers" },
    { url: "https://tourismus.li/entdecken/sommer-herbst/uebersicht/malbun.html", usedFor: "Malbun alpine village and Sareis panorama" },
    { url: "https://tourismus.li/map/poi/alte-rheinbrucke-e5935511-7115-400a-942f-e7a4911f3b58.html", usedFor: "complete covered Old Rhine Bridge and Rhine setting" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Liechtenstein flag, coat of arms, crown, official emblem, sacred symbol, religious building, ceremonial pattern, ceremonial attire, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular Alpine profiles, Rhine ribbons, castle blocks, hill contours and covered timber trusses instead.",
    palette,
    minimumCoverage: "Scenes 1388 and 1389 each carry hard large Liechtenstein motifs on three women and Mars-expedition construction language on at least two. Scenes 1390 and 1391 use four different theme-led Mars-expedition outfits without country map prints while Liechtenstein landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Vaduz Castle and Rhine Valley, Gutenberg Castle at Balzers, Malbun and Sareis, and the complete covered Old Rhine Bridge.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Liechtenstein images plus one accepted Saint Kitts and Nevis image",
    captionIfEligible: `Liechtenstein ${heartGlyph} Saint Kitts and Nevis ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-342-liechtenstein-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-342-liechtenstein-preflight.json"),
