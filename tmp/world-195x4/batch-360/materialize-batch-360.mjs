...
import { beninPalette, beninProhibitions, beninSceneSpecs } from "./benin-scene-specs.mjs";

const batch = 360;
const country = "Benin";
const countrySlug = "benin";
const firstScene = 1460;
const root = path.resolve("tmp/world-195x4/batch-360");
...
const commonProhibitions = beninProhibitions;
...
const sceneSpecs = beninSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Benin location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Rwanda",
  nextQueueBatch: 361,
  nextQueueScenes: [1464, 1465, 1466, 1467],
  nextCinematicTheme: { active: "Mars-surface expedition couture", batchOrdinalWithinTheme: 1 },
  researchSources: [
    { url: "https://www.britannica.com/place/Cotonou", usedFor: "Cotonou lagoon and Atlantic urban setting" },
    { url: "https://www.britannica.com/place/Benin", usedFor: "Benin geography, Atacora highlands and coastal plains" },
    { url: "https://whc.unesco.org/en/list/749/", usedFor: "W-Arly-Pendjari regional landscape context near Atacora" },
    { url: "https://www.britannica.com/place/Mono-River", usedFor: "Mono River estuary and Grand-Popo coastal setting" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Benin flag, coat of arms, official seal, sacred symbol, copied ceremonial or traditional pattern, political messaging, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular lagoon currents, stilt-walkway frames, red-earth terraces, palm fans, waterfall ribbons and cotton-boll arcs instead.",
    palette,
    minimumCoverage: "Scenes 1460 and 1461 each carry hard large Benin motifs on three women and orbital-spaceship construction language on at least two. Scenes 1462 and 1463 use four different theme-led spaceship outfits without country map prints while Benin landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Cotonou lagoon, Ganvie and Lake Nokoue, Tanougou Falls, and the Grand-Popo Mono estuary.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Benin images plus one accepted Guinea image",
    captionIfEligible: `Benin ${heartGlyph} Guinea ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-360-benin-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-360-benin-preflight.json"),
