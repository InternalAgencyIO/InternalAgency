...
const countrySlug = "bahamas";
const firstScene = 1320;
const root = path.resolve("tmp/world-195x4/batch-325");
...
const palette = "Atlantic cobalt, Bight turquoise, aquamarine, mangrove emerald, coral magenta, limestone ivory, pink-sand blush, cloud white, sunset gold, and deep-water navy";
...
const commonProhibitions = "Use respectful secular Bahamian island chains, cay outlines, Atlantic and Bight water bands, blue-hole circles, mangrove channels, reef arcs, limestone ridges, bridge geometry, beach ripples, boardwalk rhythms, cloud bands, and public overlooks only. No literal flag, coat of arms, official seal, sacred symbol, religious building, copied ceremonial pattern, copied uniform, badge, readable text, brand, alcohol, or political insignia.";
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Place large complete secular Bahamas motifs across at least two outfits as dominant full bodice, skirt, hip, and panel fields: ${spec.motifs}. ${commonProhibitions}`,
    ...
    rainbowOnly.active ? `Rainbow-only is active across the outfits while preserving complete Bahamas motifs.` : "Rainbow-only is inactive; do not convert the wardrobe to rainbow-only styling.",
...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch325-bahamas keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  nextQueueCountry: "Iceland",
  nextQueueBatch: 326,
  nextQueueScenes: [1324, 1325, 1326, 1327],
  researchSources: [
    { url: "https://www.bahamas.com/natural-wonders/glass-window-bridge", usedFor: "Glass Window Bridge, narrow Eleuthera limestone divide, Atlantic cobalt, and calm Bight turquoise" },
    { url: "https://www.bahamas.com/natural-wonders/exuma-cays-land-and-sea-park", usedFor: "protected Exuma Cays, aquamarine channels, deserted beaches, coral reefs, and no-take conservation" },
    { url: "https://www.bahamas.com/islands/andros", usedFor: "Andros blue holes, mangrove creeks, tidal flats, pine forest, barrier reef, and Tongue of the Ocean" },
    { url: "https://www.bahamas.com/natural-wonders/gold-rock-beach", usedFor: "Gold Rock Beach, Lucayan National Park, low-tide sand ripples, broad shoreline, and protected coast" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Bahamas flag, coat of arms, official emblem, sacred symbol, or religious building is copied. Large researched secular island, cay, reef, blue-hole, mangrove, bridge, beach, boardwalk, cloud, and route compositions replace them.",
    palette,
    minimumCoverage: "Every image carries large complete secular Bahamas compositions across at least two outfits as dominant full bodice, skirt, hip, or panel fields.",
    cultureScene: "The scenes foreground Eleuthera's Glass Window Bridge, the Exuma Cays Land and Sea Park, Andros blue-hole landscapes, and Gold Rock Beach in Lucayan National Park.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Bahamas images plus one accepted Belize image",
    captionIfEligible: `Bahamas ${heartGlyph} Belize ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-325-bahamas-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-325-bahamas-preflight.json"),
