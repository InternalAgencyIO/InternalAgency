...
import { cubaPalette, cubaProhibitions, cubaSceneSpecs } from "./cuba-scene-specs.mjs";

const batch = 371;
const country = "Cuba";
const countrySlug = "cuba";
const firstScene = 1504;
const root = path.resolve("tmp/world-195x4/batch-371");
...
const commonProhibitions = cubaProhibitions;
...
const sceneSpecs = cubaSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Cuba location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Czechia",
  nextQueueBatch: 372,
  nextQueueScenes: [1508, 1509, 1510, 1511],
  nextCinematicTheme: { active: "orbital research-station couture", batchOrdinalWithinTheme: 2 },
  researchSources: [
    { url: "https://whc.unesco.org/en/list/204/", usedFor: "Old Havana architecture and harbor setting" },
    { url: "https://www.britannica.com/place/Cuba", usedFor: "Cuba geography, cities and coasts" },
    { url: "https://whc.unesco.org/en/list/460/", usedFor: "Trinidad urban architecture and valley setting" },
    { url: "https://whc.unesco.org/en/list/840/", usedFor: "Viñales Valley mogotes and agricultural landscape" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Cuba flag, coat of arms, official seal, sacred symbol, copied ceremonial or traditional pattern, political messaging, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular Malecón curves, Havana rooflines, Trinidad cobbles, colonial arcades, Viñales mogotes, bay arcs and mountain silhouettes instead.",
    palette,
    minimumCoverage: "Scenes 1504 and 1505 each carry hard large Cuba motifs on three women and orbital-station construction language on at least two. Scenes 1506 and 1507 use four different theme-led orbital-research outfits without country map prints while Cuba landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Havana, Trinidad, Viñales Valley, and Santiago de Cuba Bay.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Cuba images plus one accepted United Arab Emirates image",
    captionIfEligible: `Cuba ${heartGlyph} United Arab Emirates ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-371-cuba-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-371-cuba-preflight.json"),
