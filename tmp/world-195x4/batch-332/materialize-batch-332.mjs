...
const batch = 332;
const country = "Kiribati";
const countrySlug = "kiribati";
const firstScene = 1348;
const root = path.resolve("tmp/world-195x4/batch-332");
...
const palette = "lagoon turquoise, reef aqua, atoll white, pandanus green, sun gold, coral red, deep Pacific blue, storm gray, coconut brown, and sunset magenta";
...
const commonProhibitions = "Use respectful secular Kiribati island-chain maps, complete atoll rings, lagoon ribbons, reef shelves, causeway lines, pandanus bands, coconut-grove rhythms, salt-flat contours, cloud bands, current arcs, and public conservation overlooks only. No literal flag, coat of arms, official seal, sacred symbol, religious building, copied ceremonial pattern, ceremonial attire, copied uniform, badge, readable text, brand, alcohol, military imagery, or political insignia.";
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Place large complete secular Kiribati motifs across at least two outfits as dominant full bodice, skirt, hip, and panel fields: ${spec.motifs}. ${commonProhibitions}`,
    ...
    rainbowOnly.active ? `Rainbow-only is active across the outfits while preserving complete Kiribati motifs.` : "Rainbow-only is inactive; do not convert the wardrobe to rainbow-only styling.",
...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch332-kiribati keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  nextQueueCountry: "Seychelles",
  nextQueueBatch: 333,
  nextQueueScenes: [1352, 1353, 1354, 1355],
  researchSources: [
    { url: "https://www.kiribatitourism.gov.ki/kiribati-islands/gilbert-islands/tarawa/", usedFor: "South Tarawa atoll ribbon, lagoon, reef flats, vegetation, and causeway setting" },
    { url: "https://www.kiribatitourism.gov.ki/kiribati-islands/gilbert-islands/abaiang/", usedFor: "Abaiang lagoon arc, reef shelves, sand islets, coconut groves, and canoe channels" },
    { url: "https://www.kiribatitourism.gov.ki/kiribati-islands/line-islands/kiritimati/", usedFor: "Kiritimati lagoon, salt flats, reef shelves, scrub, and causeway routes" },
    { url: "https://www.phoenixislands.org/", usedFor: "Kanton Island and Phoenix Islands Protected Area atoll, lagoon pass, reefs, and conservation context" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Kiribati flag, coat of arms, official emblem, sacred symbol, religious building, ceremonial pattern, or ceremonial attire is copied. Large researched secular island-chain, atoll-ring, lagoon, reef-shelf, causeway, pandanus, coconut-grove, salt-flat, cloud, and route compositions replace them.",
    palette,
    minimumCoverage: "Every image carries large complete secular Kiribati compositions across at least two outfits as dominant full bodice, skirt, hip, or panel fields.",
    cultureScene: "The scenes foreground South Tarawa, Abaiang Lagoon, Kiritimati Lagoon, and Kanton Island in the Phoenix Islands Protected Area.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Kiribati images plus one accepted Saint Lucia image",
    captionIfEligible: `Kiribati ${heartGlyph} Saint Lucia ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-332-kiribati-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-332-kiribati-preflight.json"),
