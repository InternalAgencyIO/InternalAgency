...
const countrySlug = "barbados";
const firstScene = 1332;
const root = path.resolve("tmp/world-195x4/batch-328");
...
const palette = "ultramarine blue, aquamarine, coral pink, golden sand, mahogany, sugarcane green, cloud white, limestone cream, sunset orange, and deep Atlantic navy";
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Place large complete secular Barbados motifs across at least two outfits as dominant full bodice, skirt, hip, and panel fields: ${spec.motifs}. ${commonProhibitions}`,
    ...
    rainbowOnly.active ? `Rainbow-only is active across the outfits while preserving complete Barbados motifs.` : "Rainbow-only is inactive; do not convert the wardrobe to rainbow-only styling.",
...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch328-barbados keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  nextQueueCountry: "São Tomé and Príncipe",
  nextQueueBatch: 329,
  nextQueueScenes: [1336, 1337, 1338, 1339],
  researchSources: [
    { url: "https://whc.unesco.org/en/list/1376/", usedFor: "Historic Bridgetown's secular Careenage waterfront, bridge, warehouses, and coral-stone urban geometry" },
    { url: "https://www.visitbarbados.org/bathsheba", usedFor: "Bathsheba, Soup Bowl surf, coral-limestone boulders, reef shelves, and east-coast hills" },
    { url: "https://www.visitbarbados.org/harrisons-cave-eco-adventure-park", usedFor: "Harrison's Cave chambers, stalactites, stalagmites, flowstone, pools, and visitor path" },
    { url: "https://www.visitbarbados.org/animal-flower-cave", usedFor: "Animal Flower Cave, North Point cliffs, natural rock pool, Atlantic shelves, and spray" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Barbados flag, coat of arms, official emblem, sacred symbol, or religious building is copied. Large researched secular island, Careenage, bridge, boulder, surf, coral-limestone, cave, cliff, pool, sugarcane, mahogany, cloud, and route compositions replace them.",
    palette,
    minimumCoverage: "Every image carries large complete secular Barbados compositions across at least two outfits as dominant full bodice, skirt, hip, or panel fields.",
    cultureScene: "The scenes foreground Bridgetown's Careenage, Bathsheba's Atlantic geology, Harrison's Cave, and Animal Flower Cave at North Point.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Barbados images plus one accepted Vanuatu image",
    captionIfEligible: `Barbados ${heartGlyph} Vanuatu ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-328-barbados-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-328-barbados-preflight.json"),
