...
import { sanMarinoPalette, sanMarinoProhibitions, sanMarinoSceneSpecs } from "./san-marino-scene-specs.mjs";

const batch = 345;
const country = "San Marino";
const countrySlug = "san-marino";
const firstScene = 1400;
const root = path.resolve("tmp/world-195x4/batch-345");
...
const palette = sanMarinoPalette;
...
const commonProhibitions = sanMarinoProhibitions;
...
const legacyMicronesiaDraftSceneSpecs = [
  {
    ...
    theme: "near-Sun solar-observation couture",
...
const sceneSpecs = sanMarinoSceneSpecs;
...
const weatherSafeText = (result, scene) => {
  ...
  if (result === "heavy snow or cinematic blizzard" && scene === 1400) return "the stored cinematic-blizzard roll appears as a dense curtain of harmless white sea mist, windblown pale frangipani petals, and cloud droplets beyond the waterfront shelter, with no snow accumulation and dry nonslip footing";
  if (result === "heavy snow or cinematic blizzard" && scene === 1402) return "the stored cinematic-blizzard roll appears as a dense line of natural white blowhole spray plumes beyond the covered overlook, with no snow accumulation and dry nonslip footing";
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and San Marino location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch345-san-marino keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct. Batch 345 also stores one hard-love roll, mascot-state roll and holder selector, and odd-prop activation, holder, and family selectors per scene.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  nextQueueCountry: "Palau",
  nextQueueBatch: 346,
  nextQueueScenes: [1404, 1405, 1406, 1407],
  nextCinematicTheme: { active: "near-Sun solar-observation couture", batchOrdinalWithinTheme: 2 },
  researchSources: [
    { url: "https://www.visitsanmarino.com/pub2/VisitSM/en/", usedFor: "Guaita, Cesta, Montale, Palazzo Pubblico and cableway landmark list" },
    { url: "https://www.visitsanmarino.com/pub1/VisitSM/en/contenuto/Vivi/ARTE-E-CULTURA.html", usedFor: "Mount Titano historic-centre elevation, walls and three-tower skyline" },
    { url: "https://www.visitsanmarino.com/pub2/VisitSM/en/pianifica.html", usedFor: "Old Town and Mount Titano visitor context" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal San Marino flag, coat of arms, official seal, sacred symbol, religious building, copied ceremonial pattern, political messaging, copied guard identity, military identity, or official service identity is used. Country-led scenes use large dimensional secular Mount Titano profiles, Guaita and Cesta tower geometry, medieval wall arcs, civic facade rhythms and cableway lines instead.",
    palette,
    minimumCoverage: "Scenes 1400 and 1401 each carry hard large San Marino motifs on three women and near-Sun solar-observation construction language on at least two. Scenes 1402 and 1403 use four different theme-led near-Sun solar-observation outfits without country map prints while San Marino landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Guaita First Tower, Cesta Second Tower, Palazzo Pubblico and Piazza della Liberta, and the Borgo Maggiore cableway panorama toward Mount Titano.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two San Marino images plus one accepted Marshall Islands image",
    captionIfEligible: `San Marino ${heartGlyph} Marshall Islands ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-345-san-marino-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-345-san-marino-preflight.json"),
