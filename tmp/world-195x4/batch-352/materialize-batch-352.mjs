...
import { somaliaPalette, somaliaProhibitions, somaliaSceneSpecs } from "./somalia-scene-specs.mjs";

const batch = 352;
const country = "Somalia";
const countrySlug = "somalia";
const firstScene = 1428;
...
const commonProhibitions = somaliaProhibitions;
...
const sceneSpecs = somaliaSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Somalia location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Senegal",
  nextQueueBatch: 353,
  nextQueueScenes: [1432, 1433, 1434, 1435],
  ...
  countryMotifPolicy: {
    ...
    cultureScene: "The scenes foreground Mogadishu's Secondo-Lido Lighthouse, the Hobyo grassland and dunes, Bushbushle National Park, and the Jubba estuary near Kismayo.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Somalia images plus one accepted South Sudan image",
    captionIfEligible: `Somalia ${heartGlyph} South Sudan ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-352-somalia-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-352-somalia-preflight.json"),
