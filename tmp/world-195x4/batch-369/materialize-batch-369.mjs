...
import { dominicanRepublicPalette, dominicanRepublicProhibitions, dominicanRepublicSceneSpecs } from "./dominican-republic-scene-specs.mjs";

const batch = 369;
const country = "Dominican Republic";
const countrySlug = "dominican-republic";
const firstScene = 1496;
const root = path.resolve("tmp/world-195x4/batch-369");
...
const palette = dominicanRepublicPalette;
...
const commonProhibitions = dominicanRepublicProhibitions;
...
const sceneSpecs = dominicanRepublicSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Dominican Republic location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const hashtags = ["#DominicanRepublic"];
...
const preflight = {
  ...
  nextQueueCountry: "United Arab Emirates",
  nextQueueBatch: 370,
  nextQueueScenes: [1500, 1501, 1502, 1503],
  nextCinematicTheme: { active: "polar airship couture", batchOrdinalWithinTheme: 2 },
  researchSources: [
    { url: "https://whc.unesco.org/en/list/526/", usedFor: "Santo Domingo Colonial City architecture and Ozama setting" },
    { url: "https://www.britannica.com/place/Dominican-Republic", usedFor: "Dominican Republic geography, cities and coasts" },
    { url: "https://www.britannica.com/place/Puerto-Plata-Dominican-Republic", usedFor: "Puerto Plata mountain and Atlantic setting" },
    { url: "https://www.britannica.com/place/Samana-Bay", usedFor: "Samana Bay and peninsula setting" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Dominican Republic flag, coat of arms, official seal, sacred symbol, copied ceremonial or traditional pattern, political messaging, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular colonial arcades, Ozama river bands, mountain terraces, amber facets, bay curves, palm fans and beach crescents instead.",
    palette,
    minimumCoverage: "Scenes 1496 and 1497 each carry hard large Dominican Republic motifs on three women and airship construction language on at least two. Scenes 1498 and 1499 use four different theme-led polar-airship outfits without country map prints while Dominican Republic landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Santo Domingo Colonial City, Puerto Plata, Samaná Bay, and Punta Cana.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Dominican Republic images plus one accepted Jordan image",
    captionIfEligible: `Dominican Republic ${heartGlyph} Jordan ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-369-dominican-republic-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-369-dominican-republic-preflight.json"),
