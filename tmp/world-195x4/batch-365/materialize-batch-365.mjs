...
import { southSudanPalette, southSudanProhibitions, southSudanSceneSpecs } from "./south-sudan-scene-specs.mjs";

const batch = 365;
const country = "South Sudan";
const countrySlug = "south-sudan";
const firstScene = 1480;
const root = path.resolve("tmp/world-195x4/batch-365");
...
const palette = southSudanPalette;
...
const commonProhibitions = southSudanProhibitions;
...
const sceneSpecs = southSudanSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and South Sudan location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Belgium",
  nextQueueBatch: 366,
  nextQueueScenes: [1484, 1485, 1486, 1487],
  nextCinematicTheme: { active: "near-Sun solar-observation couture", batchOrdinalWithinTheme: 2 },
  researchSources: [
    { url: "https://www.britannica.com/place/Juba", usedFor: "Juba, White Nile and city setting" },
    { url: "https://www.britannica.com/place/South-Sudan", usedFor: "South Sudan rivers, wetlands, savanna and mountains" },
    { url: "https://www.britannica.com/place/Al-Sudd", usedFor: "Sudd wetland channels and reed landscape" },
    { url: "https://www.britannica.com/place/Imatong-Mountains", usedFor: "Imatong mountain and highland landscape" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal South Sudan flag, coat of arms, official seal, sacred symbol, copied ceremonial or traditional pattern, political messaging, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular Nile currents, Juba skyline arcs, Sudd reed channels, Nimule rapids, savanna bands and Imatong mountain profiles instead.",
    palette,
    minimumCoverage: "Scenes 1480 and 1481 each carry hard large South Sudan motifs on three women and solar-observation construction language on at least two. Scenes 1482 and 1483 use four different theme-led solar-observation outfits without country map prints while South Sudan landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Juba and the White Nile, the Sudd wetlands, Nimule and Fula Rapids, and the Imatong Mountains.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two South Sudan images plus one accepted Tunisia image",
    captionIfEligible: `South Sudan ${heartGlyph} Tunisia ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-365-south-sudan-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-365-south-sudan-preflight.json"),
