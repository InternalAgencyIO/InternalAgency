...
import { antiguaPalette, antiguaProhibitions, antiguaSceneSpecs } from "./antigua-scene-specs.mjs";

const batch = 338;
const country = "Antigua and Barbuda";
const countrySlug = "antigua-and-barbuda";
const firstScene = 1372;
...
const commonProhibitions = antiguaProhibitions;
...
const sceneSpecs = antiguaSceneSpecs;
...
const weatherSafeText = (result, scene) => {
  ...
  if (result === "heavy snow or cinematic blizzard" && scene === 1372) return "the stored cinematic-blizzard roll appears as a dense curtain of harmless white sea mist, windblown pale frangipani petals, and cloud droplets beyond the waterfront shelter, with no snow accumulation and dry nonslip footing";
  if (result === "heavy snow or cinematic blizzard" && scene === 1374) return "the stored cinematic-blizzard roll appears as a dense line of natural white blowhole spray plumes beyond the covered overlook, with no snow accumulation and dry nonslip footing";
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Antigua and Barbuda location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const hashtags = ["#AntiguaAndBarbuda"];
...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch338-antigua-and-barbuda keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct. Batch 338 also stores one hard-love roll, mascot-state roll and holder selector, and odd-prop activation, holder, and family selectors per scene.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  nextQueueCountry: "Andorra",
  nextQueueBatch: 339,
  nextQueueScenes: [1376, 1377, 1378, 1379],
  nextCinematicTheme: { active: "orbital spaceship couture", batchOrdinalWithinTheme: 1 },
  researchSources: [
    { url: "https://whc.unesco.org/en/list/1499/", usedFor: "Antigua Naval Dockyard, English Harbour, Georgian buildings, deep narrow bays and surrounding highlands" },
    { url: "https://www.visitantiguabarbuda.com/things-to-do/frigate-bird-sanctuary-3/", usedFor: "Codrington Lagoon and Barbuda's frigate-bird sanctuary" },
    { url: "https://www.visitantiguabarbuda.com/things-to-do/barbuda-must-see-attractions/", usedFor: "Barbuda's pink sand, lagoon and Two Foot Bay landscape context" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Antigua and Barbuda flag, coat of arms, official emblem, sacred symbol, religious building, ceremonial pattern, ceremonial attire, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular twin-island, English-Harbour, Georgian-masonry, Shirley-Heights, limestone-arch, lagoon and Caribbean-current compositions instead.",
    palette,
    minimumCoverage: "Scenes 1372 and 1373 each carry hard large Antigua and Barbuda motifs on three women and fictional-rescue-vessel construction language on at least two. Scenes 1374 and 1375 use four different theme-led fictional-rescue-vessel outfits without country map prints while Antigua and Barbuda landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Nelson's Dockyard at English Harbour, Shirley Heights, Devil's Bridge, and Barbuda's Codrington Lagoon frigate-bird sanctuary.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Antigua and Barbuda images plus one accepted Saint Vincent and the Grenadines image",
    captionIfEligible: `Antigua and Barbuda ${heartGlyph} Saint Vincent and the Grenadines ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-338-antigua-and-barbuda-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-338-antigua-and-barbuda-preflight.json"),
