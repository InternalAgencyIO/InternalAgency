...
import { djiboutiPalette, djiboutiProhibitions, djiboutiSceneSpecs } from "./djibouti-scene-specs.mjs";

const batch = 350;
const country = "Djibouti";
const countrySlug = "djibouti";
const firstScene = 1420;
const root = path.resolve("tmp/world-195x4/batch-350");
...
const commonProhibitions = djiboutiProhibitions;
...
const sceneSpecs = djiboutiSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  if (hasMale) outfits.Male = "the established Scene 1136 adult male face with closely trimmed beard and athletic build, an opaque fitted short-sleeve salt-white top with restrained Djibouti rift-and-gulf embroidery and subtle polar-airship seam tailoring, fitted black jeans, and practical black boots";
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Djibouti location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "South Sudan",
  nextQueueBatch: 351,
  nextQueueScenes: [1424, 1425, 1426, 1427],
  ...
  researchSources: [
    ...
    { url: "https://whc.unesco.org/en/tentativelists/5962/", usedFor: "Day Forest and Goda massif landscape" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Djibouti flag, official seal, sacred symbol, copied ceremonial or traditional pattern, political messaging, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular salt crystals, rift arcs, geothermal chimneys, gulf contours and juniper fans instead.",
    palette,
    minimumCoverage: "Scenes 1420 and 1421 each carry hard large Djibouti motifs on three women and polar-airship construction language on at least two. Scenes 1422 and 1423 use four different theme-led polar-airship outfits without country map prints while Djibouti landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Lake Assal, Lake Abbe, the Gulf of Tadjoura, and Day Forest National Park.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Djibouti images plus one accepted Vatican City image",
    captionIfEligible: `Djibouti ${heartGlyph} Vatican City ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-350-djibouti-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-350-djibouti-preflight.json"),
