...
import { tuvaluPalette, tuvaluProhibitions, tuvaluSceneSpecs } from "./tuvalu-scene-specs.mjs";

const batch = 348;
const country = "Tuvalu";
const countrySlug = "tuvalu";
const firstScene = 1412;
const root = path.resolve("tmp/world-195x4/batch-348");
...
const commonProhibitions = tuvaluProhibitions;
...
const legacyMicronesiaDraftSceneSpecs = [
  {
    ...
    theme: "deep-sea submersible couture",
...
const sceneSpecs = tuvaluSceneSpecs;
...
const weatherSafeText = (result, scene) => {
  ...
  if (result === "heavy snow or cinematic blizzard" && scene === 1412) return "the stored cinematic-blizzard roll appears as a dense curtain of harmless white sea mist, windblown pale frangipani petals, and cloud droplets beyond the waterfront shelter, with no snow accumulation and dry nonslip footing";
  if (result === "heavy snow or cinematic blizzard" && scene === 1414) return "the stored cinematic-blizzard roll appears as a dense line of natural white blowhole spray plumes beyond the covered overlook, with no snow accumulation and dry nonslip footing";
...
for (const spec of sceneSpecs) {
  ...
  if (hasMale) outfits.Male = "the established Scene 1136 adult male face with closely trimmed beard and athletic build, an opaque fitted short-sleeve pearl-white top with restrained Tuvalu atoll-rim embroidery and subtle deep-sea pressure-hull tailoring, fitted black jeans, and practical black boots";
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Tuvalu location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueBatch: 349,
  nextQueueScenes: [1416, 1417, 1418, 1419],
  nextCinematicTheme: { active: "polar airship couture", batchOrdinalWithinTheme: 1 },
  researchSources: [
    { url: "https://www.timelesstuvalu.com/", usedFor: "official Funafuti and visitor context" },
    { url: "https://tuvalu-data.sprep.org/resource/funafuti-conservation-area-funafuti-atoll-tuvalu", usedFor: "Funafuti Conservation Area landscape and conservation context" },
    { url: "https://tuvalu-data.sprep.org/system/files/211110-Tuvalu-State-of-Environment-report%20Final%20Interactive.pdf", usedFor: "Funafuti, Nanumea and Nukufetau reef and lagoon geography" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Tuvalu flag, official seal, sacred symbol, religious building, copied ceremonial or traditional pattern, political messaging, war wrecks, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular atoll arcs, lagoon contours, reef shelves, sand-islet chains and pandanus fans instead.",
    palette,
    minimumCoverage: "Scenes 1412 and 1413 each carry hard large Tuvalu motifs on three women and deep-sea submersible construction language on at least two. Scenes 1414 and 1415 use four different theme-led deep-sea submersible outfits without country map prints while Tuvalu landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Funafuti Lagoon, the Funafuti Conservation Area, Nanumea Atoll, and Nukufetau Atoll.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Tuvalu images plus one accepted Nauru image",
    captionIfEligible: `Tuvalu ${heartGlyph} Nauru ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-348-tuvalu-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-348-tuvalu-preflight.json"),
