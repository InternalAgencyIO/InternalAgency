...
import { gambiaPalette, gambiaProhibitions, gambiaSceneSpecs } from "./gambia-scene-specs.mjs";

const batch = 379;
const country = "Gambia";
const countrySlug = "gambia";
const firstScene = 1536;
const root = path.resolve("tmp/world-195x4/batch-379");
...
const commonProhibitions = gambiaProhibitions;
...
const sceneSpecs = gambiaSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Gambia location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Gabon",
  nextQueueBatch: 380,
  nextQueueScenes: [1540, 1541, 1542, 1543],
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Gambia flag, coat of arms, official seal, sacred symbol, copied traditional pattern, political messaging, copied military identity, police identity, emergency-service identity, or official service identity is used. Country-led scenes use large dimensional secular river-mouth curves, Banjul skyline steps, island outlines, solemn masonry walls and river bands instead.",
    palette,
    minimumCoverage: "Scenes 1536 and 1537 each carry hard large Gambia motifs on three women and orbital-spaceship construction language on at least two. Scenes 1538 and 1539 use four different theme-led orbital-spaceship outfits without country map prints while Gambian landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Banjul, Kunta Kinteh Island, Tanji, and the River Gambia mangroves.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Gambia images plus one accepted Qatar image",
    captionIfEligible: `Gambia ${heartGlyph} Qatar ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-379-gambia-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-379-gambia-preflight.json"),
