...
import { guatemalaPalette, guatemalaProhibitions, guatemalaSceneSpecs } from "./guatemala-scene-specs.mjs";

const batch = 354;
const country = "Guatemala";
const countrySlug = "guatemala";
const firstScene = 1436;
...
const palette = guatemalaPalette;
...
const commonProhibitions = guatemalaProhibitions;
...
const sceneSpecs = guatemalaSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Guatemala location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Ecuador",
  nextQueueBatch: 355,
  nextQueueScenes: [1440, 1441, 1442, 1443],
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Guatemala flag, coat of arms, official seal, sacred symbol, copied ceremonial or traditional pattern, political messaging, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular volcanic ridges, jade facets, civic relief grids, colonial arch curves, lake currents and rainforest contours instead.",
    palette,
    ...
    cultureScene: "The scenes foreground Guatemala City's Centro Civico, Antigua Guatemala's Santa Catalina Arch, Lake Atitlan, and Tikal's Great Plaza.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Guatemala images plus one accepted Senegal image",
    captionIfEligible: `Guatemala ${heartGlyph} Senegal ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-354-guatemala-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-354-guatemala-preflight.json"),
