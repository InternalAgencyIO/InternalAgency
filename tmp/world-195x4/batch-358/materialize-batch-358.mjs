...
import { zimbabwePalette, zimbabweProhibitions, zimbabweSceneSpecs } from "./zimbabwe-scene-specs.mjs";

const batch = 358;
const country = "Zimbabwe";
const countrySlug = "zimbabwe";
const firstScene = 1452;
...
const commonProhibitions = zimbabweProhibitions;
...
const sceneSpecs = zimbabweSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Zimbabwe location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Guinea",
  nextQueueBatch: 359,
  nextQueueScenes: [1456, 1457, 1458, 1459],
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Zimbabwe flag, official bird emblem, coat of arms, official seal, sacred symbol, copied ceremonial or traditional pattern, political messaging, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular balancing-rock contours, waterfall spray arcs, granite-wall chevrons, jacaranda forms and Zambezi channels instead.",
    palette,
    minimumCoverage: "Scenes 1452 and 1453 each carry hard large Zimbabwe motifs on three women and fictional rescue-vessel construction language on at least two. Scenes 1454 and 1455 use four different theme-led vessel outfits without country map prints while Zimbabwe landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Harare, Victoria Falls, Great Zimbabwe, and Mana Pools.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Zimbabwe images plus one accepted Cambodia image",
    captionIfEligible: `Zimbabwe ${heartGlyph} Cambodia ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-358-zimbabwe-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-358-zimbabwe-preflight.json"),
