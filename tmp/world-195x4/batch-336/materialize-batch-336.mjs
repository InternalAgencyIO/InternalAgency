...
import { tongaPalette, tongaProhibitions, tongaSceneSpecs } from "./tonga-scene-specs.mjs";

const batch = 336;
const country = "Tonga";
const countrySlug = "tonga";
const firstScene = 1364;
const root = path.resolve("tmp/world-195x4/batch-336");
...
const commonProhibitions = tongaProhibitions;
...
const legacyMicronesiaDraftSceneSpecs = [
  ...
  },
  {
    ...
    motifPolicy: "Hard country motifs dominate Radiance, Ellie, and Alia through leaf architecture, cinnamon weaving, granite relief, and bird-wing abstraction. Alia and ECE carry unmistakable civilian-helicopter construction language. No repeated map-print surfaces.",
  ...
  },
  {
    ...
    culture: "Anse Source d'Argent remains an equally dominant Micronesia foreground through its pale granite boulders, shallow turquoise water, reef shelves, sand arcs, takamaka trees, and distant islands. All four outfits are civilian-helicopter aviation originals with no forced country maps or matching surface prints.",
    motifPolicy: "Theme-led scene. All four outfits use different civilian-helicopter construction ideas and no garment uses an all-over Micronesia map print. The landmark itself carries the country read at equal visual weight.",
  ...
  },
  {
    ...
    culture: "Aldabra remains an equally dominant Micronesia foreground through its vast lagoon, raised coral rim, mangrove channels, reef flats, open ocean, distant wildlife, and conservation route. All four outfits are civilian-helicopter aviation originals with no forced country maps or matching surface prints.",
    motifPolicy: "Theme-led scene. All four outfits use different civilian-helicopter silhouette and material systems. Country identity comes from the unmistakable Aldabra landmark, not repeated map graphics.",
...
const sceneSpecs = tongaSceneSpecs;
...
const weatherSafeText = (result, scene) => {
  ...
  if (result === "heavy snow or cinematic blizzard" && scene === 1364) return "the stored cinematic-blizzard roll appears as a dense curtain of harmless white sea mist, windblown pale frangipani petals, and cloud droplets beyond the waterfront shelter, with no snow accumulation and dry nonslip footing";
  if (result === "heavy snow or cinematic blizzard" && scene === 1366) return "the stored cinematic-blizzard roll appears as a dense line of natural white blowhole spray plumes beyond the covered overlook, with no snow accumulation and dry nonslip footing";
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Tonga location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch336-tonga keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct. Batch 336 also stores one hard-love roll, mascot-state roll and holder selector, and odd-prop activation, holder, and family selectors per scene.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  nextQueueCountry: "Saint Vincent and the Grenadines",
  nextQueueBatch: 337,
  nextQueueScenes: [1368, 1369, 1370, 1371],
  nextCinematicTheme: { active: "fictional coast-guard rescue-vessel couture", batchOrdinalWithinTheme: 1 },
  researchSources: [
    { url: "https://www.tongatourism.travel/our-islands/tongatapu", usedFor: "Nuku'alofa waterfront and Royal Palace exterior, Ha'amonga 'a Maui, and Tongatapu's reef landscape" },
    { url: "https://whc.unesco.org/en/tentativelists/5167/", usedFor: "Ha'amonga 'a Maui Historical Park and its monumental coral-limestone trilithon" },
    { url: "https://www.tongatourism.travel/see-and-do/mapua-a-vaea-blowholes", usedFor: "Mapu'a 'a Vaea blowholes on Tongatapu's rugged coral-reef coast" },
    { url: "https://www.tongatourism.travel/our-islands/vavau", usedFor: "Vava'u island chain, Neiafu, sheltered anchorages, hilltop viewpoints, and Port of Refuge landscape" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Tonga flag, coat of arms, official emblem, sacred symbol, religious building, ceremonial pattern, ceremonial attire, or royal person is copied. Country-led scenes use large dimensional secular archipelago, waterfront, coral-limestone, trilithon, reef-shelf, blowhole-plume, pandanus, Vava'u-island, and Pacific-current compositions instead.",
    palette,
    minimumCoverage: "Scenes 1364 and 1365 each carry hard large Tonga motifs on three women and civilian-helicopter construction language on at least two. Scenes 1366 and 1367 use four different theme-led civilian-helicopter outfits without country map prints while Tonga landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Nuku'alofa waterfront and Royal Palace exterior, Ha'amonga 'a Maui, Mapu'a 'a Vaea blowholes, and Mount Talau above Neiafu and Port of Refuge in Vava'u.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Tonga images plus one accepted Saint Lucia image",
    captionIfEligible: `Tonga ${heartGlyph} Saint Lucia ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-336-tonga-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-336-tonga-preflight.json"),
