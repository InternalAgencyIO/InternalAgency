...
const countrySlug = "saint-lucia";
const firstScene = 1344;
const root = path.resolve("tmp/world-195x4/batch-331");
...
const palette = "Piton emerald, Caribbean turquoise, volcanic black, coral red, golden sun, cloud white, rainforest green, cocoa brown, hibiscus magenta, and deep ocean blue";
...
const commonProhibitions = "Use respectful secular Saint Lucia island outlines, twin Piton silhouettes, volcanic ridges, rainforest bands, cocoa-pod contours, bay crescents, reef shelves, waterfall ribbons, sulfur-spring steam arcs, cloud bands, and public overlooks only. No literal flag, coat of arms, official seal, sacred symbol, religious building, copied ceremonial pattern, ceremonial attire, copied uniform, badge, readable text, brand, alcohol, or political insignia.";
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Place large complete secular Saint Lucia motifs across at least two outfits as dominant full bodice, skirt, hip, and panel fields: ${spec.motifs}. ${commonProhibitions}`,
    ...
    rainbowOnly.active ? `Rainbow-only is active across the outfits while preserving complete Saint Lucia motifs.` : "Rainbow-only is inactive; do not convert the wardrobe to rainbow-only styling.",
...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch331-saint-lucia keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  nextQueueCountry: "Kiribati",
  nextQueueBatch: 332,
  nextQueueScenes: [1348, 1349, 1350, 1351],
  researchSources: [
    { url: "https://www.stlucia.org/en/experiences/nature-adventure/the-pitons/", usedFor: "Gros Piton, Petit Piton, Soufriere Bay, volcanic ridges, and rainforest context" },
    { url: "https://www.stlucia.org/en/experiences/nature-adventure/tet-paul-nature-trail/", usedFor: "Tet Paul trail, twin Piton views, terraced hills, and rainforest setting" },
    { url: "https://www.stlucia.org/en/experiences/nature-adventure/sulphur-springs/", usedFor: "Sulphur Springs crater terraces, mineral pools, steam, and volcanic soil" },
    { url: "https://www.stlucia.org/en/places-to-go/marigot-bay/", usedFor: "Marigot Bay crescent, forested hills, palm shores, reefs, and marina geometry" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Saint Lucia flag, coat of arms, official emblem, sacred symbol, religious building, ceremonial pattern, or ceremonial attire is copied. Large researched secular island, twin Piton, volcanic ridge, rainforest, cocoa-pod, bay, reef, steam, cloud, and route compositions replace them.",
    palette,
    minimumCoverage: "Every image carries large complete secular Saint Lucia compositions across at least two outfits as dominant full bodice, skirt, hip, or panel fields.",
    cultureScene: "The scenes foreground Soufriere Bay and the Pitons, Tet Paul Nature Trail, Sulphur Springs, and Marigot Bay.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Saint Lucia images plus one accepted Samoa image",
    captionIfEligible: `Saint Lucia ${heartGlyph} Samoa ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-331-saint-lucia-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-331-saint-lucia-preflight.json"),
