...
const countrySlug = "vanuatu";
const firstScene = 1328;
const root = path.resolve("tmp/world-195x4/batch-327");
...
const palette = "Pacific turquoise, lagoon blue, rainforest emerald, volcanic black, lava red, coral magenta, cloud white, pandanus gold, reef violet, and deep-ocean navy";
...
const commonProhibitions = "Use respectful secular Vanuatu island outlines, volcanic ridges, caldera contours, rainforest bands, reef arcs, lagoon channels, harbor geometry, black-sand and white-sand sweeps, waterfall ribbons, cloud bands, and public overlooks only. No literal flag, coat of arms, official seal, sacred symbol, religious building, copied ceremonial pattern, copied uniform, badge, readable text, brand, alcohol, or political insignia.";
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Place large complete secular Vanuatu motifs across at least two outfits as dominant full bodice, skirt, hip, and panel fields: ${spec.motifs}. ${commonProhibitions}`,
    ...
    rainbowOnly.active ? `Rainbow-only is active across the outfits while preserving complete Vanuatu motifs.` : "Rainbow-only is inactive; do not convert the wardrobe to rainbow-only styling.",
...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch327-vanuatu keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  nextQueueCountry: "Barbados",
  nextQueueBatch: 328,
  nextQueueScenes: [1332, 1333, 1334, 1335],
  researchSources: [
    { url: "https://www.vanuatu.travel/en/provinces/efate/port-vila", usedFor: "Port Vila waterfront, Mele Bay, harbor geometry, and island ridgelines" },
    { url: "https://www.vanuatu.travel/en/provinces/tanna/mount-yasur-volcano", usedFor: "Mount Yasur's volcanic cone, ash plain, caldera rim, rainforest, and public observation setting" },
    { url: "https://www.vanuatu.travel/en/provinces/efate/blue-lagoon", usedFor: "Efate Blue Lagoon, rainforest canopy, freshwater turquoise, and limestone banks" },
    { url: "https://www.vanuatu.travel/en/provinces/espiritu-santo/champagne-beach", usedFor: "Champagne Beach, pale sand, turquoise shallows, rainforest edge, and empty bay" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Vanuatu flag, coat of arms, official emblem, sacred symbol, or religious building is copied. Large researched secular island, volcano, rainforest, reef, lagoon, harbor, beach, waterfall, cloud, and route compositions replace them.",
    palette,
    minimumCoverage: "Every image carries large complete secular Vanuatu compositions across at least two outfits as dominant full bodice, skirt, hip, or panel fields.",
    cultureScene: "The scenes foreground Port Vila and Mele Bay, Mount Yasur's volcanic landscape, Efate's Blue Lagoon, and Champagne Beach on Espiritu Santo.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Vanuatu images plus one accepted Iceland image",
    captionIfEligible: `Vanuatu ${heartGlyph} Iceland ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-327-vanuatu-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-327-vanuatu-preflight.json"),
