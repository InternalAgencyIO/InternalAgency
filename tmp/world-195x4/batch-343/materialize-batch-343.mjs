...
import { monacoPalette, monacoProhibitions, monacoSceneSpecs } from "./monaco-scene-specs.mjs";

const batch = 343;
const country = "Monaco";
const countrySlug = "monaco";
const firstScene = 1392;
const root = path.resolve("tmp/world-195x4/batch-343");
...
const commonProhibitions = monacoProhibitions;
...
const sceneSpecs = monacoSceneSpecs;
...
const weatherSafeText = (result, scene) => {
  ...
  if (result === "heavy snow or cinematic blizzard" && scene === 1392) return "the stored cinematic-blizzard roll appears as a dense curtain of harmless white sea mist, windblown pale frangipani petals, and cloud droplets beyond the waterfront shelter, with no snow accumulation and dry nonslip footing";
  if (result === "heavy snow or cinematic blizzard" && scene === 1394) return "the stored cinematic-blizzard roll appears as a dense line of natural white blowhole spray plumes beyond the covered overlook, with no snow accumulation and dry nonslip footing";
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Monaco location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch343-monaco keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct. Batch 343 also stores one hard-love roll, mascot-state roll and holder selector, and odd-prop activation, holder, and family selectors per scene.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  nextQueueCountry: "Marshall Islands",
  nextQueueBatch: 344,
  nextQueueScenes: [1396, 1397, 1398, 1399],
  nextCinematicTheme: { active: "Moon-surface expedition couture", batchOrdinalWithinTheme: 2 },
  researchSources: [
    { url: "https://www.visitmonaco.com/Documents-Site/Depasser_2017_GB.pdf", usedFor: "Prince's Palace panoramas toward Port Hercule, La Condamine and Monte-Carlo" },
    { url: "https://cvb.visitmonaco.com/en/place/the-gardens/179/the-casino-gardens", usedFor: "Place du Casino garden terraces, ponds and subtropical planting" },
    { url: "https://www.visitmonaco.com/it/esplorare/i-12-luoghi-da-non-perdere-a-monaco/museo-oceanografico-di-monaco", usedFor: "Oceanographic Museum monumental marine architecture and conservation context" },
    { url: "https://www.visitmonaco.com/fr/lieu/quartier-de-monaco/26547/le-larvotto", usedFor: "Larvotto eastern waterfront and public beach setting" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Monaco flag, coat of arms, crown, royal insignia, official emblem, sacred symbol, religious building, ceremonial pattern, ceremonial attire, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular cliff-coast contours, Port Hercule marina arcs, casino rotunda geometry, garden terraces and Mediterranean ribbons instead.",
    palette,
    minimumCoverage: "Scenes 1392 and 1393 each carry hard large Monaco motifs on three women and Moon-expedition construction language on at least two. Scenes 1394 and 1395 use four different theme-led Moon-expedition outfits without country map prints while Monaco landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground the Prince's Palace and Port Hercule panorama, Place du Casino and its gardens, the Oceanographic Museum on the Rock, and Larvotto public beach.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Monaco images plus one accepted Liechtenstein image",
    captionIfEligible: `Monaco ${heartGlyph} Liechtenstein ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-343-monaco-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-343-monaco-preflight.json"),
