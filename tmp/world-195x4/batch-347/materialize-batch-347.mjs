...
import { nauruPalette, nauruProhibitions, nauruSceneSpecs } from "./nauru-scene-specs.mjs";

const batch = 347;
const country = "Nauru";
const countrySlug = "nauru";
const firstScene = 1408;
const root = path.resolve("tmp/world-195x4/batch-347");
...
const commonProhibitions = nauruProhibitions;
...
const sceneSpecs = nauruSceneSpecs;
...
const weatherSafeText = (result, scene) => {
  ...
  if (result === "heavy snow or cinematic blizzard" && scene === 1408) return "the stored cinematic-blizzard roll appears as a dense curtain of harmless white sea mist, windblown pale frangipani petals, and cloud droplets beyond the waterfront shelter, with no snow accumulation and dry nonslip footing";
  if (result === "heavy snow or cinematic blizzard" && scene === 1410) return "the stored cinematic-blizzard roll appears as a dense line of natural white blowhole spray plumes beyond the covered overlook, with no snow accumulation and dry nonslip footing";
...
for (const spec of sceneSpecs) {
  ...
  if (hasMale) outfits.Male = "the established Scene 1136 adult male face with closely trimmed beard and athletic build, an opaque fitted short-sleeve limestone-white top with restrained Nauru reef-shelf embroidery and subtle deep-sea pressure-hull tailoring, fitted black jeans, and practical black boots";
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Nauru location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Tuvalu",
  nextQueueBatch: 348,
  nextQueueScenes: [1412, 1413, 1414, 1415],
  nextCinematicTheme: { active: "deep-sea submersible couture", batchOrdinalWithinTheme: 2 },
  researchSources: [
    { url: "https://www.nauru.gov.nr/about-nauru/visiting-nauru.aspx", usedFor: "official Nauru visitor context" },
    { url: "https://nauru-data.sprep.org/system/files/nr-nr-05-en.pdf", usedFor: "Anibare Bay and Buada Lagoon conservation landscapes" },
    { url: "https://www.nauru.gov.nr/media/201581/nauru_bulletin__10__24_june_2024__275_.pdf.pdf", usedFor: "Moqua Caves visitor-site context" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Nauru flag, official seal, sacred symbol, religious building, copied ceremonial or traditional pattern, political messaging, war wrecks, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular reef shelves, limestone pinnacles, lagoon contours, pandanus fans and cave-water reflections instead.",
    palette,
    minimumCoverage: "Scenes 1408 and 1409 each carry hard large Nauru motifs on three women and deep-sea submersible construction language on at least two. Scenes 1410 and 1411 use four different theme-led deep-sea submersible outfits without country map prints while Nauru landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Anibare Bay, Buada Lagoon, Command Ridge, and Moqua Caves.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Nauru images plus one accepted Palau image",
    captionIfEligible: `Nauru ${heartGlyph} Palau ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-347-nauru-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-347-nauru-preflight.json"),
