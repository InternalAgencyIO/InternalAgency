...
const batch = 330;
const country = "Samoa";
const countrySlug = "samoa";
const firstScene = 1340;
const root = path.resolve("tmp/world-195x4/batch-330");
...
const palette = "lagoon turquoise, rainforest green, volcanic black, coral red, ocean cobalt, sunset gold, cloud white, lava gray, hibiscus magenta, and seafoam aqua";
...
const commonProhibitions = "Use respectful secular Samoa island outlines, harbor crescents, volcanic ridges, rainforest bands, lava-stone grids, sinkhole rings, blowhole plumes, beach crescents, reef shelves, cloud bands, and public overlooks only. No literal flag, coat of arms, official seal, sacred symbol, religious building, copied ceremonial pattern, copied tatau, ceremonial attire, copied uniform, badge, readable text, brand, alcohol, or political insignia.";
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Place large complete secular Samoa motifs across at least two outfits as dominant full bodice, skirt, hip, and panel fields: ${spec.motifs}. ${commonProhibitions}`,
    ...
    rainbowOnly.active ? `Rainbow-only is active across the outfits while preserving complete Samoa motifs.` : "Rainbow-only is inactive; do not convert the wardrobe to rainbow-only styling.",
...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch330-samoa keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  nextQueueCountry: "Saint Lucia",
  nextQueueBatch: 331,
  nextQueueScenes: [1344, 1345, 1346, 1347],
  researchSources: [
    { url: "https://www.samoa.travel/discover/our-islands/upolu/apia/", usedFor: "Apia Harbour, Upolu waterfront geography, Mount Vaea, and reef context" },
    { url: "https://www.samoa.travel/plan-book/activities/to-sua-ocean-trench/", usedFor: "To Sua sinkhole pool, basalt rim, ladder, vines, and rainforest setting" },
    { url: "https://www.samoa.travel/plan-book/activities/alofaaga-blowholes/", usedFor: "Alofaaga black-lava shelves, ocean blowhole plumes, and Pacific coast" },
    { url: "https://www.samoa.travel/plan-book/activities/lalomanu-beach/", usedFor: "Lalomanu beach crescent, lagoon, Nu'utele silhouette, reefs, and rainforest edge" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Samoa flag, coat of arms, official emblem, sacred symbol, religious building, tatau, ceremonial pattern, or ceremonial attire is copied. Large researched secular island, harbor, volcanic ridge, sinkhole, lava shelf, blowhole plume, beach, reef, cloud, and route compositions replace them.",
    palette,
    minimumCoverage: "Every image carries large complete secular Samoa compositions across at least two outfits as dominant full bodice, skirt, hip, or panel fields.",
    cultureScene: "The scenes foreground Apia Harbour, To Sua Ocean Trench, Alofaaga Blowholes, and Lalomanu Beach.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Samoa images plus one accepted São Tomé and Príncipe image",
    captionIfEligible: `Samoa ${heartGlyph} São Tomé and Príncipe ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-330-samoa-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-330-samoa-preflight.json"),
