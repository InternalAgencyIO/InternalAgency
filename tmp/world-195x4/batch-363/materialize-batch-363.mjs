...
import { boliviaPalette, boliviaProhibitions, boliviaSceneSpecs } from "./bolivia-scene-specs.mjs";

const batch = 363;
const country = "Bolivia";
const countrySlug = "bolivia";
const firstScene = 1472;
const root = path.resolve("tmp/world-195x4/batch-363");
...
const commonProhibitions = boliviaProhibitions;
...
const sceneSpecs = boliviaSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Bolivia location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Tunisia",
  nextQueueBatch: 364,
  nextQueueScenes: [1476, 1477, 1478, 1479],
  nextCinematicTheme: { active: "Moon-surface expedition couture", batchOrdinalWithinTheme: 2 },
  researchSources: [
    { url: "https://www.britannica.com/place/La-Paz-Bolivia", usedFor: "La Paz basin, cable-car and Illimani setting" },
    { url: "https://www.britannica.com/place/Bolivia", usedFor: "Bolivia Altiplano, mountains, salt flats and cities" },
    { url: "https://www.britannica.com/place/Salar-de-Uyuni", usedFor: "Salar de Uyuni salt plain and horizon" },
    { url: "https://www.britannica.com/place/Lake-Titicaca", usedFor: "Lake Titicaca and Cordillera landscape" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Bolivia flag, coat of arms, official seal, sacred symbol, copied ceremonial or traditional pattern, political messaging, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular mountain profiles, cable-car arcs, salt polygons, lake currents, white-city terraces and highland strata instead.",
    palette,
    minimumCoverage: "Scenes 1472 and 1473 each carry hard large Bolivia motifs on three women and Moon-expedition construction language on at least two. Scenes 1474 and 1475 use four different theme-led lunar expedition outfits without country map prints while Bolivia landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground La Paz and Illimani, Salar de Uyuni, Lake Titicaca and Copacabana, and Sucre.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Bolivia images plus one accepted Burundi image",
    captionIfEligible: `Bolivia ${heartGlyph} Burundi ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-363-bolivia-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-363-bolivia-preflight.json"),
