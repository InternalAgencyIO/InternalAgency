...
import { jordanPalette, jordanProhibitions, jordanSceneSpecs } from "./jordan-scene-specs.mjs";

const batch = 368;
const country = "Jordan";
const countrySlug = "jordan";
const firstScene = 1492;
const root = path.resolve("tmp/world-195x4/batch-368");
...
const commonProhibitions = jordanProhibitions;
...
const sceneSpecs = jordanSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Jordan location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Dominican Republic",
  nextQueueBatch: 369,
  nextQueueScenes: [1496, 1497, 1498, 1499],
  nextCinematicTheme: { active: "polar airship couture", batchOrdinalWithinTheme: 1 },
  researchSources: [
    { url: "https://www.britannica.com/place/Amman", usedFor: "Amman Citadel and limestone hill-city setting" },
    { url: "https://www.britannica.com/place/Jordan", usedFor: "Jordan cities, desert geography and Aqaba coastline" },
    { url: "https://whc.unesco.org/en/list/326/", usedFor: "Petra architecture and rose-stone canyon setting" },
    { url: "https://whc.unesco.org/en/list/1377/", usedFor: "Wadi Rum desert landscape and sandstone formations" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Jordan flag, coat of arms, official seal, sacred symbol, copied ceremonial or traditional pattern, political messaging, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular limestone terraces, rose-stone facades, canyon strata, desert arches, gulf bands and mountain silhouettes instead.",
    palette,
    minimumCoverage: "Scenes 1492 and 1493 each carry hard large Jordan motifs on three women and submersible construction language on at least two. Scenes 1494 and 1495 use four different theme-led deep-sea outfits without country map prints while Jordan landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Amman Citadel, Petra, Wadi Rum, and Aqaba.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Jordan images plus one accepted Haiti image",
    captionIfEligible: `Jordan ${heartGlyph} Haiti ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-368-jordan-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-368-jordan-preflight.json"),
