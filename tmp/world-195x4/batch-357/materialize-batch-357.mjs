...
import { cambodiaPalette, cambodiaProhibitions, cambodiaSceneSpecs } from "./cambodia-scene-specs.mjs";

const batch = 357;
const country = "Cambodia";
const countrySlug = "cambodia";
const firstScene = 1448;
...
const commonProhibitions = cambodiaProhibitions;
...
const sceneSpecs = cambodiaSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Cambodia location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Zimbabwe",
  nextQueueBatch: 358,
  nextQueueScenes: [1452, 1453, 1454, 1455],
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Cambodia flag, coat of arms, official seal, sacred symbol, copied ceremonial or traditional pattern, political messaging, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular Mekong braids, sandstone tower profiles, sugar-palm fans, lake currents, island contours and coral-water arcs instead.",
    palette,
    minimumCoverage: "Scenes 1448 and 1449 each carry hard large Cambodia motifs on three women and fictional rescue-vessel construction language on at least two. Scenes 1450 and 1451 use four different theme-led vessel outfits without country map prints while Cambodia landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Phnom Penh's river confluence, Angkor's archaeological landscape, Tonle Sap, and Koh Rong.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Cambodia images plus one accepted Netherlands image",
    captionIfEligible: `Cambodia ${heartGlyph} Netherlands ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-357-cambodia-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-357-cambodia-preflight.json"),
