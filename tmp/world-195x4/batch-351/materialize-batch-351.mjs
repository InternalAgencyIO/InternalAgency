...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch351-south-sudan keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct. Batch 351 also stores one hard-love roll, mascot-state roll and holder selector, and odd-prop activation, holder, and family selectors per scene.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  nextQueueCountry: "Somalia",
  nextQueueBatch: 352,
  nextQueueScenes: [1428, 1429, 1430, 1431],
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal South Sudan flag, official seal, sacred symbol, copied ceremonial or traditional pattern, political messaging, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular Nile-current bands, papyrus fans, wetland channels, savanna migration arcs and mountain contours instead.",
    palette,
    minimumCoverage: "Scenes 1424 and 1425 each carry hard large South Sudan motifs on three women and orbital research-station construction language on at least two. Scenes 1426 and 1427 use four different theme-led orbital-station outfits without country map prints while South Sudan landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Juba and the White Nile, the Sudd wetlands, Boma National Park, and the Imatong Mountains.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two South Sudan images plus one accepted Djibouti image",
    captionIfEligible: `South Sudan ${heartGlyph} Djibouti ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-351-south-sudan-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-351-south-sudan-preflight.json"),
