...
const firstScene = 1284;
const root = path.resolve("tmp/world-195x4/batch-316");
...
const sceneSpecs = [
  ...
  },
  {
    ...
    "culture": "Use Phobjikha's glacial valley, threatened black-necked cranes, wetland, dwarf bamboo, potato farms, rhododendrons, village life, and conservation monitoring respectfully. Paris runway model couture is original fictional fashion with no copied designer, logo, uniform, unsafe runway, or sexualized presentation. Use secular Bhutan landscape, wildlife, food, farming, weaving, conservation, sport, and civic-infrastructure references only. No literal flag, thunder dragon, coat of arms, official seal, sacred symbol, religious building, prayer flag, copied ceremonial dress, copied uniform, badge, readable text, brand, alcohol, or political insignia.",
    "expected": {
      "weather": "stacked lenticular clouds",
      "paws": true,
      "pole": false,
      "rainbowOnly": false,
      "rainbowHosiery": true,
...
if (maleScene !== 1285) throw new Error(`Male scene drifted to ${maleScene}`);
...
for (const spec of sceneSpecs) {
  ...
  const rainbowOnlyLine = rainbowOnly.active
    ? "Rainbow-only is active. All outfits use varied public-safe rainbow palettes while preserving the large Bhutan motifs."
  ...
  const renderPrompt = [
    ...
    `Large complete secular Bhutan motifs dominate at least two outfits: ${spec.motifs.join("; ")}. ${spec.culture}`,
...
fs.writeFileSync(path.join(root, "batch-316-bhutan-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");
