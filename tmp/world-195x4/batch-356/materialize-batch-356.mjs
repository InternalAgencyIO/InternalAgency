...
import { netherlandsPalette, netherlandsProhibitions, netherlandsSceneSpecs } from "./netherlands-scene-specs.mjs";

const batch = 356;
const country = "Netherlands";
const countrySlug = "netherlands";
const firstScene = 1444;
...
const palette = netherlandsPalette;
...
const commonProhibitions = netherlandsProhibitions;
...
const sceneSpecs = netherlandsSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Netherlands location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Cambodia",
  nextQueueBatch: 357,
  nextQueueScenes: [1448, 1449, 1450, 1451],
...
fs.writeFileSync(path.join(root, "batch-356-netherlands-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-356-netherlands-preflight.json"),
