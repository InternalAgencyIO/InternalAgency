...
const batch = 322;
const country = "Cabo Verde";
const countrySlug = "cabo-verde";
const firstScene = 1308;
...
const palette = "Atlantic cobalt, volcanic charcoal, basalt black, bougainvillea magenta, sugarcane green, colonial-stone ivory, harbor turquoise, sunset gold, cloud white, and deep-ocean navy";
...
const commonProhibitions = "Use respectful secular Cabo Verde island silhouettes, Atlantic waves, volcanic calderas, basalt ridges, green valleys, harbor geometry, cobbled streets, stone walls, sail routes, and public overlooks only. No literal flag, coat of arms, official seal, sacred symbol, religious building, copied ceremonial pattern, copied uniform, badge, readable text, brand, alcohol, or political insignia.";
...
const weatherSafeText = (result, scene) => {
  if (scene === 1310 && result === "snow flurries") return "the stored snow-flurries weather roll appears as harmless windblown white cloud-mist flecks across the high caldera, with no accumulation and a dry terrace";
  if (result === "snow flurries") return "the stored snow-flurries weather roll appears as windblown white Atlantic sea-spray flakes in tropical air, never snow accumulation";
  return result;
};
...
const garment = (character, plan, spec, index) => {
  const colors = ["Atlantic cobalt", "bougainvillea magenta", "sugarcane green", "volcanic charcoal"];
...
for (const spec of sceneSpecs) {
  ...
  const resolvedHands = poseTargetRoll.roll >= 60 && poseTargetRoll.roll <= 74
    ? wristGuidanceHands(shortHandler, hasMale)
    : paws.active
  ...
  if (hasMale) outfits.Male = `the established Scene 1136 adult male face with closely trimmed beard and athletic build, an opaque fitted short-sleeve colonial-stone ivory top carrying a large complete ${spec.motifs} field, fitted black jeans, and practical black boots`;
  ...
  const renderPrompt = [
    ...
    `Place large complete secular Cabo Verde motifs across at least two outfits as dominant full bodice, skirt, hip, and panel fields: ${spec.motifs}. ${commonProhibitions}`,
    ...
    rainbowOnly.active ? `Rainbow-only is active across the outfits while preserving complete Cabo Verde motifs.` : "Rainbow-only is inactive; do not convert the wardrobe to rainbow-only styling.",
...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch322-cabo-verde keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  nextQueueCountry: "Brunei",
  nextQueueBatch: 323,
  nextQueueScenes: [1312, 1313, 1314, 1315],
  researchSources: [
    { url: "https://whc.unesco.org/en/list/1310", usedFor: "Cidade Velha historic street layout, stone architecture, valley-to-sea relationship, and hilltop fortress setting" },
    { url: "https://www.visit-caboverde.com/en/islands/santo-antao-island", usedFor: "Santo Antao rocky terrain, green valleys, Paul, mountain roads, and Atlantic setting" },
    { url: "https://www.visit-caboverde.com/en/islands/fogo-island", usedFor: "Fogo volcanic terrain, Pico do Fogo cone, caldera, black lava, and highland agriculture" },
    { url: "https://www.visit-caboverde.com/en/islands/sao-vicente-island", usedFor: "Mindelo, Sao Vicente volcanic ridges, Monte Verde, colorful architecture, music culture, and harbor setting" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Cabo Verde flag, coat of arms, official emblem, or sacred symbol is copied. Large researched secular island, Atlantic, volcanic, valley, harbor, stone-street, cloud, and route compositions replace them.",
    palette,
    minimumCoverage: "Every image carries large complete secular Cabo Verde compositions across at least two outfits as dominant full bodice, skirt, hip, or panel fields.",
    cultureScene: "The scenes foreground Cidade Velha on Santiago, Paul Valley on Santo Antao, Cha das Caldeiras on Fogo, and Mindelo's Porto Grande on Sao Vicente.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Cabo Verde images plus one accepted Maldives image",
    captionIfEligible: `Cabo Verde ${heartGlyph} Maldives ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-322-cabo-verde-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-322-cabo-verde-preflight.json"),
