...
for (let offset = 0; offset < 4; offset += 1) {
  ...
  for (const character of characters) {
    ...
    charactersOut[character] = {
      ...
      visibleMidriff: { roll: roll(`${prefix}-${character}-visibleMidriff`), active: roll(`${prefix}-${character}-visibleMidriff`) <= 49 },
      straplessDress: { roll: roll(`${prefix}-${character}-straplessDress`), active: roll(`${prefix}-${character}-straplessDress`) <= 34 },
      fullyOpenBack: { roll: roll(`${prefix}-${character}-fullyOpenBack`), active: roll(`${prefix}-${character}-fullyOpenBack`) <= 29 },
    };
  }
  scenes[scene] = {
    position: offset + 1,
    theme: offset < 2 ? "nurse-care couture" : "doctor-clinical-command couture",
    ...
    compoundLoveBeat: {
      ...
      result: contract.romance.compoundLoveBeatRolls[compoundRoll % contract.romance.compoundLoveBeatRolls.length],
    },
