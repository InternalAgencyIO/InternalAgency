...
import { palauPalette, palauProhibitions, palauSceneSpecs } from "./palau-scene-specs.mjs";

const batch = 346;
const country = "Palau";
const countrySlug = "palau";
const firstScene = 1404;
const root = path.resolve("tmp/world-195x4/batch-346");
...
const commonProhibitions = palauProhibitions;
...
const sceneSpecs = palauSceneSpecs;
...
const weatherSafeText = (result, scene) => {
  ...
  if (result === "heavy snow or cinematic blizzard" && scene === 1404) return "the stored cinematic-blizzard roll appears as a dense curtain of harmless white sea mist, windblown pale frangipani petals, and cloud droplets beyond the waterfront shelter, with no snow accumulation and dry nonslip footing";
  if (result === "heavy snow or cinematic blizzard" && scene === 1406) return "the stored cinematic-blizzard roll appears as a dense line of natural white blowhole spray plumes beyond the covered overlook, with no snow accumulation and dry nonslip footing";
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Palau location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch346-palau keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct. Batch 346 also stores one hard-love roll, mascot-state roll and holder selector, and odd-prop activation, holder, and family selectors per scene.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  nextQueueCountry: "Nauru",
  nextQueueBatch: 347,
  nextQueueScenes: [1408, 1409, 1410, 1411],
  nextCinematicTheme: { active: "deep-sea submersible couture", batchOrdinalWithinTheme: 1 },
  researchSources: [
    { url: "https://www.kororstategov.com/tourist.html", usedFor: "Rock Islands Southern Lagoon scale and Koror visitor context" },
    { url: "https://whc.unesco.org/en/list/1386/", usedFor: "protected Rock Islands Southern Lagoon landscape and conservation context" },
    { url: "https://pic.or.jp/en/wp-content/uploads/2024/08/Pacific-Island-Tourism-Brochure_Final.pdf", usedFor: "Milky Way, Long Beach and Ngardmau Waterfall visitor landmarks" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Palau flag, official seal, sacred symbol, religious building, copied ceremonial or traditional pattern, political messaging, war wrecks, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular mushroom-islet silhouettes, lagoon channels, coral fans, sandbar ribbons and waterfall veils instead.",
    palette,
    minimumCoverage: "Scenes 1404 and 1405 each carry hard large Palau motifs on three women and near-Sun solar-observation construction language on at least two. Scenes 1406 and 1407 use four different theme-led near-Sun solar-observation outfits without country map prints while Palau landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground the Rock Islands Southern Lagoon, Milky Way Lagoon, Long Beach, and Ngardmau Waterfall.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Palau images plus one accepted San Marino image",
    captionIfEligible: `Palau ${heartGlyph} San Marino ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-346-palau-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-346-palau-preflight.json"),
