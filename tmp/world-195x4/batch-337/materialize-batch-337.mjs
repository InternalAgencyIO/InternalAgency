...
import { saintVincentPalette, saintVincentProhibitions, saintVincentSceneSpecs } from "./saint-vincent-scene-specs.mjs";

const batch = 337;
const country = "Saint Vincent and the Grenadines";
const countrySlug = "saint-vincent-and-the-grenadines";
const firstScene = 1368;
...
const palette = saintVincentPalette;
...
const commonProhibitions = saintVincentProhibitions;
...
const legacyMicronesiaDraftSceneSpecs = [
  ...
  },
  {
    ...
    motifPolicy: "Hard country motifs dominate Radiance, Ellie, and Alia through leaf architecture, cinnamon weaving, granite relief, and bird-wing abstraction. Alia and ECE carry unmistakable fictional-rescue-vessel construction language. No repeated map-print surfaces.",
  ...
  },
  {
    ...
    culture: "Anse Source d'Argent remains an equally dominant Micronesia foreground through its pale granite boulders, shallow turquoise water, reef shelves, sand arcs, takamaka trees, and distant islands. All four outfits are fictional-rescue-vessel aviation originals with no forced country maps or matching surface prints.",
    motifPolicy: "Theme-led scene. All four outfits use different fictional-rescue-vessel construction ideas and no garment uses an all-over Micronesia map print. The landmark itself carries the country read at equal visual weight.",
  ...
  },
  {
    ...
    culture: "Aldabra remains an equally dominant Micronesia foreground through its vast lagoon, raised coral rim, mangrove channels, reef flats, open ocean, distant wildlife, and conservation route. All four outfits are fictional-rescue-vessel aviation originals with no forced country maps or matching surface prints.",
    motifPolicy: "Theme-led scene. All four outfits use different fictional-rescue-vessel silhouette and material systems. Country identity comes from the unmistakable Aldabra landmark, not repeated map graphics.",
...
const sceneSpecs = saintVincentSceneSpecs;
...
const weatherSafeText = (result, scene) => {
  ...
  if (result === "heavy snow or cinematic blizzard" && scene === 1368) return "the stored cinematic-blizzard roll appears as a dense curtain of harmless white sea mist, windblown pale frangipani petals, and cloud droplets beyond the waterfront shelter, with no snow accumulation and dry nonslip footing";
  if (result === "heavy snow or cinematic blizzard" && scene === 1370) return "the stored cinematic-blizzard roll appears as a dense line of natural white blowhole spray plumes beyond the covered overlook, with no snow accumulation and dry nonslip footing";
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Saint Vincent and the Grenadines location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const hashtags = ["#SaintVincentAndTheGrenadines"];
...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch337-saint-vincent-and-the-grenadines keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct. Batch 337 also stores one hard-love roll, mascot-state roll and holder selector, and odd-prop activation, holder, and family selectors per scene.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  nextQueueCountry: "Antigua and Barbuda",
  nextQueueBatch: 338,
  nextQueueScenes: [1372, 1373, 1374, 1375],
  nextCinematicTheme: { active: "fictional coast-guard rescue-vessel couture", batchOrdinalWithinTheme: 2 },
  researchSources: [
    { url: "https://whc.unesco.org/en/tentativelists/5751/", usedFor: "La Soufriere and Saint Vincent's volcanic Lesser Antilles landscape" },
    { url: "https://whc.unesco.org/en/tentativelists/5750/", usedFor: "The Grenadines island group, Tobago Cays reef and island-chain setting" },
    { url: "https://discoversvg.com/what-to-do/eco-adventures/", usedFor: "Dark View Falls and its two cascades on Saint Vincent's northwest coast" },
    { url: "https://discoversvg.com/island/bequia/", usedFor: "Bequia, Port Elizabeth, Admiralty Bay and the sheltered anchorage" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Saint Vincent and the Grenadines flag, coat of arms, official emblem, sacred symbol, religious building, ceremonial pattern, ceremonial attire, or official service identity is copied. Country-led scenes use large dimensional secular island-chain, Kingstown-harbor, Fort Charlotte stone, waterfall, volcanic-ridge, reef-ring and Caribbean-current compositions instead.",
    palette,
    minimumCoverage: "Scenes 1368 and 1369 each carry hard large Saint Vincent and the Grenadines motifs on three women and fictional-rescue-vessel construction language on at least two. Scenes 1370 and 1371 use four different theme-led fictional-rescue-vessel outfits without country map prints while Saint Vincent and the Grenadines landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Kingstown harbor and Fort Charlotte, Dark View Falls, Tobago Cays Marine Park, and Bequia's Port Elizabeth and Admiralty Bay.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Saint Vincent and the Grenadines images plus one accepted Tonga image",
    captionIfEligible: `Saint Vincent and the Grenadines ${heartGlyph} Tonga ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-337-saint-vincent-and-the-grenadines-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-337-saint-vincent-and-the-grenadines-preflight.json"),
