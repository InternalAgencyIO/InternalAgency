...
import { marshallIslandsPalette, marshallIslandsProhibitions, marshallIslandsSceneSpecs } from "./marshall-islands-scene-specs.mjs";

const batch = 344;
const country = "Marshall Islands";
const countrySlug = "marshall-islands";
const firstScene = 1396;
const root = path.resolve("tmp/world-195x4/batch-344");
...
const palette = marshallIslandsPalette;
...
const commonProhibitions = marshallIslandsProhibitions;
...
const legacyMicronesiaDraftSceneSpecs = [
  {
    ...
    theme: "Moon-surface expedition couture",
...
const sceneSpecs = marshallIslandsSceneSpecs;
...
const weatherSafeText = (result, scene) => {
  ...
  if (result === "heavy snow or cinematic blizzard" && scene === 1396) return "the stored cinematic-blizzard roll appears as a dense curtain of harmless white sea mist, windblown pale frangipani petals, and cloud droplets beyond the waterfront shelter, with no snow accumulation and dry nonslip footing";
  if (result === "heavy snow or cinematic blizzard" && scene === 1398) return "the stored cinematic-blizzard roll appears as a dense line of natural white blowhole spray plumes beyond the covered overlook, with no snow accumulation and dry nonslip footing";
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Marshall Islands location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const hashtags = ["#MarshallIslands"];
...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch344-marshall-islands keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct. Batch 344 also stores one hard-love roll, mascot-state roll and holder selector, and odd-prop activation, holder, and family selectors per scene.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  cinematicTheme: {
    active: "Moon-surface expedition couture",
  ...
  nextQueueCountry: "San Marino",
  nextQueueBatch: 345,
  nextQueueScenes: [1400, 1401, 1402, 1403],
  nextCinematicTheme: { active: "near-Sun solar-observation couture", batchOrdinalWithinTheme: 1 },
  researchSources: [
    { url: "https://www.visitmarshallislands.mh/about", usedFor: "national atoll geography, Majuro capital context and Arno lagoon character" },
    { url: "https://www.visitmarshallislands.mh/things-to-do", usedFor: "Majuro, Arno and outer-atoll visitor context" },
    { url: "https://www.ocit.mh/aa/getting-around", usedFor: "Laura Beach Park and Arno beach and lagoon access" },
    { url: "https://www.visitmarshallislands.mh/plan-your-trip", usedFor: "Majuro, Arno and Jaluit destination context" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Marshall Islands flag, seal, official emblem, sacred symbol, religious building, copied ceremonial or traditional pattern, political messaging, nuclear-test imagery, war wrecks, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular atoll rings, lagoon channels, coral-islet chains, abstract navigation lines and Pacific-current ribbons instead.",
    palette,
    minimumCoverage: "Scenes 1396 and 1397 each carry hard large Marshall Islands motifs on three women and Moon-expedition construction language on at least two. Scenes 1398 and 1399 use four different theme-led Moon-expedition outfits without country map prints while Marshall Islands landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Majuro Lagoon and the capital ribbon, Laura Beach, Arno Atoll, and Jaluit Atoll.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Marshall Islands images plus one accepted Monaco image",
    captionIfEligible: `Marshall Islands ${heartGlyph} Monaco ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-344-marshall-islands-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-344-marshall-islands-preflight.json"),
