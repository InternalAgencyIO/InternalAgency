...
import { unitedArabEmiratesPalette, unitedArabEmiratesProhibitions, unitedArabEmiratesSceneSpecs } from "./united-arab-emirates-scene-specs.mjs";

const batch = 370;
const country = "United Arab Emirates";
const countrySlug = "united-arab-emirates";
const firstScene = 1500;
const root = path.resolve("tmp/world-195x4/batch-370");
...
const palette = unitedArabEmiratesPalette;
...
const commonProhibitions = unitedArabEmiratesProhibitions;
...
const sceneSpecs = unitedArabEmiratesSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and United Arab Emirates location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const hashtags = ["#UnitedArabEmirates"];
...
const preflight = {
  ...
  nextQueueCountry: "Cuba",
  nextQueueBatch: 371,
  nextQueueScenes: [1504, 1505, 1506, 1507],
  nextCinematicTheme: { active: "orbital research-station couture", batchOrdinalWithinTheme: 1 },
  researchSources: [
    { url: "https://www.britannica.com/place/Dubai-United-Arab-Emirates", usedFor: "Dubai skyline and desert setting" },
    { url: "https://www.britannica.com/place/United-Arab-Emirates", usedFor: "United Arab Emirates cities, Gulf coast and mountain geography" },
    { url: "https://whc.unesco.org/en/list/1343/", usedFor: "Al Ain oasis and falaj cultural landscape" },
    { url: "https://www.louvreabudhabi.ae/en/about-us", usedFor: "Louvre Abu Dhabi dome and waterfront architecture" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal United Arab Emirates flag, coat of arms, official seal, sacred symbol, copied ceremonial or traditional pattern, political messaging, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular skyline spires, Gulf curves, museum-dome lattice, oasis channels, palm fans, mountain switchbacks and escarpment layers instead.",
    palette,
    minimumCoverage: "Scenes 1500 and 1501 each carry hard large United Arab Emirates motifs on three women and airship construction language on at least two. Scenes 1502 and 1503 use four different theme-led polar-airship outfits without country map prints while United Arab Emirates landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Dubai, Abu Dhabi, Al Ain Oasis, and Jebel Jais.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two United Arab Emirates images plus one accepted Dominican Republic image",
    captionIfEligible: `United Arab Emirates ${heartGlyph} Dominican Republic ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-370-united-arab-emirates-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-370-united-arab-emirates-preflight.json"),
