...
import { qatarPalette, qatarProhibitions, qatarSceneSpecs } from "./qatar-scene-specs.mjs";

const batch = 377;
const country = "Qatar";
const countrySlug = "qatar";
const firstScene = 1528;
const root = path.resolve("tmp/world-195x4/batch-377");
...
const commonProhibitions = qatarProhibitions;
...
const sceneSpecs = qatarSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Qatar location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Jamaica",
  nextQueueBatch: 378,
  nextQueueScenes: [1532, 1533, 1534, 1535],
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Qatar flag, coat of arms, official seal, sacred symbol, copied traditional pattern, political messaging, copied military identity, police identity, emergency-service identity, or official service identity is used. Country-led scenes use large dimensional secular Corniche curves, skyline fins, dhow-harbor ribs, fort walls, corner towers and settlement grids instead.",
    palette,
    minimumCoverage: "Scenes 1528 and 1529 each carry hard large Qatar motifs on three women and fictional rescue-vessel construction language on at least two. Scenes 1530 and 1531 use four different theme-led rescue-vessel outfits without country map prints while Qatar landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Doha Corniche, Al Zubarah, Khor Al Adaid, and Al Thakhira mangroves.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Qatar images plus one accepted Armenia image",
    captionIfEligible: `Qatar ${heartGlyph} Armenia ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-377-qatar-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-377-qatar-preflight.json"),
