...
import { lithuaniaPalette, lithuaniaProhibitions, lithuaniaSceneSpecs } from "./lithuania-scene-specs.mjs";

const batch = 376;
const country = "Lithuania";
const countrySlug = "lithuania";
const firstScene = 1524;
const root = path.resolve("tmp/world-195x4/batch-376");
...
const palette = lithuaniaPalette;
...
const commonProhibitions = lithuaniaProhibitions;
...
const sceneSpecs = lithuaniaSceneSpecs;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Lithuania location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Qatar",
  nextQueueBatch: 377,
  nextQueueScenes: [1528, 1529, 1530, 1531],
  nextCinematicTheme: { active: "fictional coast-guard rescue-vessel couture", batchOrdinalWithinTheme: 1 },
  researchSources: [
    { url: "https://www.britannica.com/place/Lithuania", usedFor: "Lithuania geography, cities and regions" },
    { url: "https://whc.unesco.org/en/list/541/", usedFor: "Vilnius historic center and skyline" },
    { url: "https://whc.unesco.org/en/list/994/", usedFor: "Curonian Spit dunes and lagoon" },
    { url: "https://www.britannica.com/place/Kaunas", usedFor: "Kaunas confluence city setting" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Lithuania flag, coat of arms, official seal, sacred symbol, copied folk pattern, copied ceremonial pattern, political messaging, copied military identity, police identity, emergency-service identity, or official service identity is used. Country-led scenes use large dimensional secular river curves, Old Town rooflines, lake bands, brick towers, dune arcs, forest strips and confluence curves instead.",
    palette,
    minimumCoverage: "Scenes 1524 and 1525 each carry hard large Lithuania motifs on three women and civilian-helicopter construction language on at least two. Scenes 1526 and 1527 use four different theme-led civilian-helicopter outfits without country map prints while Lithuania landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Vilnius, Trakai, the Curonian Spit, and Kaunas.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Lithuania images plus one accepted Namibia image",
    captionIfEligible: `Lithuania ${heartGlyph} Namibia ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-376-lithuania-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-376-lithuania-preflight.json"),
