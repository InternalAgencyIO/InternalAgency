...
import { dominicaPalette, dominicaProhibitions, dominicaSceneSpecs } from "./dominica-scene-specs.mjs";

const batch = 340;
const country = "Dominica";
const countrySlug = "dominica";
const firstScene = 1380;
const root = path.resolve("tmp/world-195x4/batch-340");
...
const commonProhibitions = dominicaProhibitions;
...
const sceneSpecs = dominicaSceneSpecs;
...
const weatherSafeText = (result, scene) => {
  ...
  if (result === "heavy snow or cinematic blizzard" && scene === 1380) return "the stored cinematic-blizzard roll appears as a dense curtain of harmless white sea mist, windblown pale frangipani petals, and cloud droplets beyond the waterfront shelter, with no snow accumulation and dry nonslip footing";
  if (result === "heavy snow or cinematic blizzard" && scene === 1382) return "the stored cinematic-blizzard roll appears as a dense line of natural white blowhole spray plumes beyond the covered overlook, with no snow accumulation and dry nonslip footing";
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Dominica location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  rollMethod: "FNV-1a over the recorded batch340-dominica keys, reduced modulo 100. Male scene selection uses the full 32-bit hash reduced modulo 4. Repeated raw emotion labels receive one stored deterministic disambiguation roll so all four visible performances remain distinct. Batch 340 also stores one hard-love roll, mascot-state roll and holder selector, and odd-prop activation, holder, and family selectors per scene.",
  rollThresholds: {
    visibleMidriff: "0-49",
    straplessDress: "0-34",
    fullyOpenBack: "0-29",
  ...
  nextQueueCountry: "Saint Kitts and Nevis",
  nextQueueBatch: 341,
  nextQueueScenes: [1384, 1385, 1386, 1387],
  nextCinematicTheme: { active: "Mars-surface expedition couture", batchOrdinalWithinTheme: 1 },
  researchSources: [
    { url: "https://legacy.discoverdominica.com/en/places/77/roseau", usedFor: "Roseau waterfront, Roseau River and Morne Bruce setting" },
    { url: "https://legacy.discoverdominica.com/en/places/68/trafalgar-falls", usedFor: "Trafalgar twin waterfalls and viewing landscape" },
    { url: "https://legacy.discoverdominica.com/en/places/72/emerald-pool", usedFor: "Emerald Pool waterfall, green basin and rainforest setting" },
    { url: "https://whc.unesco.org/en/list/814", usedFor: "Morne Trois Pitons volcanic rainforest, waterfalls and freshwater features" },
    { url: "https://legacy.discoverdominica.com/en/places/242/nature-island-dive", usedFor: "Soufriere and Scott's Head marine reserve context" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Dominica flag, coat of arms, official emblem, sacred symbol, religious building, ceremonial pattern, ceremonial attire, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular island-contour, Roseau-waterfront, Morne-Bruce, Trafalgar-waterfall, rainforest, volcanic-valley and Caribbean-current compositions instead.",
    palette,
    minimumCoverage: "Scenes 1380 and 1381 each carry hard large Dominica motifs on three women and orbital-spaceship construction language on at least two. Scenes 1382 and 1383 use four different theme-led orbital-spaceship outfits without country map prints while Dominica landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Roseau waterfront and Morne Bruce, Trafalgar Falls, Emerald Pool in Morne Trois Pitons, and Scott's Head Bay and peninsula.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Dominica images plus one accepted Andorra image",
    captionIfEligible: `Dominica ${heartGlyph} Andorra ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-340-dominica-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-340-dominica-preflight.json"),
