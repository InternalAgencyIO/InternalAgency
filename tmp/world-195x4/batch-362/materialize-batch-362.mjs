...
const countrySlug = "burundi";
const firstScene = 1468;
...
for (const spec of sceneSpecs) {
  ...
  const renderPrompt = [
    ...
    `Scene mode: ${spec.mode}. Active cinematic fashion theme: ${spec.theme}. The theme and Burundi location must both be immediate foreground reads. ${spec.culture}`,
    stored,
...
const preflight = {
  ...
  nextQueueCountry: "Bolivia",
  nextQueueBatch: 363,
  nextQueueScenes: [1472, 1473, 1474, 1475],
  nextCinematicTheme: { active: "Moon-surface expedition couture", batchOrdinalWithinTheme: 1 },
  researchSources: [
    { url: "https://www.britannica.com/place/Bujumbura", usedFor: "Bujumbura and Lake Tanganyika setting" },
    { url: "https://www.britannica.com/place/Burundi", usedFor: "Burundi highlands, lakes and forest geography" },
    { url: "https://www.britannica.com/place/Lake-Tanganyika", usedFor: "Lake Tanganyika shore and mountain landscape" },
    { url: "https://www.britannica.com/place/Gitega", usedFor: "Gitega central-highland city setting" },
  ],
  faceAnchors: {
  ...
  countryMotifPolicy: {
    flagMotifDecision: "No literal Burundi flag, coat of arms, official seal, sacred symbol, copied ceremonial or traditional pattern, political messaging, copied military identity, or official service identity is used. Country-led scenes use large dimensional secular lake currents, highland terraces, city arcs, canopy layers, tea-field ribbons and waterfall steps instead.",
    palette,
    minimumCoverage: "Scenes 1468 and 1469 each carry hard large Burundi motifs on three women and Mars-expedition construction language on at least two. Scenes 1470 and 1471 use four different theme-led Mars expedition outfits without country map prints while Burundi landmarks remain equally foregrounded.",
    cultureScene: "The scenes foreground Bujumbura and Lake Tanganyika, Gitega, Kibira forest, and Karera Falls.",
    prohibitions: commonProhibitions,
  },
  xPublishingRolls,
  xPublishingPlan: {
    minimumCurrentCountryAcceptedAssets: 2,
    attachmentShape: "two Burundi images plus one accepted Rwanda image",
    captionIfEligible: `Burundi ${heartGlyph} Rwanda ${hashtags.join(" ")}`,
...
fs.writeFileSync(path.join(root, "batch-362-burundi-preflight.json"), `${JSON.stringify(preflight, null, 2)}\n`, "utf8");

console.log(JSON.stringify({
  preflight: path.join(root, "batch-362-burundi-preflight.json"),
