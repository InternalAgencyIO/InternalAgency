...
**GO** only when all six actions are completed on devnet and each action has its own public transaction signature in the evidence packet: mint-account creation, mint initialization, test-recipient creation, test-supply minting, mint-authority revocation, and freeze-authority revocation. Completed rehearsal records must use canonical Solana Explorer URLs with `cluster=devnet`: transaction records use `/tx/<signature>?cluster=devnet` and the test-recipient record uses `/address/<address>?cluster=devnet`. A generic HTTPS link, mainnet Explorer page, reused transaction, or address-only substitute is not sufficient.
