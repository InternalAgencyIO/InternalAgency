...
Status: **HOLD**. Replace bracketed fields only after independent on-chain verification, then change this line to `Status: **VERIFIED**`. Copy the final values unchanged to the website, pinned post, and broadcast screen. Each evidence field must link to its own direct public record; do not reuse a landing page or a single URL as multiple proofs.
...
Do not use this payload if a field is pending, estimated, abbreviated, malformed, or not independently verifiable. A verified $IAT payload must state exactly `1000000000 IAT` and `1000000000000000000` base units (9 decimals). The verification timestamp must be a real UTC minute, and the verifier label must identify the reviewing role or public verifier.
