...
Status: source-only operator procedure. Mainnet remains **HOLD**. The B2 upgrade attempt is terminal under [`IAT_V2_ATTENDED_DEVNET_SIGNING_HEADROOM_INCIDENT_20260906.md`](./IAT_V2_ATTENDED_DEVNET_SIGNING_HEADROOM_INCIDENT_20260906.md); preserve its page, profile, signed-pending record, and prompt latch without broadcast, retry, refresh, discard, or storage changes. The current attended-entry gates are exactly the immutable migration artifact/evidence preflight, the fresh attended program-ceremony runtime binding at its exact clean successor `B`, and fresh finalized in-console observations of the reviewed buffer and program state. The older recovery-runtime `S`/`B` lane is immutable historical staging evidence already completed; it is not a current-HEAD gate and must not be rebased, rebound, or rerun from the fresh ceremony successor.
...
Do not copy placeholders literally. Set `$SourceS`, `$CheckoutM`, `$RunId`, and `$RunAttempt` only from the reviewed exact-head public CI run and its downloaded canonical `iat-v2-build-evidence.json`. Derive the pull-request number only from that manifest's exact `ciProvenance.workflowRef`; never reuse a historical pull-request number:
...
Program recovery classifies latch-without-pending states separately as entered/unknown, failed/unknown, or verified/missing-record. Pending-without-latch, failed-latch-with-pending, and message/signer mismatch each remain an explicit HOLD. A provider or callback failure after permanent prompt entry is never proof that the device did not sign. If signed-pending persistence succeeds but final promotion of the prompt latch cannot be proved, the coordinator makes no compensating `PROMPT_FAILED` rewrite and does not alter the pending record. Reload recovery remains possible only if it independently reads and validates an entered or verified latch and the exact matching pending bytes. These classifications never reconstruct signed bytes, prove a device or send outcome, authorize a retry, or replace finalized chain evidence.
...
Before the program-capacity/upgrade transaction prompt, the global exclusive prompt lock completes all application network preparation while the action is still unsigned and no prompt latch exists. The provider first verifies canonical Devnet Genesis and returns an opaque, internally branded, provider-bound, transaction-bound, Devnet-only, one-use prepared signing capability. The final admission check then validates the exact freshly simulated message at finalized and processed commitment, derives the processed remaining height, and verifies the same message bytes again. The whole final candidate interval, beginning before final blockhash acquisition and exact simulation and ending after admission, must finish in no more than **5,000 ms** on a visible page. Admission requires at least **100 remaining blocks**. Only then may the coordinator persist `PROMPT_ENTERED` and immediately invoke the prepared Trezor Connect method. No application RPC, blockhash refresh, simulation, timer, or storage mutation may occur between latch entry and that SDK invocation. An insufficient, stale, hidden, mismatched, or failed preparation writes no prompt latch and never calls the device; a later explicit unsigned attempt may rebuild because no prompt occurred. A prepared capability is single-use and expires after 10,000 ms; substitution, cross-provider use, reuse, or non-Devnet use is a fail-closed error. This does not clear or retry a consumed latch. The existing **40-block broadcast cutoff** is unchanged. These thresholds provide policy headroom, not a guaranteed signing duration; device, popup, or operator delay can still exhaust an admitted window. After signing, return immediately to review and use the separate enabled Broadcast control. Terminal text reports **LAST OBSERVED** blocks and **COUNTDOWN STOPPED**, or observation unavailable; it is not a continuing live countdown. A terminal signed attempt must never be sent or re-signed.
...
1. Press **CONNECT 7XZ MODEL T DIRECTLY + SIMULATE + SIGN** once. The same activation performs final blockhash acquisition, exact simulation, prepared canonical-Devnet verification, and the 100-block admission check before consuming the prompt latch. If unsigned preparation fails, review the error and start a later explicit unsigned attempt only after confirming that no prompt latch was written and no device request occurred.
