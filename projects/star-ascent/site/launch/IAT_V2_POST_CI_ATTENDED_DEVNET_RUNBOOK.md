...
Before the program-capacity/upgrade transaction prompt, the global exclusive prompt lock performs an unsigned read-only admission check on the exact freshly simulated message. It requires at least **80 remaining blocks**, a visible page at observation start and completion, and a finite monotonic observation duration no greater than **5,000 ms**. Finalized and processed validity plus the processed height must agree with the exact blockhash, last-valid height, and simulation minimum context. The same message bytes are checked again immediately after the observation, before irreversible prompt entry. An insufficient, stale, hidden, or failed preparation writes no prompt latch and never calls the device; a later explicit unsigned attempt may rebuild because no prompt occurred. This does not clear or retry a consumed latch. The existing **40-block broadcast cutoff** is unchanged. These thresholds provide policy headroom, not a guaranteed signing duration; the RPC observation can wait on network availability, and device/popup delays can still exhaust an admitted window. After signing, return immediately to review and use the separate enabled Broadcast control. Terminal text reports **LAST OBSERVED** blocks and **COUNTDOWN STOPPED**, or observation unavailable; it is not a continuing live countdown. A terminal signed attempt must never be sent or re-signed.
