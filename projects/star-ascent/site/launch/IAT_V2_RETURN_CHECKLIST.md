...
- Feature evidence status remains
  ...
  review is retained only as historical evidence; the hardened source still
  requires a fresh signed Devnet run and source-bound automated comparison.
...
1. Reproduce the current SBF from the bound source and compare SHA-256
...
2. Run the complete signed Devnet rehearsal against that exact binary and
   obtain automated source/binary/account/receipt/state comparison evidence.
...
8. Automatically validate the regenerated packet and local host-program
...
11. Before the first stage, validate the source-bound
   ...
   confirmed stage, stop for automated post-state reconciliation. The first
...
- Ceremony entry now fails closed on
  ...
  source-bound automated final-code evidence are mandatory.
- Regeneration, automated-evidence, and Model T path-review summary booleans
  ...
  they must be `READY` and `ARMED`, respectively. The automated evidence lane
  must record artifact and stage-plan observations without signing authority, and the
