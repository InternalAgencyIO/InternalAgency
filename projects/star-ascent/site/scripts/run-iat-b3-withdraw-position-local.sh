...
schema="iat-b3-withdraw-position-production-executor-loopback/v1"
script_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)
site_dir=$(cd -- "$script_dir/.." && pwd -P)
fixture_dir="$site_dir/tests/fixtures/iat-b3-withdraw-position"
driver="$script_dir/iat-b3-withdraw-position-local-driver.mjs"
evidence="$site_dir/docs/b3/evidence/local-validator-withdraw-position-production-executor-20260812.json"
evidence_dir=$(dirname -- "$evidence")
deploy_dir="$fixture_dir/target/deploy"
economy_artifact="$deploy_dir/iat_b3_withdraw_position_rehearsal_economy.so"
law_artifact="$deploy_dir/iat_b3_withdraw_position_rehearsal_law_hook.so"
economy_generated_keypair="$deploy_dir/iat_b3_withdraw_position_rehearsal_economy-keypair.json"
law_generated_keypair="$deploy_dir/iat_b3_withdraw_position_rehearsal_law_hook-keypair.json"
...
rpc_port="${IAT_B3_WITHDRAW_POSITION_RPC_PORT:-19499}"
...
mkdir -p -- "$deploy_dir"
temp_dir=$(mktemp -d /tmp/iat-b3-withdraw-position.XXXXXX)
temp_dir=$(cd -- "$temp_dir" && pwd -P)
case "$temp_dir" in
  /tmp/iat-b3-withdraw-position.*) ;;
  *)
...
cleanup() {
  ...
  case "$temp_dir" in
    /tmp/iat-b3-withdraw-position.*) rm -rf -- "$temp_dir" ;;
    *) cleanup_status=1 ;;
  esac
...
runner_sha256=$(sha256sum "$script_dir/run-iat-b3-withdraw-position-local.sh" | awk '{print $1}')
