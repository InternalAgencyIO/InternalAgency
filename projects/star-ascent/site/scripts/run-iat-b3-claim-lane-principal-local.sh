...
schema="iat-b3-claim-lane-principal-production-executor-loopback/v1"
script_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)
site_dir=$(cd -- "$script_dir/.." && pwd -P)
fixture_dir="$site_dir/tests/fixtures/iat-b3-claim-lane-principal"
driver="$script_dir/iat-b3-claim-lane-principal-local-driver.mjs"
evidence="$site_dir/docs/b3/evidence/local-validator-claim-lane-principal-production-executor-20260813.json"
evidence_dir=$(dirname -- "$evidence")
deploy_dir="$fixture_dir/target/deploy"
economy_artifact="$deploy_dir/iat_b3_claim_lane_principal_rehearsal_economy.so"
law_artifact="$deploy_dir/iat_b3_claim_lane_principal_rehearsal_law_hook.so"
economy_generated_keypair="$deploy_dir/iat_b3_claim_lane_principal_rehearsal_economy-keypair.json"
law_generated_keypair="$deploy_dir/iat_b3_claim_lane_principal_rehearsal_law_hook-keypair.json"
...
rpc_port="${IAT_B3_CLAIM_LANE_PRINCIPAL_RPC_PORT:-19499}"
...
mkdir -p -- "$deploy_dir"
temp_dir=$(mktemp -d /tmp/iat-b3-claim-lane-principal.XXXXXX)
temp_dir=$(cd -- "$temp_dir" && pwd -P)
case "$temp_dir" in
  /tmp/iat-b3-claim-lane-principal.*) ;;
  *)
...
cleanup() {
  ...
  case "$temp_dir" in
    /tmp/iat-b3-claim-lane-principal.*) rm -rf -- "$temp_dir" ;;
    *) cleanup_status=1 ;;
  esac
...
for name in sponsor owner mint source treasury-destination ecosystem-destination core-destination liquidity-destination; do
  solana-keygen new \
    --no-bip39-passphrase \
    --silent \
    --force \
...
treasury_destination_pubkey=$(solana-keygen pubkey "$temp_dir/treasury-destination.json")
ecosystem_destination_pubkey=$(solana-keygen pubkey "$temp_dir/ecosystem-destination.json")
core_destination_pubkey=$(solana-keygen pubkey "$temp_dir/core-destination.json")
liquidity_destination_pubkey=$(solana-keygen pubkey "$temp_dir/liquidity-destination.json")
treasury_beneficiary="CucS4oym18YjEMUmXYVx45q6HUGhW35wE3qpwkcnSCFQ"
ecosystem_beneficiary="HypAfe9RwaBRnZeLpqvYU1rBbAwHTSBnm24enRL6Qx18"
core_beneficiary="2yBK1NkeUoTToE4cfz33WRckho4Qr2BV1ZtCTrw3AHyB"
liquidity_beneficiary="2d41i3afUpWuo2LqpuKao5D1ToEU88aBokiQ3z8HQtPC"
...
for pair in \
  "treasury-destination:$treasury_beneficiary" \
  "ecosystem-destination:$ecosystem_beneficiary" \
  "core-destination:$core_beneficiary" \
  "liquidity-destination:$liquidity_beneficiary"; do
  name=${pair%%:*}
  beneficiary=${pair#*:}
  spl-token create-account \
    --url "$rpc_url" \
    ...
    --owner "$beneficiary" \
    --program-2022 \
    "$mint_pubkey" "$temp_dir/$name.json" >/dev/null
done
spl-token mint \
  --url "$rpc_url" \
  --fee-payer "$temp_dir/sponsor.json" \
...
"$node_bin" "$(to_node_path "$driver")" \
  ...
  --treasury-destination "$treasury_destination_pubkey" \
  --ecosystem-destination "$ecosystem_destination_pubkey" \
  --core-destination "$core_destination_pubkey" \
  --liquidity-destination "$liquidity_destination_pubkey" \
...
runner_sha256=$(sha256sum "$script_dir/run-iat-b3-claim-lane-principal-local.sh" | awk '{print $1}')
