...
schema="iat-b3-open-position-production-executor-loopback/v1"
script_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)
site_dir=$(cd -- "$script_dir/.." && pwd -P)
fixture_dir="$site_dir/tests/fixtures/iat-b3-open-position"
driver="$script_dir/iat-b3-open-position-local-driver.mjs"
evidence="$site_dir/docs/b3/evidence/local-validator-open-position-production-executor-20260811.json"
deploy_dir="$fixture_dir/target/deploy"
economy_artifact="$deploy_dir/iat_b3_open_position_rehearsal_economy.so"
law_artifact="$deploy_dir/iat_b3_open_position_rehearsal_law_hook.so"
economy_generated_keypair="$deploy_dir/iat_b3_open_position_rehearsal_economy-keypair.json"
law_generated_keypair="$deploy_dir/iat_b3_open_position_rehearsal_law_hook-keypair.json"
...
rpc_port="${IAT_B3_OPEN_POSITION_RPC_PORT:-19499}"
rpc_url="http://127.0.0.1:${rpc_port}"
faucet_port=$((rpc_port - 1))
dynamic_min=$((rpc_port + 2))
dynamic_max=$((rpc_port + 102))

for command_name in cargo rustc solana solana-keygen solana-test-validator spl-token sha256sum; do
...
mkdir -p -- "$deploy_dir"
temp_dir=$(mktemp -d /tmp/iat-b3-open-position.XXXXXX)
temp_dir=$(cd -- "$temp_dir" && pwd -P)
case "$temp_dir" in
  /tmp/iat-b3-open-position.*) ;;
  *)
...
cleanup() {
  ...
  case "$temp_dir" in
    /tmp/iat-b3-open-position.*) rm -rf -- "$temp_dir" ;;
    *) cleanup_status=1 ;;
  esac
...
for name in sponsor owner mint source prior-delegate; do
  solana-keygen new \
    --no-bip39-passphrase \
    --silent \
    --force \
...
runner_sha256=$(sha256sum "$script_dir/run-iat-b3-open-position-local.sh" | awk '{print $1}')
