...
[[ "${IAT_V2_CLEAN_ENVIRONMENT:-}" == "iat-v2-devnet-buffer-in-place-continuation-from-54720-v1" ]] \
  || hold "use the exact clean Ubuntu-24.04 WSL2 continuation launcher from the attended runbook"
...
CONTINUATION_RUNTIME_BINDING_POLICY='REQUIRE_EXACT_PUBLIC_CI_SOURCE_AND_BINDING_SUCCESSOR'
[[ "$CONTINUATION_RUNTIME_BINDING_POLICY" == 'REQUIRE_EXACT_PUBLIC_CI_SOURCE_AND_BINDING_SUCCESSOR' ]] \
  || hold "in-place continuation runtime-binding policy drifted"
...
EXPECTED_PARTIAL_HASH="c8b842bae57c2f23da0de4219ab879147971a0dafeda8755f6a90e8ca5db0dd3"
EXPECTED_PREFIX_BYTES="54720"
EXPECTED_REMAINING_BYTES="594960"
MIN_FINALIZED_SLOT="490807312"
BASELINE_ACCOUNT_SLOT="490807312"
BASELINE_EVIDENCE_BODY_SHA256="bceff73e737dee68f812e7d73c3554d30e08b899ca723e08a798b2275609f429"
...
RECOVERY_ROOT="$RECOVERY_PARENT/devnet-buffer-in-place-continuation-from-54720-v1"
ATTEMPT_DIR="$RECOVERY_ROOT/attempt-one-use"
CONFIRMATION_PHRASE="AUTHORIZE-DEVNET-IN-PLACE-BUFFER-CONTINUATION-564XrjVAyqXrChSe9sDJ68XFtNL7tVVLYdwFc9mh1GHH-FROM-54720-OF-649680-CURRENT-c8b842bae57c2f23da0de4219ab879147971a0dafeda8755f6a90e8ca5db0dd3-TARGET-771c87bcd9afacf7e8e6bf43cd7ba05915fceb11c45a6a89d8080f6b52778a01"
...
verify_committed_source() {
  ...
  for specification in 'scripts/continue-iat-v2-devnet-buffer-in-place-from-54720.sh 100755' 'scripts/reconcile-iat-v2-devnet-buffer-finalized.mjs 100644'; do
...
validate_partial() {
  ...
  ' -- "$BUFFER_ADDRESS" "$EXPECTED_AUTHORITY" "$EXPECTED_OWNER" "$EXPECTED_GENESIS" "$EXPECTED_BUFFER_LAMPORTS" "$EXPECTED_ACCOUNT_BYTES" "$EXPECTED_METADATA_BYTES" "$EXPECTED_PROGRAM_BYTES" "$EXPECTED_PARTIAL_HASH" "$EXPECTED_PREFIX_BYTES" "$EXPECTED_REMAINING_BYTES" "$MIN_FINALIZED_SLOT" "$BASELINE_ACCOUNT_SLOT" "$TARGET_ARTIFACT_SHA256"
}
...
capture_post_reconciliation() {
  "$NODE_BIN" --input-type=module -e '
    ...
    writeFileSync(resultPath,`${JSON.stringify({schema:"iat-v2-devnet-buffer-in-place-continuation-poststate-capture/v1",exitStatus:result.status,signal:result.signal,error:result.error?.message??null})}\n`,{flag:"wx",mode:0o600,flush:true});
...
write_manifest() {
  ...
  "$NODE_BIN" --input-type=module -e '
    import { writeFileSync } from "node:fs";
    const a=process.argv.slice(1); if (a.length!==14) throw new Error("manifest argument map drifted");
    const [path,buffer,authority,currentHash,prefix,targetHash,targetBytes,baselineMinSlot,baselineAccountSlot,baselineEvidenceHash,minSlot,accountSlot,prestateHash,payerLamports]=a;
    const record={schema:"iat-v2-devnet-buffer-in-place-continuation-reservation/v1",phase:"RESERVED_BEFORE_SOLE_WRITE_INVOCATION",network:"devnet",mainnetStatus:"HOLD",bufferAddress:buffer,expectedAuthority:authority,protectedBufferKeypairUsed:false,newBufferCreated:false,currentProgramSha256:currentHash,matchingPrefixBytes:Number(prefix),targetArtifactSha256:targetHash,targetArtifactBytes:Number(targetBytes),reviewedBaselineFinalizedPrestate:{minContextSlot:Number(baselineMinSlot),accountContextSlot:Number(baselineAccountSlot),evidenceBodySha256:baselineEvidenceHash,programSha256:currentHash,matchingPrefixBytes:Number(prefix)},finalizedPrestate:{minContextSlot:Number(minSlot),accountContextSlot:Number(accountSlot),evidenceBodySha256:prestateHash},payerLamports,mutationPolicy:{cliInvocations:1,maxSignAttempts:20,timeoutSeconds:1500,transport:"QUIC_TPU",doNotRerunAfterAmbiguity:true},authorityHandoff:false,deployment:false};
    writeFileSync(path, `${JSON.stringify(record,null,2)}\n`, {flag:"wx",mode:0o600,flush:true});
  ' -- "$destination" "$BUFFER_ADDRESS" "$EXPECTED_AUTHORITY" "$EXPECTED_PARTIAL_HASH" "$EXPECTED_PREFIX_BYTES" "$TARGET_ARTIFACT_SHA256" "$TARGET_ARTIFACT_BYTES" "$MIN_FINALIZED_SLOT" "$BASELINE_ACCOUNT_SLOT" "$BASELINE_EVIDENCE_BODY_SHA256" "$min_slot" "$account_slot" "$prestate_hash" "$payer_lamports" \
    || hold "one-use continuation manifest could not be written"
}
...
cleanup() {
  ...
  if [[ "${BOUNDARY_ENTERED:-false}" == "true" ]]; then
    echo "HOLD: the one-use continuation mutation boundary was entered. DO NOT RERUN; use signer-free finalized reconciliation only." >&2
  fi
}
trap cleanup EXIT
...
echo "DEVNET ONLY // EXISTING BUFFER IN-PLACE CONTINUATION FROM 54,720"
...
echo "RETRY DISCLOSURE: --max-sign-attempts 20 may re-sign and resend remaining Devnet chunk transactions for up to the 1,500-second process timeout."
echo "TRANSPORT: explicit QUIC TPU submission; finalized reconciliation remains signer-free and RPC-observed."
...
echo "Any timeout, crash, error, partial result, or unclear output after the boundary permanently consumes this continuation CAS."
...
[[ -z "$(/usr/bin/find "$RECOVERY_ROOT" -mindepth 1 -maxdepth 1 -print -quit)" ]] || hold "in-place continuation root is not empty; preserve it for review"
...
set +e
write_output="$(iat_v2_run_keyless_solana_timeout 1500 "$SOLANA_BIN" program write-buffer /proc/self/fd/11 --buffer 564XrjVAyqXrChSe9sDJ68XFtNL7tVVLYdwFc9mh1GHH --buffer-authority /proc/self/fd/9 --fee-payer /proc/self/fd/9 --keypair /proc/self/fd/9 --url devnet --commitment finalized --use-quic --with-compute-unit-price 1000 --max-sign-attempts 20 2>&1)"
write_status=$?
set -e
exec 9<&- 11<&-
printf '%s\n' "$write_output" | write_exclusive_stdin "$ATTEMPT_DIR/write-output.txt"
printf '{"schema":"iat-v2-devnet-buffer-in-place-continuation-write-result/v1","exitStatus":%s,"doNotRerun":true}\n' "$write_status" | write_exclusive_stdin "$ATTEMPT_DIR/write-result.json"
...
post_status="$("$NODE_BIN" --input-type=module -e '
  import { closeSync, constants, fstatSync, lstatSync, openSync, readFileSync } from "node:fs";
  const p=process.argv[1], before=lstatSync(p,{bigint:true});
  ...
  try { const opened=fstatSync(fd,{bigint:true}); if(opened.dev!==before.dev||opened.ino!==before.ino) throw new Error("poststate result changed");
    ...
    if(v.schema!=="iat-v2-devnet-buffer-in-place-continuation-poststate-capture/v1"||![0,2].includes(v.exitStatus)||v.signal!==null||v.error!==null) throw new Error("poststate command failed");
    process.stdout.write(`${v.exitStatus}`);
  } finally { closeSync(fd); }
' -- "$ATTEMPT_DIR/poststate-result.json")" || hold "post-write finalized reconciliation process failed; DO NOT RERUN"
...
post_classification="$(printf '%s' "$post_record" | "$NODE_BIN" --input-type=module -e '
  import { createHash } from "node:crypto";
  const chunks=[]; for await (const c of process.stdin) chunks.push(c); const v=JSON.parse(Buffer.concat(chunks));
  const a=process.argv.slice(1); if(a.length!==22) throw new Error("poststate argument map drifted");
  const [buffer,authority,owner,genesis,lamports,accountBytes,metadataBytes,target,bytes,minSlot,baselineAccountSlot,preMinSlot,preAccountSlot,latestMinSlot,latestAccountSlot,sourceHead,ciRunId,evidenceHash,rpc,baselinePrefix,baselineHash,exitStatus]=a;
  ...
  const monotonic=Number.isSafeInteger(v.minContextSlot) && Number.isSafeInteger(v.accountContextSlot)
    ...
    && v.accountContextSlot >= v.minContextSlot && v.accountContextSlot >= Number(baselineAccountSlot) && v.accountContextSlot >= Number(preAccountSlot)
  ...
  const common=v.schema==="iat-v2-devnet-buffer-finalized-reconciliation/v1"
    ...
    && boundary.protectedRecoveryStateRead===false;
  const exact=common && Number(exitStatus)===0 && v.status==="EXACT_FINALIZED_BUFFER"
    && account.programSha256===target
    ...
    && boundary.next==="SEPARATE_ATTENDED_ACTION_REVIEW_REQUIRED";
  const prefix=comparison?.matchingPrefixBytes;
  const partial=common && Number(exitStatus)===2 && v.status==="HOLD_PARTIAL_EXACT_PREFIX_ZERO_TAIL"
    && /^[0-9a-f]{64}$/.test(account.programSha256) && account.programSha256===comparison?.observedProgramSha256
    && comparison.classification==="PARTIAL_EXACT_PREFIX_ZERO_TAIL" && comparison.exact===false
    && Number.isSafeInteger(prefix) && prefix>=Number(baselinePrefix) && prefix<Number(bytes)
    && (prefix>Number(baselinePrefix) || account.programSha256===baselineHash)
    && comparison.expectedRemainingBytes===Number(bytes)-prefix && comparison.observedSuffixIsZero===true
    && Number.isSafeInteger(comparison.trailingZeroBytes) && comparison.trailingZeroBytes>=comparison.expectedRemainingBytes && comparison.trailingZeroBytes<=Number(bytes)
    && comparison.firstMismatchOffset===prefix && Number.isSafeInteger(comparison.firstMismatchExpectedByte)
    && comparison.firstMismatchExpectedByte>=1 && comparison.firstMismatchExpectedByte<=255 && comparison.firstMismatchObservedByte===0
    && `${comparison.observedProgramBytes}`===bytes
    && validation?.authorityAdmitted===true && validation.authorityMatchesExpected===true && validation.sizeMatches===true
    && validation.hashMatches===false && validation.exact===false && validation.partialExactPrefixZeroTail===true
    && JSON.stringify(validation.holdReasons)==="[\"PROGRAM_SHA256_MISMATCH\"]"
    && boundary.next==="PRESERVE_EXISTING_ADDRESS_AND_DO_NOT_RESUBMIT";
  const rootKeys=["schema","status","network","rpc","genesisHash","commitment","minContextSlot","accountContextSlot","observedAtUtc","bufferAddress","expectedAuthority","observedAuthority","observedAuthorityRole","account","publicCiArtifact","comparison","validation","boundary","evidenceBodySha256","evidenceFile"];
  const shapes=v.evidenceFile===null && exactKeys(v,rootKeys)
  ...
  const {evidenceBodySha256,evidenceFile,...body}=v;
  const recomputed=createHash("sha256").update(Buffer.from(`${JSON.stringify(body,null,2)}\n`,"utf8")).digest("hex");
  if(!shapes || !/^[0-9a-f]{64}$/.test(evidenceBodySha256) || recomputed!==evidenceBodySha256 || (!exact&&!partial)) throw new Error("poststate is not an admitted monotonic finalized outcome");
  process.stdout.write(exact?"EXACT":"PARTIAL");
 ' -- "$BUFFER_ADDRESS" "$EXPECTED_AUTHORITY" "$EXPECTED_OWNER" "$EXPECTED_GENESIS" "$EXPECTED_BUFFER_LAMPORTS" "$EXPECTED_ACCOUNT_BYTES" "$EXPECTED_METADATA_BYTES" "$TARGET_ARTIFACT_SHA256" "$TARGET_ARTIFACT_BYTES" "$MIN_FINALIZED_SLOT" "$BASELINE_ACCOUNT_SLOT" "$MUTATION_PRESTATE_MIN_SLOT" "$MUTATION_PRESTATE_ACCOUNT_SLOT" "${PARTIAL_FIELDS[0]}" "${PARTIAL_FIELDS[1]}" "$TARGET_SOURCE_HEAD" "$TARGET_CI_RUN_ID" "$TARGET_EVIDENCE_SHA256" 'https://api.devnet.solana.com' "$EXPECTED_PREFIX_BYTES" "$EXPECTED_PARTIAL_HASH" "$post_status")" \
  || hold "post-write finalized outcome validation failed; DO NOT RERUN"

if [[ "$post_classification" == "PARTIAL" ]]; then
  hold "post-write finalized state is a validated monotonic partial; continuation CAS is consumed; DO NOT RERUN"
fi
[[ "$post_classification" == "EXACT" ]] || hold "post-write finalized outcome classification drifted; DO NOT RERUN"

BOUNDARY_ENTERED=false
trap - EXIT
echo "EXACT FINALIZED DEVNET BUFFER CONTINUATION VERIFIED"
echo "BUFFER: $BUFFER_ADDRESS"
