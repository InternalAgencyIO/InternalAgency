...
const path = "scripts/continue-iat-v2-devnet-buffer-in-place-from-35520.sh";
const source = readFileSync(path, "utf8").replaceAll("\r\n", "\n");
const incidentPath = "launch/IAT_V2_DEVNET_BUFFER_IN_PLACE_CONTINUATION_INCIDENT_20260831.md";
...
const partial = "72f835371ef4a29710d3976683c8449c2db5a56a06311ead0d439abae53d4398";
const target = "771c87bcd9afacf7e8e6bf43cd7ba05915fceb11c45a6a89d8080f6b52778a01";
const phrase = `AUTHORIZE-DEVNET-IN-PLACE-BUFFER-CONTINUATION-${buffer}-FROM-35520-OF-649680-CURRENT-${partial}-TARGET-${target}`;
...
function poststateFixture({
  kind = "exact",
  minContextSlot = 490776148,
  accountContextSlot = 490776148,
  matchingPrefixBytes = kind === "exact" ? 649680 : 36480,
...
test("continuation incident and runtime closure preserve the consumed lane and exact new baseline", () => {
  for (const exact of [
    ...
    partial,
    target,
    "payload bytes `[0, 35520)`",
    "payload bytes `[35520, 649680)`",
    "614,160",
    "7b89459c06b3ee6768d7862c8cb14e06730f2a920aa1ac90b68b7b93277f03ee",
    "17 finalized entries with `err: null` in slot",
    "490771674",
  ]) assert.ok(incident.includes(exact), "incident must retain exact continuation evidence: " + exact);
  assert.match(incident, /does not[\s\S]*prove that those entries are[\s\S]*complete causal set/u);
  assert.match(incident, /must never be rerun/u);
  assert.match(incident, /No device prompt, protected-state read, signature,[\s\S]*broadcast, or mutation occurred/u);
...
test("exact partial prestate, finalized slot floor, and target are all pinned", () => {
  for (const value of [
    ...
    "4522976880", "649717", "649680", "35520", "614160", "490776148",
    "7b89459c06b3ee6768d7862c8cb14e06730f2a920aa1ac90b68b7b93277f03ee",
    "ca19c4ebec300031528014e3d3373889a7b171589158ba366536e6200a3ac2a9",
...
test("real Bash evaluation preserves the exact partial-state hold reason string", () => {
  ...
  const record = {
    ...
    comparison: {
      classification: "PARTIAL_EXACT_PREFIX_ZERO_TAIL",
      exact: false,
      matchingPrefixBytes: 35520,
      expectedRemainingBytes: 614160,
      observedSuffixIsZero: true,
      firstMismatchOffset: 35520,
      firstMismatchObservedByte: 0,
    },
    validation: {
...
`;
  ...
  assert.equal(result.stdout, `490776148\n490776148\n${"0".repeat(64)}\n`);
});
...
test("continuation never reads the consumed CAS or protected buffer signer and has no adjacent mutation", () => {
  ...
  assert.doesNotMatch(source, /devnet-buffer-rebuild-v1|devnet-buffer-in-place-recovery-v1|program (?:close|deploy|set-buffer-authority)|set-buffer-authority|program upgrade|--url (?:mainnet|mainnet-beta)/u);
...
test("separate persistent CAS is durable before the sole boundary and never cleaned", () => {
  assert.match(source, /devnet-buffer-in-place-continuation-from-35520-v1/u);
...
test("real poststate validator pairs exit status with exact or consumed-partial records", () => {
  ...
  const { evidenceBodySha256: _discarded, ...divergentBody } = progressed;
  divergentBody.comparison = {
    ...divergentBody.comparison,
    classification: "DIVERGENT_BYTES",
    observedSuffixIsZero: false,
  };
  assert.notEqual(runPoststateValidator(sealPoststate(divergentBody), 2).status, 0);

  const { evidenceBodySha256: _ignored, ...extraKeyBody } = exact;
  extraKeyBody.unreviewed = true;
  assert.notEqual(runPoststateValidator(sealPoststate(extraKeyBody), 0).status, 0);
});
