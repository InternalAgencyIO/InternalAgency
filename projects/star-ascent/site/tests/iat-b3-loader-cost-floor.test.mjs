...
const B3_LAW_OPTIMIZED_ELF_BYTES = 154_952n;
...
test("optimistic loader-v4 rent still puts retained V2 plus B3 law above 3 SOL", () => {
  ...
  assert.equal(law, 1_079_690_880n);
  assert.equal(aggregate, 4_732_632_960n);
...
test("the cost document states the measured lower bound and refuses feature gutting", () => {
  ...
  assert.match(document, /B3 Daily Law \(`154,952` bytes\): `1\.07969088 SOL`/u);
  assert.match(document, /both binaries alone: `4\.73263296 SOL`/u);
