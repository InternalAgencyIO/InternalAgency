...
fn execute_hook(program_id: &Pubkey, accounts: &[AccountInfo<'_>], _amount: u64) -> ProgramResult {
    let account_iter = &mut accounts.iter();
    ...
    let funding_fixture =
        destination.key == &stake_vault && authority.key == &source_state.base.owner;
    let production_withdrawal =
        source.key == &stake_vault && !authority.is_signer && authority.key == &vault_authority;
    if !funding_fixture && !production_withdrawal {
    ...
    let destination_state = StateWithExtensions::<TokenAccount>::unpack(&destination_data)
        .map_err(|_| FixtureLawError::InvalidAccount)?;
    if production_withdrawal && destination_state.base.delegate.is_some() {
