...
use iat_b3_economy::production_withdraw_position_executor::{
    execute_runtime_production_withdraw_position_account_infos,
    ProductionWithdrawPositionExecutorError,
};
...
use iat_b3_economy::{
    ...
    encode_position_state, initialize_config, initialize_lane_vault, initialize_stake_vault,
    set_eligibility, ActivateInput, CanonicalDailyLawBinding, ConfigGenesisState, EconomyError,
    GenesisPhase, InitializeConfigInput, InitializeLaneVaultInput, InitializeStakeVaultInput,
    LaneState, PositionState, ReadonlyMintState, ReadonlyTokenState, SetEligibilityInput,
    COMMUNITY_CUSTODY, CONFIG_GENESIS_ACCOUNT_LEN, CORE_TEAM, ECOSYSTEM, ELIGIBILITY_ACCOUNT_LEN,
    LANE_ACCOUNT_LEN, LIQUIDITY, MAINNET_SUPPLY, ON_DEMAND_MAINNET_PID, POSITION_ACCOUNT_LEN,
    PROGRAM_ADMIN, SECONDS_PER_WEEK, TOKEN_DECIMALS, TREASURY, USER_TERM_WEEKS,
};
...
const WITHDRAW_PRINCIPAL: u64 = 17;
const WITHDRAW_POSITION_ID: u64 = 7;
...
pub fn process_instruction(
    ...
    match data[0] {
        ...
        2 if data.len() == 1 => seed_withdraw_position(program_id, accounts),
        3 if data.len() == 34 && data[1] <= 1 => {
            execute_real_withdraw_position(program_id, accounts, data[1], &data[2..34])
        }
...
#[inline(never)]
fn execute_real_withdraw_position(
    program_id: &Pubkey,
    accounts: &[AccountInfo<'_>],
    mode: u8,
...
#[inline(never)]
fn seed_withdraw_position(program_id: &Pubkey, accounts: &[AccountInfo<'_>]) -> ProgramResult {
    if accounts.len() != 6 {
        return Err(FixtureEconomyError::InvalidAccount.into());
    }
    let payer = &accounts[0];
    let config_account = &accounts[1];
    let position_account = &accounts[2];
    let mint = &accounts[3];
    let system = &accounts[4];
    let law_state = &accounts[5];
    if !payer.is_signer || !payer.is_writable || mint.owner != &TOKEN_2022_PROGRAM_ID {
        return Err(FixtureEconomyError::InvalidAccount.into());
    }
    let binding = NativeEconomyBinding::new(program_id.to_bytes(), mint.key.to_bytes())
    ...
    let gate = verify_daily_law_open_account_info(&law_binding(mint.key), law_state)
        .map_err(map_runtime_law_error)?;
    let mut prepared = prepare_seed_state(&binding, &gate, payer.key, mint)?;
    prepared.config.config.staked_principal = WITHDRAW_PRINCIPAL;
    prepared.config.config.genesis_timestamp = gate.unix_timestamp() - 55 * SECONDS_PER_WEEK;
    persist_config(
        program_id,
        payer,
        mint,
    ...
    let identity = PdaIdentity::Position {
        ...
        position_id: WITHDRAW_POSITION_ID,
    };
    let derived = derive_pda(&binding, identity).map_err(|_| FixtureEconomyError::SeedRejected)?;
    require_vacant_target(position_account, derived.key)?;
    ...
    create_and_write(
        ...
        &[
            ...
            payer.key.as_ref(),
            &WITHDRAW_POSITION_ID.to_le_bytes(),
            &[derived.bump],
        ],
        &encoded,
    )
...
fn map_executor_error(error: ProductionWithdrawPositionExecutorError) -> ProgramError {
    match error {
        ProductionWithdrawPositionExecutorError::Program(program) => program,
