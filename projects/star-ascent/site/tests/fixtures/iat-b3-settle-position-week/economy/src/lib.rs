...
use iat_b3_economy::production_settle_position_week_executor::{
...
use iat_b3_economy::{
    activate, encode_config_genesis_state, encode_lane_state, encode_position_state,
    ...
    PositionState, ReadonlyMintState, ReadonlyTokenState, COMMUNITY_CUSTODY,
    CONFIG_GENESIS_ACCOUNT_LEN, CORE_TEAM, ECOSYSTEM, LANE_ACCOUNT_LEN, LIQUIDITY, MAINNET_SUPPLY,
    ON_DEMAND_MAINNET_PID, POSITION_ACCOUNT_LEN, PROGRAM_ADMIN, SECONDS_PER_WEEK,
    STANDARD_RATE_BPS, TOKEN_DECIMALS, TREASURY, USER_TERM_WEEKS,
};
...
const MAIN_POSITION_ID: u64 = 7;
const ZERO_SKIP_POSITION_ID: u64 = 8;
const POSITION_PRINCIPAL: u64 = 5_200_000;
const SETTLEMENT_WEEK: u64 = 4;
const MAIN_RESERVATIONS: [u64; 3] = [6_000, 3_000, 2_000];
const ZERO_SKIP_RESERVATIONS: [u64; 3] = [0, 11_000, 0];
...
pub fn process_instruction(
    ...
    match data[0] {
        ...
        2 if data.len() == 34 && data[1] <= 1 => {
            execute_real_settle_position_week(program_id, accounts, data[1], &data[2..34])
        }
...
#[inline(never)]
fn execute_real_settle_position_week(
    program_id: &Pubkey,
    accounts: &[AccountInfo<'_>],
    mode: u8,
...
fn seed_rehearsal_state(program_id: &Pubkey, accounts: &[AccountInfo<'_>]) -> ProgramResult {
    ...
    let liquidity_account = &accounts[5];
    let main_position_account = &accounts[6];
    let zero_skip_position_account = &accounts[7];
    let system = &accounts[8];
    let law_state = &accounts[9];
    if !payer.is_signer
        || !payer.is_writable
    ...
    let gate = verify_daily_law_open_account_info(&law_binding(mint.key), law_state)
        .map_err(map_runtime_law_error)?;
    let mut prepared = prepare_seed_state(&binding, &gate, mint)?;
    prepared.config.config.genesis_timestamp = gate
        .unix_timestamp()
        .checked_sub(SETTLEMENT_WEEK as i64 * SECONDS_PER_WEEK)
        .ok_or(FixtureEconomyError::SeedRejected)?;
    prepared.config.config.staked_principal = POSITION_PRINCIPAL
        .checked_mul(2)
        .ok_or(FixtureEconomyError::SeedRejected)?;
    prepared.treasury.reserved = MAIN_RESERVATIONS[0] + ZERO_SKIP_RESERVATIONS[0];
    prepared.ecosystem.reserved = MAIN_RESERVATIONS[1] + ZERO_SKIP_RESERVATIONS[1];
    prepared.liquidity.reserved = MAIN_RESERVATIONS[2] + ZERO_SKIP_RESERVATIONS[2];
    persist_config(
        program_id,
        payer,
        mint,
    ...
    persist_position(
        program_id,
        payer,
        zero_skip_position_account,
        system,
        &binding,
        ZERO_SKIP_POSITION_ID,
        ZERO_SKIP_RESERVATIONS,
    )
}
...
fn map_executor_error(error: ProductionSettlePositionStandardExecutorError) -> ProgramError {
    match error {
        ProductionSettlePositionStandardExecutorError::Program(program) => program,
...
fn initialize_lane_token(
    ...
    let binding = NativeEconomyBinding::new(program_id.to_bytes(), mint.key.to_bytes())
        .map_err(|_| FixtureEconomyError::InvalidAccount)?;
    if !matches!(lane, TREASURY | ECOSYSTEM | LIQUIDITY) {
        return Err(FixtureEconomyError::InvalidInstruction.into());
    }
...
fn persist_position<'info>(
    ...
    let state = PositionState {
        ...
        accepted_week: SETTLEMENT_WEEK - 1,
        first_accrual_week: SETTLEMENT_WEEK,
    ...
    encode_position_state(&state, &mut encoded).map_err(|_| FixtureEconomyError::SeedRejected)?;
    create_and_write(
        program_id,
        payer,
        target,
...
fn prepare_seed_state(
    ...
    Ok(finalize_seed_state(activation))
}
...
#[inline(never)]
fn finalize_seed_state(activation: Box<iat_b3_economy::ActivateResult>) -> Box<PreparedSeedState> {
