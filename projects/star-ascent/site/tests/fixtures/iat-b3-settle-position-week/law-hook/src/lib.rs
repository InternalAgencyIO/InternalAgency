...
fn execute_hook(program_id: &Pubkey, accounts: &[AccountInfo<'_>], amount: u64) -> ProgramResult {
    ...
    for (ordinal, lane) in [1u8, 2, 4].into_iter().enumerate() {
        ...
        if source.key == &lane_token {
            source_ordinal =
                Some(u8::try_from(ordinal + 1).map_err(|_| FixtureLawError::InvalidAccount)?);
        }
    ...
    if transfer_class == LaneTransferClass::ProductionSettlement
        && source_ordinal == Some(rejection)
    {
...
fn classify_lane_transfer(facts: LaneTransferFacts) -> Option<LaneTransferClass> {
    ...
    let production_settlement = facts.source_ordinal.is_some()
        && !facts.authority_is_signer
    ...
    } else if production_settlement {
        Some(LaneTransferClass::ProductionSettlement)
    } else {
        None
    }
}
...
mod tests {
    ...
    #[test]
    fn production_settlement_classifier_is_unchanged() {
