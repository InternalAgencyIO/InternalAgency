...
const sourceBinding = { commit: "b".repeat(40), tree: "c".repeat(40) };
...
function reconcile(candidateRecords = records) {
...
test("all 50 exact source-bound records reconcile to one 7,500-page aggregate", () => {
...
test("reconciliation fails closed on missing, duplicate, failed, drifted, or overclaimed evidence", () => {
  ...
  sourceDrift[6].sourceBinding.commit = "d".repeat(40);
  assert.throws(() => reconcile(sourceDrift), /Shard 7 commit binding differs/);
