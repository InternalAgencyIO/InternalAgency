...
export const rewardEpochs = sqliteTable("reward_epochs", {
  id: text("id").primaryKey(), epochDateUtc: text("epoch_date_utc").notNull(), state: text("state", { enum: ["collecting", "published", "held"] }).notNull().default("collecting"), policyHash: text("policy_hash").notNull(), merkleRoot: text("merkle_root"), manifestDigest: text("manifest_digest"), eligibleWalletCount: integer("eligible_wallet_count").notNull().default(0), totalClaimableBaseUnits: text("total_claimable_base_units").notNull().default("0"), publishedAtUtc: text("published_at_utc"),
}, (table) => [uniqueIndex("reward_epochs_date_unique").on(table.epochDateUtc)]);
export const rewardClaims = sqliteTable("reward_claims", {
  id: text("id").primaryKey(), epochId: text("epoch_id").notNull().references(() => rewardEpochs.id), walletAddress: text("wallet_address").notNull(), amountBaseUnits: text("amount_base_units").notNull(), leaf: text("leaf").notNull(), proofJson: text("proof_json").notNull(), state: text("state", { enum: ["eligible", "held", "claimed", "expired"] }).notNull().default("eligible"), claimTransaction: text("claim_transaction"), createdAtUtc: text("created_at_utc").notNull(), claimedAtUtc: text("claimed_at_utc"),
