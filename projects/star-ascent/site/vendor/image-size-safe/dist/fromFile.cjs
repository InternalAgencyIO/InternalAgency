...
var ICNS = {
  ...
  calculate(input) {
    ...
    if (inputLength < SIZE_HEADER2) throw new TypeError("Invalid ICNS header");
    ...
    while (imageOffset < fileLength && imageOffset < inputLength) {
      ...
      if (entryLength < SIZE_HEADER2 || entryLength > fileLength - imageOffset) {
...
var processQueue = async () => {
  ...
  const promises2 = jobs.map(async ({ filePath, resolve: resolve2, reject }) => {
    let handle;
    try {
      handle = await fs__namespace.promises.open(path__namespace.resolve(filePath), "r");
    } catch (err) {
      return reject(err);
    }
    try {
