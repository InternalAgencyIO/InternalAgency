...
Repository evidence policy uses one human gate only: physical confirmation on a
Trezor Model T when an actual cryptographic signature is required. Editing this
repository policy is not a cryptographic-signature operation and requires no
Trezor confirmation. Non-signature predicates may close only from exact,
source-bound automated direct evidence. Human independent/native review is not
a prerequisite. Unobserved, inferred, stale, partial, or contradictory claims
remain `BLOCKED` / `HOLD`.

The exact predicate migration is source-controlled and nonactivating:

1. `SIGNED_LIVE_ESTATE_CANONICAL_MINT_DECISION_PACKET` -> `TREZOR_MODEL_T_SIGNED_LIVE_ESTATE_CANONICAL_MINT_DECISION_PACKET`
2. `EXTERNAL_CHECKPOINT_PRODUCTION_REVIEW_PACKET_SCOPED_OUTPUT` -> `EXTERNAL_CHECKPOINT_AUTOMATED_DIRECT_EVIDENCE_PACKET_SCOPED_OUTPUT`
3. `X_SOCIAL_PRODUCTION_REVIEW_PACKET_SCOPED_OUTPUT` -> `X_SOCIAL_AUTOMATED_DIRECT_EVIDENCE_PACKET_SCOPED_OUTPUT`
4. `PRIVACY_VAULT_FULL_LIFECYCLE_REVIEW_PACKET` -> `PRIVACY_VAULT_FULL_LIFECYCLE_AUTOMATED_DIRECT_EVIDENCE_PACKET`
5. `ZERO_UNACCEPTED_DEPENDENCY_FINDINGS_PACKET` -> `ZERO_UNRESOLVED_DEPENDENCY_FINDINGS_AUTOMATED_DIRECT_EVIDENCE_PACKET`
6. `ALL_50_LOCALES_ACCEPTED_NATIVE_REVIEW_PACKET` -> `ALL_50_LOCALE_CLAIMS_AUTOMATED_DIRECT_EVIDENCE_PACKET`
7. `INDEPENDENT_SECURITY_ECONOMIC_PRIVACY_LEGAL_REVIEW` / `INDEPENDENT_MULTI_DOMAIN_REVIEW_PACKET` -> `AUTOMATED_SECURITY_ECONOMIC_PRIVACY_LEGAL_EVIDENCE` / `AUTOMATED_MULTI_DOMAIN_DIRECT_EVIDENCE_PACKET`

The seven replacements alter predicate names and the one node identifier only.
They do not supply evidence, complete a node, authorize signing, or activate a
release. The retained V2 parity packet is repinned to this migrated graph only
after exact comparison preserves all 53 feature rows and requirements.
