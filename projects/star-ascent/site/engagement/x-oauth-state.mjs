...
export function issueXOAuthState({ nodeId, secret, now = new Date(), ttlMs = 5 * 60_000, nonce = undefined }) {
...
export function verifyXOAuthState({ state, secret, now = new Date() }) {
  ...
  if (now.valueOf() > payload.exp) throw new Error("OAuth state expired");
  return payload;
}

export function pkceVerifier({ state, secret, now = new Date() }) {
  verifyXOAuthState({ state, secret, now });
  return mac(secret, `pkce:${state}`);
}
