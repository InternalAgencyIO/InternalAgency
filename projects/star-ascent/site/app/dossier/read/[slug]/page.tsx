...
const SIGNAL_FOUR_ART: Record<"anchor" | "finale" | "relay" | "tension", SignalFourArt> = {
  anchor: {
    src: "/future/casino/nightflight/signal-four-hanoi-anchor-latex-lace-v2.webp",
    width: 720,
    height: 1280,
    alt: "Radiance, Ellie, AI ECE, and Alia, four fictional adult women, linking hands around a launch crystal while PAWS pounces beside them",
    caption: "SIGNAL FOUR // HANOI ANCHOR // PAWS CRYSTAL POUNCE",
  },
  finale: {
    src: "/future/casino/nightflight/signal-four-da-lat-finale-latex-lace-v2.webp",
    width: 720,
    height: 1280,
    alt: "Radiance, Ellie, AI ECE, and Alia, four fictional adult women, standing together at the arrival beacon",
    caption: "SIGNAL FOUR // DA LAT ARRIVAL",
  },
  relay: {
    src: "/future/casino/nightflight/signal-four-ninh-binh-relay-latex-lace-v2.webp",
    width: 720,
    height: 1280,
    alt: "Radiance, Ellie, AI ECE, and Alia, four fictional adult women, forming a linked relay while PAWS dashes through the crew",
    caption: "SIGNAL FOUR // NINH BINH RELAY // PAWS CREW DASH",
  },
  tension: {
    src: "/future/casino/nightflight/signal-four-orbital-tension-latex-lace-v3.webp",
    width: 720,
    height: 1280,
    alt: "Radiance, Ellie, AI ECE, and Alia, four fictional adult women, inside an orbital signal gallery while PAWS scouts the foreground",
    caption: "SIGNAL FOUR // ORBITAL GALLERY // PAWS SIGNAL SCOUT",
  },
};
...
const EN: Record<string, Copy> = {
  ...
  tokenomics: { label: "CANONICAL RECORD / 02", title: "TOKENOMICS", deck: "The B3 architecture retains the V2 feature set while making the IAT-wide lockdown law and adverse outcomes visible before execution.", state: "B3 ARCHITECTURE / V2 FEATURES RETAINED / MAINNET HOLD", blocks: [["FIXED SUPPLY", "1,000,000,000 IAT: Community 50%; Treasury 20%; Ecosystem 15%; Core Team 10%; Liquidity 5%. The on-chain mint and authority evidence are not yet public."], ["REWARD RESERVE", "Treasury, ecosystem and liquidity form an ordered, intentionally exhaustible 400M IAT reserve. The Genesis target unlocks 25% of each lane, or 100M total. New positions must be fully collateralized before acceptance."], ["RATES + CCC", "Simple annual rates, paid weekly without automatic compounding: core team 17%, standard user 10%, CCC Agent 28%, eligible downstream CCC associate 20%. A weekly public random draw pauses one CCC Agency and its snapshotted downstream group for that turn. Every exact protocol tie uses one final, exact-uniform, publicly verifiable roll over a precommitted candidate set. Full retained terms: /tokenomics."], ["IMMUTABLE IAT LOCKDOWN LAW", "Each protocol-local day has a 1% selection chance, except Friday at 66.67%. A selected day closes state-changing IAT actions for 24 hours, from 00:01 to 00:01 in fixed UTC+03:00. Read-only views and the Solana network continue normally; the law applies across IAT, not only the optional Privacy Vault."]], next: "OPEN GENESIS PROOF →" },
  "mint-manifest": { label: "OPERATOR RECORD / 03", title: "MINT MANIFEST", deck: "The sequence that must be rehearsed, signed physically, and evidenced without shortcuts.", state: "HOLD UNTIL SIGNER REHEARSAL", blocks: [["SIGNER GATE", "The fee payer must be signed on the Trezor device. Any required one-time mint-account signer must be treated as a rehearsal gate, never an invisible detail."], ["B3 TOKEN PROGRAM", "The intended B3 mint uses Token-2022 with the native IAT transfer-hook law. Default transfers remain public and the Privacy Vault is optional; no hidden mint path, transfer tax, or surprise authority belongs in the launch."], ["PUBLICATION", "Mint address, program, supply, authority evidence, allocation addresses, locks, and circulating-supply calculation must be published together."]], next: "OPEN GENESIS RUN →" },
...
const tailoredEN: Record<string, Copy> = {
  ...
  "technical-spec": { label: "SYSTEM RECORD / 09", title: "TECHNICAL SPEC", deck: "A readable technical contract between the signal, the chain, and the people verifying both.", state: "B3 REFERENCE EDITION", blocks: [["TOKEN STANDARD", "The intended B3 design uses Token-2022, nine decimals, a fixed supply target, and the native IAT transfer-hook law. Default IAT transfers remain public; shielded Privacy Vault transfers are an optional path."], ["CHAIN-DERIVED LAW", "The IAT-wide daily decision uses Solana-provided time and a lagged ancestor slot hash. It uses no external oracle, does not claim to pause Solana itself, and is publicly replayable without claiming threshold-VRF bias resistance."], ["NO HIDDEN LAYER", "No transfer tax, surprise authority, or opaque execution path belongs in the public design. The first B3 Devnet rehearsal and its evidence remain launch gates."]], next: "OPEN MINT MANIFEST →" },
...
export default function DossierReaderPage() {
  ...
  const radianceArt = params.slug === "broadcast-pack" || params.slug === "social-kit"
    ? SIGNAL_FOUR_ART.finale
    : params.slug === "genesis-run" || params.slug === "readiness"
      ? SIGNAL_FOUR_ART.relay
      : params.slug === "white-dossier"
        ? SIGNAL_FOUR_ART.anchor
        : SIGNAL_FOUR_ART.tension;
  ...
  return <main className="reader-page">
    ...
    <section className="reader-hero"><div><p>{record.label}</p><h1>{record.title}</h1><strong>{record.state}</strong><p className="reader-deck">{record.deck}</p>{params.slug === "genesis-proof" && <p className="reader-live-note">PUBLIC RECORD SLOT: canonical Genesis links appear here only after verified publication.</p>}</div><figure>
      ...
      <img src={radianceArt.src} width={radianceArt.width} height={radianceArt.height} style={{ aspectRatio: "9 / 16" }} loading="lazy" decoding="async" alt={radianceArt.alt} />
      <figcaption>{radianceArt.caption}</figcaption>
    </figure></section>
