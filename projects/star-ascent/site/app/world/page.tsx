...
export default function WorldPage() {
  ...
  return <main className="world-page"><div className="world-stars" aria-hidden="true" /><nav className="world-nav"><a href="/">IA<span>///</span></a><div><a href="/dossier">{language === "tr" ? "DOSYA" : "DOSSIER"}</a><button onClick={() => setLanguage(language === "en" ? "tr" : "en")}>{language === "en" ? "TR" : "EN"}</button></div></nav><section className="world-hero"><p>{t.kicker}</p><h1>{t.title}</h1><p>{t.intro}</p></section><figure className="world-art"><img src="/images/radiance-roller-rave.webp" alt={language === "tr" ? "Radiance, canlı sinyalin sahne operatörü" : "Radiance, stage operator of the live signal"} /><figcaption>RADIANCE // THE SIGNAL HAS A FACE</figcaption></figure><section className="world-stations">{t.stations.map(([number, title, text]) => <article key={number}><span>{number}</span><h2>{title}</h2><p>{text}</p></article>)}</section><section className="world-oath"><p>SCORPION GENERATION</p><h2>{t.oath}</h2><a href="/dossier">{t.footer}</a></section></main>;
}
