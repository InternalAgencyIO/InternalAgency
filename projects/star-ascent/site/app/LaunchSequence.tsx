...
export function LaunchSequence({ language }: { language: "en" | "tr" }) {
  ...
  return <section className="launch-sequence" aria-label="STAR ASCENT launch sequence"><div className="launch-fire" aria-hidden="true" /><div className="launch-sequence-copy"><p>{t.label}</p><div className="launch-lines" aria-live="off">{t.lines.slice(0, visible).map((line) => <span key={line}>{line}</span>)}</div><h2>{t.title}</h2><div className="launch-sequence-actions"><button type="button" onClick={() => { setDismissed(true); document.querySelector("#main-content")?.scrollIntoView({ behavior: "smooth" }); }}>{t.enter} <b>↓</b></button>{visible === t.lines.length && <a href="/launch">{t.launch} ↗</a>}</div></div></section>;
}
