  ...
  return <div className="activation-backdrop" role="presentation" onMouseDown={onClose}>
    <section ref={dialogRef} className="activation-terminal" role="dialog" aria-modal="true" aria-labelledby="activation-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="activation-panel">
        ...
        {tab === "claim" && <><p className="activation-lede">{copy.claimText}</p><div className="claim-slate"><span>{copy.claimStatus}</span><b>— — — —</b><small>NO MINT · NO TRANSACTION · NO TRANSFER</small></div></>}
        {tab === "broadcast" && <><p className="activation-kicker">{copy.broadcastText}</p><h3>{copy.broadcastTime}</h3><p className="activation-lede">{copy.broadcastNote}</p><a href="/dossier">{copy.return} →</a></>}
      </div>
