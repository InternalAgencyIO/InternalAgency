...
function repairLegacyEncoding(value: unknown): any {
  ...
  if (Array.isArray(value)) return value.map(repairLegacyEncoding);
  if (value && typeof value === "object" && !("$$typeof" in value)) {
    return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, repairLegacyEncoding(item)]));
  }
  return value;
}
...
const launchPlan = {
  en: {
    ...
    title: "A public, verifiable run-of-show.",
    note: "Times are UTC. Any change will be posted on this page before it is repeated elsewhere.",
    items: [
      ["13:30", "Preflight broadcast", "Safety briefing, disclosure recap, and channel verification. No wallet action."],
      ["14:00", "Genesis event", "Project introduction and launch documentation walkthrough."],
      ["After verification", "Address publication", "The mint address appears only after on-screen and website verification. No exact minute is promised."],
      ["After site update", "Registration opens", "Use the on-page status—not a direct message—to confirm availability."],
    ],
  },
  tr: {
    eyebrow: "LANSMAN AKIŞI",
    title: "Herkese açık, doğrulanabilir yayın planı.",
    note: "Saatler UTC'dir. Her değişiklik başka yerde paylaşılmadan önce bu sayfada yayımlanır.",
    items: [
      ["13:30", "Yayın öncesi kontrol", "Güvenlik bilgilendirmesi, açıklama özeti ve kanal doğrulaması. Cüzdan işlemi yoktur."],
      ["14:00", "Başlangıç etkinliği", "Proje tanıtımı ve lansman belgelerinin adım adım incelenmesi."],
      ["Doğrulamadan sonra", "Adres yayını", "Mint adresi yalnızca ekranda ve web sitesinde doğrulandıktan sonra görünür. Kesin dakika vaat edilmez."],
...
export default function Home() {
  ...
  return <>
    ...
    <section className="brief"><p className="eyebrow">{m.brief}</p><h2>{m.briefTitle}</h2><div className="brief-console">{m.lines.map(line => <p key={line}>{line}</p>)}</div></section>
    ...
    <section className="token-disclosure" aria-labelledby="token-disclosure-title"><p className="eyebrow">{disclosure.eyebrow}</p><h2 id="token-disclosure-title">{disclosure.title}</h2><p className="disclosure-intro">{disclosure.intro}</p><dl>{disclosure.items.map(([term, detail]) => <div key={term}><dt>{term}<span>{disclosure.pending}</span></dt><dd>{detail}</dd></div>)}</dl><p className="disclosure-gate"><strong>{disclosure.gate}</strong> <a href="#evidence">{language === "en" ? "Review the publication gate ↓" : "Yayın eşiğini incele ↓"}</a></p></section>
    <section className="evidence-pack" id="evidence" aria-labelledby="evidence-title"><p className="eyebrow">{evidence.eyebrow}</p><h2 id="evidence-title">{evidence.title}</h2><p className="evidence-intro">{evidence.intro}</p><ol>{evidence.items.map(([title, detail], index) => <li key={title}><span aria-hidden="true">0{index + 1}</span><div><h3>{title}</h3><p>{detail}</p></div><strong>{evidence.status}</strong></li>)}</ol><p className="evidence-note">{evidence.note}</p><a className="document-link" href={language === "en" ? "/disclosures/iat-allocation-authority-checklist-en.txt" : "/disclosures/iat-allocation-authority-checklist-tr.txt"} download>{evidence.download}</a></section>
    ...
    <section className="schedule" aria-labelledby="schedule-title"><p className="eyebrow">{schedule.eyebrow}</p><h2 id="schedule-title">{schedule.title}</h2><p className="schedule-note">{schedule.note}</p><ol>{schedule.items.map(([time, title, description]) => <li key={time}><time>{time}</time><div><h3>{title}</h3><p>{description}</p></div></li>)}</ol></section>
    <section className="roadmap" id="roadmap"><p className="eyebrow">{t.staged}</p><div className="steps">{t.phases.map(([title, description], i) => <article key={title}><span>0{i + 1}</span><h3>{title}</h3><p>{description}</p></article>)}</div></section>
    ...
    <section className="anti-scam" aria-labelledby="anti-scam-title"><p className="eyebrow">{antiScam.eyebrow}</p><h2 id="anti-scam-title">{antiScam.title}</h2><p className="anti-scam-intro">{antiScam.intro}</p><div className="safety-steps">{antiScam.steps.map(([label, guidance], index) => <article key={label}><span aria-hidden="true">0{index + 1}</span><h3>{label}</h3><p>{guidance}</p></article>)}</div><p className="safety-warning"><strong>{antiScam.warning}</strong></p></section>
