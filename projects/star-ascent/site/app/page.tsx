...
const tokenPolicyVoice = {
  ...
  tr: {
    briefLines: ["[01] İleri Akıl ilk kamuya açık bölümünü açıyor.", "[02] Önerilen IAT ödül sözleşmesi kamusal, ancak aktif değil.", "[03] Kod, fonlama, inceleme ve kanıt eşleşene kadar mainnet BEKLET durumundadır."],
    yield: "ÖDÜL SÖZLEŞMESİ",
    none: "ÖNERİ / BEKLET",
    questions: [["Resmî token adresi nerede?", "Resmî mint adresi yalnızca burada, sabitlenmiş resmî duyuruda ve lansman canlı yayınında paylaşılır."], ["Şimdi cüzdan bağlamam gerekiyor mu?", "Hayır. Kayıt ve staking açık değildir. Seed phrase veya özel anahtarınızı asla paylaşmayın."], ["IAT yatırım vaadi midir?", "Token fiyatı, kâr veya garantili piyasa değeri vaat edilmez. Önerilen ödül oranları kabul edilmiş ve tam teminatlandırılmış pozisyonlar için sözleşme kurallarıdır ve aktif değildir."]],
    risk: "© 2026 İleri Akıl. IAT yüksek riskli ve spekülatiftir. Token fiyatı, kâr veya garantili piyasa değeri vaat edilmez.",
  },
};
...
const tokenDisclosure = {
  ...
  tr: {
    ...
    note: "Ekonomi politikası /tokenomics adresinde kamusaldır. Resmî mint, program kasaları, ödül yönlendiricisi ve yetki kanıtı henüz yayımlanmadı; program aktif değildir.",
    pending: "BEKLİYOR",
    items: [
      ...
      ["Tahsis ve serbest bırakma", "%50/%20/%15/%10/%5 tahsis, 400M sıralı ödül rezervi, hak ediş takvimleri, yıllık ödül oranları, haftalık CCC Wildcard ve evrensel tek çekilişli eşitlik bozma yöntemi öneri olarak yayımlandı. Zincir üstü uygulama ve kanıt bekliyor."],
    ],
    gate: "Lansman açısından kritik herhangi bir uygulama veya kanıt beklemedeyken dağıtım ve ödül aktivasyonu başlamamalıdır.",
  },
};
...
const documentPack = {
  ...
  tr: {
    ...
    tokenPolicy: "IAT ekonomi politikası V2'yi aç",
...
export default function Home() {
  ...
  const t = language === "tr"
    ? { ...repairLegacyEncoding(copy.tr), ...trPublicVoice, ...tokenPolicyVoice.tr }
    : { ...repairLegacyEncoding(copy.en), ...tokenPolicyVoice.en };
  const m = { ...repairLegacyEncoding(manifesto[language]), lines: tokenPolicyVoice[language].briefLines };
  ...
  return <>
    ...
    <section className="signal-ticker" aria-label={language === "en" ? "Project signal" : "Proje sinyali"}><div>{language === "en" ? "THE SIGNAL IS OPEN  //  BUILD THE STRANGE  //  BUILD THE BEAUTIFUL  //  BUILD IN PUBLIC  //  STAR ASCENT" : "SİNYAL AÇIK  //  TUHAFI İNŞA ET  //  GÜZELİ İNŞA ET  //  HERKESİN GÖZÜ ÖNÜNDE İNŞA ET  //  STAR ASCENT"}</div></section>
    ...
    <section className="outer-comms">
      ...
      <div><p className="eyebrow">{language === "en" ? "OUTER COMMS // SECURE" : "DIŞ İLETİŞİM // GÜVENLİ"}</p><h2>{language === "en" ? <>THE SIGNAL<br />IS ALIVE.</> : <>SİNYAL<br />CANLI.</>}</h2><p>{language === "en" ? "A constellation answers from the black. One room. One rising ship. Every line is moving." : "Takımyıldız karanlıktan cevap veriyor. Tek oda. Yükselen tek gemi. Her hat hareket ediyor."}</p></div>
    </section>
    <section className="genesis-console" aria-labelledby="genesis-console-title"><div className="genesis-console-heading"><p>{language === "en" ? "GENESIS // COMMAND CENTER" : "BAŞLANGIÇ // KOMUTA MERKEZİ"}</p><h2 id="genesis-console-title">{language === "en" ? <>CHOOSE YOUR<br />ENTRY VECTOR.</> : <>GİRİŞ VEKTÖRÜNÜ<br />SEÇ.</>}</h2><span>{language === "en" ? "Three public surfaces. One verification order." : "Üç kamuya açık alan. Tek doğrulama sırası."}</span></div><div className="genesis-console-grid"><a href="/dossier"><small>01 // {language === "en" ? "CANONICAL RECORD" : "KANONİK KAYIT"}</small><strong>{language === "en" ? "OPEN THE DOSSIER" : "DOSYAYI AÇ"}</strong><em>{language === "en" ? "Read the public record, token design target, and evidence gates." : "Kamu kaydını, token tasarım hedefini ve kanıt eşiklerini oku."}</em><b>↗</b></a><a href="/network"><small>02 // {language === "en" ? "READ THE CHAIN" : "ZİNCİRİ OKU"}</small><strong>{language === "en" ? "OPEN IAT NETWORK" : "IAT AĞINI AÇ"}</strong><em>{language === "en" ? "Inspect Solana, player wallets, IAT positions, rewards, and CCC state from one read-only screen." : "Solana, oyuncu cüzdanı, IAT pozisyonu, ödül ve CCC durumunu tek salt okunur ekranda incele."}</em><b>↗</b></a><a href="#schedule"><small>03 // {language === "en" ? "WITNESS" : "TANIK OL"}</small><strong>{language === "en" ? "BROADCAST WINDOW" : "YAYIN PENCERESİ"}</strong><em>{language === "en" ? "The window opens exactly; transaction execution remains human-approved." : "Pencere tam zamanında açılır; işlem yürütme insan onaylı kalır."}</em><b>↓</b></a></div></section>
    ...
    <figure className="scorpion-story">
      ...
      <img src="/images/scorpion-launch-control-v1.webp" width={1728} height={909} loading="lazy" decoding="async" alt={language === "en" ? "Adult Scorpion Generation stage director and light operators at a starship launch-rave control deck" : "Yıldız gemisi fırlatma-rave kontrol güvertesinde yetişkin Akrep Nesli sahne direktörü ve ışık operatörleri"} />
    </figure>
    <figure className="crew-arrival"><div className="crew-arrival-copy"><p>{language === "en" ? "THE CREW // ARRIVAL WINDOW" : "EKİP // VARIŞ PENCERESİ"}</p><h2>{language === "en" ? <>THE LIGHTS<br />ARE ALREADY<br />ON.</> : <>IŞIKLAR<br />ÇOKTAN<br />AÇIK.</>}</h2><span>{language === "en" ? "Not a promise. A scene set for the people arriving with their eyes open." : "Bir vaat değil. Gözleri açık gelenler için kurulmuş bir sahne."}</span></div>
      {/* eslint-disable-next-line @next/next/no-img-element -- Vinext runtime does not safely support next/image; this local asset has exact intrinsic dimensions. */}
      <img src="/images/scorpion-crew-arrival-v1.webp" width={1672} height={941} loading="lazy" decoding="async" alt={language === "en" ? "Adult stage commander and light operators in a red spacecraft launch hangar" : "Kırmızı uzay aracı fırlatma hangarında yetişkin sahne komutanı ve ışık operatörleri"} />
    </figure>
    <section className="ascent-ritual" aria-label={language === "en" ? "The ascent ritual" : "Yükseliş ritüeli"}>
      {/* eslint-disable-next-line @next/next/no-img-element -- Vinext runtime does not safely support next/image; this local asset has exact intrinsic dimensions. */}
      <img src="/images/ascent-ritual-v1.webp" width={1536} height={1024} loading="lazy" decoding="async" alt={language === "en" ? "A crew witnessing the STAR ASCENT launch beneath a luminous constellation" : "Işıklı bir takımyıldızın altında STAR ASCENT fırlatmasını izleyen ekip"} />
      <div><p className="eyebrow">{language === "en" ? "THE SCORPION GENERATION" : "AKREP NESLİ"}</p><h2>{language === "en" ? <>WE DON&apos;T WATCH<br />THE FUTURE.</> : <>GELECEĞİ<br />İZLEMEYİZ.</>}</h2><p>{language === "en" ? "We gather at the edge of the signal: artists, operators, believers and night people. The ship rises; the room becomes a constellation." : "Sinyalin kıyısında buluşuruz: sanatçılar, operatörler, inananlar ve gece insanları. Gemi yükselir; oda bir takımyıldızına dönüşür."}</p></div>
    </section>
    <section className="statement" id="mission"><p className="eyebrow">{m.premise}</p><h2>{m.thesis}</h2><p>{m.body}</p></section>
    <section className="token" id="token"><div><p className="eyebrow">{t.token}</p><h2>{m.clear}</h2></div><div className="token-grid"><div><span>{t.supply}</span><b>{t.supplyTarget}</b></div><div><span>{t.network}</span><b>Solana</b></div><div><span>{t.presale}</span><b>{language === "en" ? "None" : "Yok"}</b></div><div><span>{t.yield}</span><b>{t.none}</b></div></div><p className="note">{disclosure.note} <a href="/tokenomics">{language === "en" ? "Read the complete proposed terms →" : "Önerilen şartların tamamını oku →"}</a></p></section>
