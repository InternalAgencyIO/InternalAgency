...
export async function POST(request: Request) {
  ...
  try { verifyNodeChallenge({ wallet: input.wallet, nonce: input.nonce, issuedAtUtc: challenge.issued_at_utc, expiresAtUtc: challenge.expires_at_utc, origin, signatureBase58: input.signatureBase58 }); } catch { return json({ error: "INVALID_OR_EXPIRED_WALLET_PROOF" }, 400); }
  const now = new Date().toISOString(), bindingId = crypto.randomUUID();
