...
export async function GET(request: Request) {
  const url = new URL(request.url), nodeId = url.searchParams.get("nodeId") ?? "", clientId = env.X_CLIENT_ID, secret = env.X_OAUTH_STATE_SECRET, redirectUri = env.X_OAUTH_REDIRECT_URI ?? callbackDefault;
  ...
  const state = issueXOAuthState({ nodeId, secret }), verifier = pkceVerifier({ state, secret }), authorize = new URL("https://x.com/i/oauth2/authorize");
