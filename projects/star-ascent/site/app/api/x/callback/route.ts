...
export async function GET(request: Request) {
  ...
  const clientId = env.X_CLIENT_ID, secret = env.X_OAUTH_STATE_SECRET, redirectUri = env.X_OAUTH_REDIRECT_URI ?? callbackDefault;
  ...
  const update = await env.DB.prepare("UPDATE node_bindings SET x_user_id = ?, state = 'active', activated_at_utc = ? WHERE id = ? AND state = 'pending' AND x_user_id IS NULL AND country_code IS NOT NULL").bind(xUserId, now, payload.nodeId).run();
  if (update.meta.changes !== 1) return fail("node-already-bound");
  return Response.redirect(new URL(`/rewards?binding=active&node=${encodeURIComponent(payload.nodeId)}`, "https://internalagency.io"), 302);
}
