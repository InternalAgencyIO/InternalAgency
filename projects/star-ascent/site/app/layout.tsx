...
export default async function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  return <html lang={htmlLanguageTag(locale)} dir={localeDirection(locale)} data-locale-ready="true"><head>{locale !== "en" && !turkishHost ? <link rel="preload" href={`/i18n/${locale}.json`} as="fetch" crossOrigin="anonymous" /> : null}</head><body className={`${geistSans.variable} ${geistMono.variable} antialiased`}>{children}<DocumentLinkUpgrade /><CrewSignal /><DossierDock /><LocaleRuntime locale={locale} promptCopy={promptCopy} publicPath={publicPath} turkishHost={turkishHost} /><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(structuredData).replace(/</g, "\\u003c") }} /></body></html>;
}
