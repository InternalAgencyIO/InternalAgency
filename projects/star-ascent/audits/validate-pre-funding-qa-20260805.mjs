...
check(
  currentHydration.completedShards === 6 && currentHydration.requiredShards === 50
    && currentHydration.completedPages === 900 && currentHydration.fullProfilePages === 7500
    && currentHydration.failedPages === 0 && currentHydration.incompletePages === 0,
  "current hydration summary drifted",
);
check(currentHydration.batches.length === 6 && currentHydration.records.length === 6, "current hydration inventory drifted");
const activeSources = [
  ...
  ["da625223f82c1fea291584b8e194c2748d4f2a27", "507476476032837eb72ca5796c7fab66bbf9e9eb"],
  ["5987d0c66071e5f7fe9bba3e436f81c824e64670", "538777fe6a22f0f3b845902b94d37d6ca7de7e3b"],
  ["a8f04f60a0f1125f8111764466db1ec2b700fe7b", "948032aef60c0e15caec220fabcb6add38fe5df3"],
];
const activeBatchDigests = [
  ...
  "7a5fc6757b53386b69c58692c78eb0016613130300a56a7df522cfe9d0eab346",
  "f4d5b72e998a603d3276b0c4ed4e104d9ff8b54fb38025adb875e2be258e8e9d",
  "6ef31f6aef5e5404ad1cf19c9f76d49eeccbfe8f89ff1f2990689c42f537e204",
];
...
for (const [index, currentBatch] of currentHydration.batches.entries()) {
  ...
  check(currentBatch.schema === "iat-v2-hydration-shard-batch-summary/v1", `current hydration batch ${shardIndex} schema drifted`);
  check(currentBatch.status === "PARTIAL_PASS_NOT_AGGREGATE", `current hydration batch ${shardIndex} overclaims aggregate proof`);
  check(currentBatch.sourceBinding.commit === activeSources[index][0] && currentBatch.sourceBinding.tree === activeSources[index][1], `current hydration batch ${shardIndex} source drifted`);
  check(git("rev-parse", `${currentBatch.sourceBinding.commit}^{tree}`) === currentBatch.sourceBinding.tree, `current hydration batch ${shardIndex} tree is not Git-bound`);
  check(isAncestor(currentBatch.sourceBinding.commit, binding.commit), `current hydration batch ${shardIndex} does not precede the evidence source`);
  check(currentBatch.sourceBinding.scopePath === binding.sitePath && currentBatch.sourceBinding.scopeTree === binding.siteTree, `current hydration batch ${shardIndex} site scope drifted`);
  check(git("rev-parse", `${currentBatch.sourceBinding.commit}:${currentBatch.sourceBinding.scopePath}`) === binding.siteTree, `current hydration batch ${shardIndex} site tree is not Git-bound`);
  check(JSON.stringify(currentBatch.range) === JSON.stringify({ shardStart: shardIndex, shardEnd: shardIndex }), `current hydration batch ${shardIndex} range drifted`);
  check(currentBatch.evidenceSetSha256 === activeBatchDigests[index], `current hydration batch ${shardIndex} digest drifted`);
...
const activeLocales = ["ar", "az", "be", "bg", "bn", "bs"];
...
for (const [index, currentRecord] of currentHydration.records.entries()) {
  const shardIndex = index + 1;
  check(currentRecord.schema === "iat-v2-hydration-shard-record/v2" && currentRecord.status === "SHARD_PASS_NOT_AGGREGATE", `current hydration shard ${shardIndex} status drifted`);
  check(Number.isFinite(Date.parse(currentRecord.recordedAtUtc)), `current hydration shard ${shardIndex} timestamp drifted`);
  check(currentRecord.sourceBinding.commit === activeSources[index][0] && currentRecord.sourceBinding.tree === activeSources[index][1], `current hydration shard ${shardIndex} source drifted`);
  check(git("rev-parse", `${currentRecord.sourceBinding.commit}^{tree}`) === currentRecord.sourceBinding.tree, `current hydration shard ${shardIndex} tree is not Git-bound`);
  check(isAncestor(currentRecord.sourceBinding.commit, binding.commit), `current hydration shard ${shardIndex} does not precede the evidence source`);
  check(currentRecord.sourceBinding.scopePath === binding.sitePath && currentRecord.sourceBinding.scopeTree === binding.siteTree, `current hydration shard ${shardIndex} site scope drifted`);
  check(git("rev-parse", `${currentRecord.sourceBinding.commit}:${currentRecord.sourceBinding.scopePath}`) === binding.siteTree, `current hydration shard ${shardIndex} site tree is not Git-bound`);
  check(currentRecord.catalogSha256 === binding.catalogSha256, `current hydration shard ${shardIndex} catalog drifted`);
  check(currentRecord.profile.shardIndex === shardIndex && currentRecord.profile.shardCount === 50 && currentRecord.profile.locale === activeLocales[index], `current hydration shard ${shardIndex} profile drifted`);
  check(JSON.stringify(currentRecord.profile.hosts) === JSON.stringify(["internalagency", "ileriakil"]), `current hydration shard ${shardIndex} hosts drifted`);
  check(currentRecord.profile.canonicalRoutes === 25 && JSON.stringify(currentRecord.profile.engines) === JSON.stringify({ chromium: 50, firefox: 50, webkit: 50 }), `current hydration shard ${shardIndex} coverage drifted`);
  check(currentRecord.profile.assignedPages === 150 && currentRecord.profile.fullProfilePages === 7500, `current hydration shard ${shardIndex} page totals drifted`);
  check(currentRecord.profile.assignedJobsSha256 === activeAssignments[index], `current hydration shard ${shardIndex} assignment drifted`);
  check(currentRecord.profile.fullProfileJobsSha256 === "1f035cca45792e63056e961dc90b6783f1d210d62968b837e3dc8216746ccbd7", `current hydration shard ${shardIndex} full profile drifted`);
  check(JSON.stringify(currentRecord.result) === JSON.stringify({ completedPages: 150, failedPages: 0, incompletePages: 0 }), `current hydration shard ${shardIndex} result drifted`);
  check(Object.values(currentRecord.assurance).every((value) => value === false) && currentRecord.mainnetStatus === "UNSCHEDULED_HOLD", `current hydration shard ${shardIndex} assurance drifted`);
}
...
const justSuperseded = supersededCurrents[0];
check(justSuperseded.schema === "iat-v2-hydration-historical-evidence/v1", "just-superseded hydration schema drifted");
check(justSuperseded.status === "HISTORICAL_PARTIAL_NOT_CURRENT_SOURCE_PROOF", "just-superseded hydration overclaims current proof");
check(justSuperseded.sourceBinding.commit === "a20e6bd829f0fc02bb494df99a35b7d337295b88", "just-superseded source commit drifted");
check(justSuperseded.sourceBinding.tree === "083c31afabcffa91ec921b269834a2016a4ad92c", "just-superseded source tree drifted");
check(justSuperseded.sourceBinding.siteTree === "48f969f6cbeb9bfaba6f6b102fa7bb7d6ed07146", "just-superseded site tree drifted");
check(justSuperseded.supersededBySiteTree === binding.siteTree, "just-superseded hydration successor drifted");
check(git("rev-parse", `${justSuperseded.sourceBinding.commit}^{tree}`) === justSuperseded.sourceBinding.tree, "just-superseded source tree is not Git-bound");
check(git("rev-parse", `${justSuperseded.sourceBinding.commit}:${justSuperseded.sourceBinding.sitePath}`) === justSuperseded.sourceBinding.siteTree, "just-superseded site tree is not Git-bound");
check(isAncestor(justSuperseded.sourceBinding.commit, binding.commit), "just-superseded source does not precede current source");
check(justSuperseded.sourceBinding.catalogSha256 === binding.catalogSha256, "just-superseded catalog drifted");
check(
  justSuperseded.completedShards === 4 && justSuperseded.requiredShards === 50
    && justSuperseded.completedPages === 600 && justSuperseded.fullProfilePages === 7500
    && justSuperseded.failedPages === 0 && justSuperseded.incompletePages === 0,
  "just-superseded hydration summary drifted",
);
check(justSuperseded.batches.length === 2 && justSuperseded.records.length === 4, "just-superseded hydration inventory drifted");
const expectedCurrentBatches = [
  {
    range: { shardStart: 3, shardEnd: 4 },
    evidenceSetSha256: "1ccf7547b6640a270758a22b40e8fc16bdd84585d7a166cfb54d8f8960e4557e",
    commit: "b51e8fd864a688179b195af90b01be18945ad5af",
    tree: "ed2684854d857e958236c6848c6f78efae8c9fc7",
  },
  {
    range: { shardStart: 5, shardEnd: 6 },
    evidenceSetSha256: "dd8b5f5536b63dcf8cf060705ce143440b56ee7a009c9280d080dd410c41934a",
    commit: "a20e6bd829f0fc02bb494df99a35b7d337295b88",
    tree: "083c31afabcffa91ec921b269834a2016a4ad92c",
  },
];
for (const [index, currentBatch] of justSuperseded.batches.entries()) {
  const expected = expectedCurrentBatches[index];
  check(JSON.stringify(currentBatch.range) === JSON.stringify(expected.range), `current hydration batch ${index + 1} range drifted`);
  check(currentBatch.evidenceSetSha256 === expected.evidenceSetSha256, `current hydration batch ${index + 1} digest drifted`);
  check(currentBatch.sourceBinding.commit === expected.commit && currentBatch.sourceBinding.tree === expected.tree, `current hydration batch ${index + 1} source drifted`);
  check(git("rev-parse", `${currentBatch.sourceBinding.commit}^{tree}`) === currentBatch.sourceBinding.tree, `current hydration batch ${index + 1} tree is not Git-bound`);
  check(isAncestor(currentBatch.sourceBinding.commit, binding.commit), `current hydration batch ${index + 1} does not precede the evidence source`);
  check(currentBatch.sourceBinding.scopePath === justSuperseded.sourceBinding.sitePath && currentBatch.sourceBinding.scopeTree === justSuperseded.sourceBinding.siteTree, `just-superseded hydration batch ${index + 1} scope drifted`);
  check(git("rev-parse", `${currentBatch.sourceBinding.commit}:${currentBatch.sourceBinding.scopePath}`) === justSuperseded.sourceBinding.siteTree, `just-superseded hydration batch ${index + 1} site tree is not Git-bound`);
}
const currentLocales = ["be", "bg", "bn", "bs"];
...
const currentSourceBindings = [
  ["b51e8fd864a688179b195af90b01be18945ad5af", "ed2684854d857e958236c6848c6f78efae8c9fc7"],
  ["b51e8fd864a688179b195af90b01be18945ad5af", "ed2684854d857e958236c6848c6f78efae8c9fc7"],
  ["a20e6bd829f0fc02bb494df99a35b7d337295b88", "083c31afabcffa91ec921b269834a2016a4ad92c"],
  ["a20e6bd829f0fc02bb494df99a35b7d337295b88", "083c31afabcffa91ec921b269834a2016a4ad92c"],
];
for (const [index, record] of justSuperseded.records.entries()) {
  ...
  check(Number.isFinite(Date.parse(record.recordedAtUtc)), `current shard ${shardIndex} timestamp drifted`);
  check(record.sourceBinding.commit === currentSourceBindings[index][0] && record.sourceBinding.tree === currentSourceBindings[index][1], `current shard ${shardIndex} source drifted`);
  ...
  check(record.sourceBinding.scopePath === justSuperseded.sourceBinding.sitePath && record.sourceBinding.scopeTree === justSuperseded.sourceBinding.siteTree, `just-superseded shard ${shardIndex} scope drifted`);
  check(record.catalogSha256 === justSuperseded.sourceBinding.catalogSha256, `just-superseded shard ${shardIndex} catalog drifted`);
...
const immediatelySuperseded = supersededCurrents[1];
...
check(immediatelySuperseded.supersededBySiteTree === justSuperseded.sourceBinding.siteTree, "immediately superseded hydration successor drifted");
...
const supersededCurrent = supersededCurrents[2];
...
check(evidence.limitations.some((item) => /six of fifty/u.test(item) && /not aggregate proof/u.test(item)), "current partial hydration limitation missing");
